...
%% ESO enabled
ESO_enabled = 1;
ESO_type_enabled = 1; % 1 = ESO, 0 = FTESO

if ESO_type_enabled == 1
  ESO_type = 1; % ESO
else
  ESO_type = 2; % FTESO
end

% 1 = square(step) disturbance, 2 = high-frequency sinosoidal disturbance
% 3 = low-frequency sinosoidal disturbance
disturbance_type = 2; 
Disturbance_enabled = 1;

w0 = 60; % bandwith of ESO
w0_FTESO = 10; % bandwith of FTESO
%% simulink execution
animation_plot = 0;
T_final = 100;
t = 0:Ts:T_final;
y = sim('ASMC_sim_CSEI_FTESO.slx');
time = y.tout;
...
configuration = ['ASMC on/off: ',num2str(ASMC_enabled),  '  ESO on/off: ',num2str(ESO_enabled),  '  FESO on/off(1= ESO, 2 = FTESO): ',num2str(ESO_type)];
disp(configuration)
...
RMSE_dx = rmse(disturbance_true(:,1), disturbance_hat(1,:)');
RMSE_dy = rmse(disturbance_true(:,2), disturbance_hat(2,:)');
RMSE_dn = rmse(disturbance_true(:,3), disturbance_hat(3,:)');


RMSE = ['RMSE dX: ', num2str(RMSE_dx),  '  RMSE dY: ', num2str(RMSE_dy), '  RMSE dN: ', num2str(RMSE_dn)];
disp(RMSE)
...
xlabel('time (s)'); ylabel('Fx (N)'); legend('True', 'Estimated'); grid on; ylim([-3 3]);
title('Disturbance estimation (ESO)');
subplot(3,1,2)
plot(time, disturbance_true(:,2), 'LineWidth', 2); hold on;
plot(time, disturbance_hat(2,:), ':', 'LineWidth', 2); hold on;
xlabel('time (s)'); ylabel('Fy (N)'); legend('True', 'Estimated'); grid on; ylim([-3 3]);
subplot(3,1,3)
plot(time, disturbance_true(:,3), 'LineWidth', 2); hold on;
plot(time, disturbance_hat(3,:), ':', 'LineWidth', 2); hold on;
xlabel('time (s)'); ylabel('Moment (N*M)'); legend('True', 'Estimated'); grid on; ylim([-3 3]);
