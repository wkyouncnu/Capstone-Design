...
20. **예제마다 오프라인 운동모델로 먼저, VRX 는 그다음.** 같은 제어기·버튼 블록을 같은 함수로 만들고
    Gazebo 자리(ROS Pub/Sub)만 `MotionModel` 로 바꾼 쌍둥이를 둔다 (3주차 `W03_0_offline`, 4주차 `W04_0_offline`,
    5주차 `wamv_sim.py`, 6주차 `*_offline`, 7~10주차 `W0X_0_offline`). 운동방정식의 유도는 3주차 1-8 절 한 곳에 둔다.
21. **VRX 에서 첫 유효 샘플은 시각으로 자르지 않는다.** odom 도착이 0.5 s 를 넘길 때가 있다 (2026-09-19 9주차).
