...
> [!warning] `md2pdf.sh` 를 고치면 PDF 18편이 전부 다시 뽑힌다
> 주석 한 줄만 고쳐도 그렇다. 조판 도구 목록에 들어 있기 때문이고, 그것이 맞다 —
> 이 스크립트는 HTML 을 조립하므로 내용이 바뀌면 결과가 달라질 수 있다.
> 다만 **바이너리 PDF 18개가 통째로 커밋에 들어간다.** 사소한 수정은 모아서 한 번에 한다.
>
> 임시 파일 `.md2pdf_$$.html` 은 EXIT·INT·TERM 트랩으로 지운다. 중단되면 남아서
> 자동 커밋에 쓸려 들어간다 — 2026-09-18 에 실제로 두 개가 들어갔다.
> `.gitignore` 에도 `.md2pdf_*.html` 이 있다. 둘 다 지우지 말 것.
