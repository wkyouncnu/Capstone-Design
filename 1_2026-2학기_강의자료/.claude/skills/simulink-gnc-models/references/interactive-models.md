...
## 실시간 화면 — 제어 모델의 필수품 (교수 지시 2026-09-24)

모델이 무엇을 하는지는 **블록도가 아니라 돌아가는 화면**이 말한다. 제어 주차의 모델은
돌리는 동안 다음을 보여 준다.

| 자리 | 그리는 것 |
|---|---|
| 왼쪽 큰 칸 | 궤적. 그 위에 **선체 모양과 선수 방향** (가로축 `y` 동쪽, 세로축 `x` 북쪽) |
| 오른쪽 ① | **지령 대비 응답** — `u_ref` 와 `u` 를 겹쳐서 |
| 오른쪽 ② | **지령 대비 응답** — `psi_ref` 와 `psi` 를 겹쳐서 |
| 오른쪽 ③ | 나머지 상태값 `v`, `r` 과 추력 `F_L`, `F_R` |

- 원형은 3주차 `W03_animate.m` (궤적 + 선수 방향) 과 `W03_teleop_plot.m` (좌우 추력 겹쳐 그리기)
- **오프라인과 VRX 쌍둥이가 같은 화면 함수를 쓴다.** 신호만 갈아 끼우고 그리는 코드는 하나다
- Animate 상자는 포트 없는 서브시스템으로 두고, `Clk` 와 `From` 태그로 값을 받는다
  (포트 번호를 바꾸면 `AnimateFcn` 인자 순서도 함께 바뀌므로 빌더 주석에 순서를 적는다)
- 느린 PC 를 위해 `animate` · `animate_every` 로 끄고 건너뛸 수 있게 한다 (`W03_setup.m` 규약)
- **이 화면의 캡처를 그 주차 자료에 싣는다.** 속도 제어·헤딩 제어가 도는 전체 화면 한 장씩
