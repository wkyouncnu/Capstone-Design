...
- `WNN_setup` 이 **다른 변수에서 계산해 두는** 값이 있다. 7주차의 `beta_c = deg2rad(current_direction)` 이 그렇다
- 모델은 `beta_c` 를 읽는다. setup 을 부른 **뒤에** 스크립트에서 `current_direction` 만 바꾸면 조류 방향은 그대로다
- 2026-09-18 — 이렇게 잰 7주차 "동쪽 조류" 표 셋이 실은 북쪽 조류였다. `W07_compare` 는 `beta_c` 를 다시 계산하고 있어서 맞았다
...
- 기본값 하나(7주차 `sw_mode`)를 바꾸면 같은 모델로 잰 **모든 표**가 영향을 받는다. 조류가 없는 조건은 그대로여도 조류가 있는 조건은 바뀐다
