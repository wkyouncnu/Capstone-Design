...
```matlab
check_lines(모델, true)      % 겹침 · 블록 관통 · 꺾임 3회+ · 매달림 · 사선 · 블록겹침
```

- **여섯 다 0 이 합격선.** 최상위와 모든 서브시스템에 대해
- **블록겹침 0** — 블록 사각형끼리, 이름표(블록 아래 글자 줄, 약 14 px)가 다른 블록에,
  ...
  `drop_tag`·`feed_from` 이 `spot_free` 로 빈자리를 재서 놓는다. "이름표 위 선" 은 참고값
- **사선 0** — 모든 선 토막은 가로 아니면 세로. 몇 px 기운 선도 도면에서는 비뚤게 보임
  (2026-09-19 W06_P2 Dcompare 에서 교수 지적). 원인은 거의 둘
