...
| MATLAB | R2024b + Simulink + ROS Toolbox |
| 배포 파일 | `W05_simulink/` (모델 2개 + 스크립트 11개: setup · plot · animate · anim_capture · compare · sweep · lookahead_demo · crab_demo · vrx_run · build · tidy_layout), `W05_matlab/` (연구실 참고 코드 — 목록은 문서 끝 "배포 파일") |
...
| 창 | 내용 |
|---|---|
| **W05 실시간 유도 화면** | 도는 동안 항적과 유도 지표를 네 칸으로 보여 줌 |
...
- 5주차가 답해야 하는 질문은 하나 더 있음 — **경로를 따라가고 있는가**
- 그래서 4주차와 같은 네 칸 배치를 쓰되 오른쪽 칸의 내용이 5주차 것으로 바뀜

![실시간 유도 화면 — 오프라인 모델](W05_simulink/img/W05_anim_offline.png)

- 조건 — `W05_0_offline`, LOS 유도, $\Delta = 10$ m, $R = 5$ m, $u_{\text{ref}} = 1.5$ m/s,
  $K_{p,\psi} = 400$ · $K_{d,\psi} = 200$, $K_{p,u} = 300$ · $K_{i,u} = 100$, 조류 0, 240 s 주행 (2026-09-24 실측)

| 자리 | 그리는 것 | 왜 그 자리에 있는가 |
|---|---|---|
| 왼쪽 큰 칸 | 항적 + **웨이포인트 · 수락반경** + 선체 모양과 선수 방향 | 궤적선만으로는 뱃머리가 어디를 보는지 안 보임. 초록 원이 **지금 겨누는 웨이포인트**임 |
| 오른쪽 ① | $\psi_{\text{ref}}$ (회색 점선) 와 $\psi$ (주황) 를 **겹쳐서** (deg, `ssa`) | 유도가 낸 지령을 헤딩 루프가 따라가는지 한 칸에서 읽힘 |
| 오른쪽 ② | **$y_e$ 와 0 선** | 5주차의 핵심 지표. 항적만 보면 "대충 맞는 것 같다" 에서 멈추고 몇 미터인지 읽히지 않음 |
...
- ③ 의 지령선은 설정값 $u_{\text{ref}}$ 가 아니라 **$u_{\text{ref}} \times \text{gate}$** 임. 완주하면 `gate` 가 0 이 되어 지령이 0 으로 떨어짐 (그림에서 207 s 부근)

**어디를 보면 유도가 되고 있는가**

| 볼 곳 | 정상이면 | 아니면 |
|---|---|---|
| ② 의 $y_e$ | 출발 직후 −20 m 에서 **0 으로 올라와 붙음.** 웨이포인트를 돌 때만 $\pm 5$ m 안쪽으로 튀었다 다시 0 | 0 으로 안 붙으면 유도가 아니라 헤딩 루프를 의심 (① 을 볼 것) |
| ① 의 두 선 | 응답선이 지령선에 **붙어서 같이 감** | 붙지 않으면 $K_{p,\psi}$ 가 작거나 추력이 포화임 |
| ① 의 세로선 | $\pm 180°$ 이음매를 지날 때만 생김 — `ssa` 로 접은 값이라 그럼. 고장이 아님 | — |
| 왼쪽 칸의 초록 원 | 수락반경에 닿을 때마다 **다음 웨이포인트로 옮겨 감** | 한 점에 머물러 있으면 전환 판정이 걸린 것 (C절 `sw_mode` 참고) |
| ③ 의 $F_L$ · $F_R$ | 직선 구간에서 **같은 값으로 모임** — 요 모멘트가 0 | 계속 벌어져 있으면 아직 도는 중 |
...
> [!important] 오프라인 모델과 VRX 쌍둥이가 화면 함수 하나(`W05_animate.m`)를 같이 씀
> 갈아 끼우는 것은 신호를 어디서 뽑았는지뿐임 — 운동방정식이냐, 참값 오도메트리냐
> 따라서 위 그림과 E-4 의 VRX 화면은 **같은 함수가 그린 같은 배치**이며 나란히 놓고 읽을 수 있음

> [!note] 모델 안에서는 포트 없는 회색 상자 `Animate` 하나임
> 신호는 `From` 태그로 받으므로 **제어 신호 사슬에 선이 한 줄도 늘지 않음**
> 받는 태그는 `x_n` · `y_n` · `psi` · `psi_ref` · `y_e` · `u` · `gate` · `FL` · `FR` · `wp_idx` 열 개이며, 전부 **이미 있던 태그**임
...
> W05_anim_capture('offline','Stop',120)  % 120초에서 끊어 찍기
> ```
> 조건(게인 · 유도 설정 · 정지 시간 · 페이싱)을 구조체로 함께 돌려주므로 그림과 조건이 어긋나지 않음
...
- 실시간 화면에서 **배가 20 m 벗어난 곳에서 출발해 경로에 붙는 것**을 눈으로 확인. 오른쪽 ② 의 $y_e$ 가 −20 m 에서 0 으로 올라옴
...
> VRX 모델에도 오프라인과 같은 `Animate` 블록이 들어 있고 **같은 함수**(`W05_animate.m`)가 그림
...
![실시간 유도 화면 — VRX 연동](W05_simulink/img/W05_anim_vrx.png)

- 조건 — `W05_1_vrx`, **3주차 경량 모델**(`W03_vrx_lite/run_vrx.sh lite`), `ROS_DOMAIN_ID=7`, 스폰 ENU $(-532.3,\ 200.0)$,
  페이싱 1.072(= MATLAB 이 20초 동안 잰 RTF), 240 s 주행에 벽시계 241.7 s, 유도·게인은 위 오프라인과 동일 (2026-09-24 실측)
- 경량 모델의 Gazebo RTF 는 **0.989** 였음 (`W03_vrx_lite/measure_vrx.sh 60`, 60초, 표준편차 0.001, 화면 49.5 fps)
- 읽는 법은 위 A절의 네 칸 표와 **완전히 같음.** 같은 함수가 그린 같은 배치이기 때문임

| 볼 곳 | 오프라인 | VRX | 뜻 |
|---|---|---|---|
| 완주 시각 (③ 의 지령이 0 으로 떨어지는 시점) | 약 207 s | 약 197 s | 초기 선수각이 달라 첫 구간에서 벌어짐 |
| ② 의 정착 후 $y_e$ | 0 부근 | 0 부근 | **유도 성능은 시뮬레이터가 바뀌어도 같음** |
| ③ 의 정상상태 $u$ | 1.50 m/s | 1.50 m/s | 추력·항력 계수가 같다는 증거 |

> [!note] 두 화면의 ② 가 거의 겹침
> 왼쪽 항적은 눈으로는 구분이 안 됨. **숫자로 대조하는 것은 E-6** 이며, 이 화면은 "도는 동안 무엇을 보고 있어야 하는가" 를 위한 것임
...
- [ ] **실시간 화면** 네 칸의 뜻을 알고, ② 의 $y_e$ 가 0 으로 붙는 것을 눈으로 확인했음
...
| `W05_simulink/W05_animate.m` | 실시간 화면 네 칸 그리기 — 오프라인·VRX 공용 (모델 안 `Animate` 블록이 호출) |
| `W05_simulink/W05_anim_capture.m` | 그 화면을 조건과 함께 PNG 로 재현 (`img/W05_anim_offline.png` · `W05_anim_vrx.png`) |
