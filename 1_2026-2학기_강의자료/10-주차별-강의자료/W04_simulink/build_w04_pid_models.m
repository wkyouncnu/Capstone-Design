...
% BUILD_W04_PID_MODELS  4주차 PID 입문 실습 모델 3개를 생성한다.
%
...
%
%   전제
%     >> W04_pid_setup      % 파라미터가 base workspace 에 있어야 한다
%
...
%   모델이 깨졌을 때 이 스크립트를 다시 돌리면 원래대로 복구된다.
    ...
    names = {'W04_P1_pid_step','W04_P2_pid_byhand','W04_P3_boat_speed'};
    for k = 1:numel(names)
...
'du = (X - (Xu + Xuu*abs(u))*u)/mm;']);
    ...
    stamp(m, ROW+400, [ ...
        ...
        '  >> W04_pid_compare(''boat'')   안티와인드업 유무를 겹쳐 그린다']);
