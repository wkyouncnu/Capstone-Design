function V = W04_vrx_record(T_end)
% W04_VRX_RECORD  VRX 에서 2단계 시나리오를 그대로 돌리고 항적을 기록한다.
%
%   >> V = W04_vrx_record;          % 80초
%   >> V = W04_vrx_record(80);
%
%   W04_2_turn.slx 이 하는 일과 같다. 다만 결과를 **기록**해서
%   오프라인 모델(W04_5_offline)과 겹쳐 볼 수 있게 만든다.
%
...
n  = ros2node(sprintf('/w06rec_%d', randi(9999)));
c  = onCleanup(@() clear('n'));
...
fprintf('\n  >> out = sim(''W04_5_offline''); W04_offline_plot(out, V)\n');
end
...
% W04_2_turn / W04_5_offline 의 Scenario 블록과 같은 코드
if t < 20
    FL = 200;  FR = 200;
elseif t < 40
    FL = 300;  FR =  50;
