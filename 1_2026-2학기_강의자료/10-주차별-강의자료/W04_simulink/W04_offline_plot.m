function S = W04_offline_plot(out, V)
% W04_OFFLINE_PLOT  5단계 오프라인 모델의 결과를 그림 네 장으로 그린다.
%
...
%     S.R_port    좌선회 반경 [m]
%
%   두 번째 인수로 W04_vrx_record 의 결과를 넘기면 VRX 항적을 겹쳐 그린다.
%     >> V = W04_vrx_record;
%     >> out = sim('W04_5_offline');
%     >> W04_offline_plot(out, V)
...
figure('Name','W04 오프라인 WAM-V','Color','w','Position',[80 60 1080 720]);
...
if hasV
    sgtitle('W04 5단계 — 오프라인 WAM-V(파랑) vs VRX(빨강)');
else
    sgtitle('W04 5단계 — 오프라인 WAM-V (직진 / 우선회 / 좌선회)');
end
...
if hasV
    ...
    fprintf('\n  같은 시나리오를 VRX 로 돌린 것이 W04_2_turn 이다.\n');
    fprintf('  >> V = W04_vrx_record;  로 기록하면 겹쳐 그릴 수 있다.\n');
end
end
