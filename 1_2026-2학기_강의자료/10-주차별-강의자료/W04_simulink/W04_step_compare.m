function R = W04_step_compare(stage, T_end)
% W04_STEP_COMPARE  3·4단계 제어기를 오프라인 쌍둥이로 먼저 돌리고, VRX 가 떠 있으면 같은 것을 VRX 로 돌려 겹친다.
%
%   >> R = W04_step_compare(3)        % 헤딩 제어       W04_3_heading_offline  -> W04_3_heading
%   >> R = W04_step_compare(4)        % 속도 + 헤딩     W04_4_inner_loop_offline -> W04_4_inner_loop
%   >> R = W04_step_compare(3, 40)    % 40 초 (기본)
%
%   순서 — 4주차 H·I 절
...
if stage == 3
    mOff = 'W04_3_heading_offline';     mVrx = 'W04_3_heading';
else
...
f = figure('Name', sprintf('W04 %d단계 — 오프라인 vs VRX', stage), 'Color','w', 'Position',[80 80 900 400+200*(stage==4)]);
n = 1 + (stage == 4);
...
exportgraphics(f, fullfile(d, sprintf('W04_%d_offline_vs_vrx.png', stage)), 'Resolution', 110);
fprintf('   그림 저장: img/W04_%d_offline_vs_vrx.png\n', stage);
end
