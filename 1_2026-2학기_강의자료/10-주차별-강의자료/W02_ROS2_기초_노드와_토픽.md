...
| **필요할 때** — 참고 | 1-3 워크스페이스 · 2-5 조사 명령어 · 2-6 rqt · RViz2 | 명령어가 기억나지 않을 때 돌아와 찾는다 |
...
> 이 유의 사항을 지금 손으로 만들어 봐야, 실선 LiDAR·카메라 드라이버(흔히 BEST_EFFORT)를 만났을 때 스스로 알아챔. VRX 는 전부 RELIABLE 이라 시뮬레이터에서는 걸리지 않는다 (4주차 2-3 절).
...
> [!warning] 이 식은 turtlesim 좌표계의 식이다 — 3주차부터는 규약이 바뀐다
> turtlesim 은 수학 교과서와 같은 평면을 쓴다. $x$ 오른쪽, $y$ 위쪽, 각 $\theta$ 는 $x$ 축에서 **반시계**가 $+$ 다.
> 3주차부터 쓰는 NED 는 $x$ 가 **북**(위쪽), $y$ 가 **동**(오른쪽), 각 $\psi$ 는 북에서 **시계**가 $+$ 다.
>
...
> | 회전 속도 | $\omega$ | $r$ (요각속도) |
>
> 두 식은 **모양이 같다** — 둘 다 "두 번째 축의 차이, 첫 번째 축의 차이" 순서다. NED 에서는 첫 번째 축이 북이기
> 때문에 같은 모양의 식이 시계방향 각을 준다. 식을 외우지 말고 **어느 축이 첫 번째인지**를 볼 것.
> 그리고 $v$ 는 3주차부터 **옆으로 미끄러지는 속도**를 뜻한다. 앞으로 가는 속도는 $u$ 다.
