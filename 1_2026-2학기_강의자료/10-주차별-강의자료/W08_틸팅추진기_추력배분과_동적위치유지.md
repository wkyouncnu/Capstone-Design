...
|---|---|
| 환경 | 1\~7주차 그대로 |
| 배포 파일 | `W08_simulink/` (모델 2개 + 스크립트 9개: setup · plot · animate · anim_capture · alloc_demo · sway_demo · vrx_run · build · tidy_layout) |
...
- 설정 파일을 실행하면 곧바로 돌고 **실시간 화면**이 뜸

### C-0. 실시간 화면 — 네 칸을 읽는 법

- 4\~7주차와 **같은 배치**임. 왼쪽 큰 칸 하나 + 오른쪽 세 칸
- 오프라인 모델과 VRX 쌍둥이가 **같은 함수**(`W08_animate.m`)를 씀. 신호를 어디서 뽑는지만 다름

![오프라인 DP 실시간 화면](W08_simulink/img/W08_anim_offline.png)

| 자리 | 그리는 것 | 왜 거기 있나 |
|---|---|---|
| 왼쪽 큰 칸 | 위치 평면. 항적, 선체와 선수 방향, 계단 목표 $\boldsymbol{\eta}_{\text{raw}}$(회색 □)와 기준모델 목표 $\boldsymbol{\eta}_d$(초록 ○), 그 위에 **추진기 2기의 방위각·추력 화살표** | DP 는 "목표 위에 배가 겹쳐 있는" 것이 정상임. 겹쳐 보이면 성공 |
| 오른쪽 ① | **위치 오차** $e_x$ · $e_y$ 와 거리 $\lvert e\rvert$, 0 선 | 평면 그림만으로는 "대충 목표 근처"에서 멈춤. 몇 cm 인지 읽는 칸 |
| 오른쪽 ② | $\psi_{\text{ref}}$ 와 $\psi$ 를 겹쳐서 (deg, `ssa`) | 위치를 지키면서 **선수각도** 지키는가 |
| 오른쪽 ③ | 추진기 2기의 추력 $T_i$(왼쪽 축)와 방위각 $\delta_i$(오른쪽 축), $\pm45^\circ$ 한계선 | 배분이 실제로 무엇을 시켰는가 |

> [!note] 추진기는 **두 기**임. 제어입력이 넷일 뿐
> 배분 행렬 $\mathbf{T}_e$ 가 4열인 것은 추진기가 넷이어서가 아니라,
> 한 기의 힘을 $(F_x, F_y)$ 두 성분으로 쪼갰기 때문임 (§1-6)
> VRX 의 WAM-V 는 **후방 방위추진기 2기**임 (`wamv_aft_thrusters.xacro`)
> 그래서 화살표도 둘, ③ 칸의 곡선도 추력 둘 · 방위각 둘임

**어디를 보면 DP 가 되고 있는가**

| 보는 곳 | 되고 있다 | 안 되고 있다 |
|---|---|---|
| ① 위치 오차 | 계단 뒤 $\lvert e\rvert$ 가 **0 으로 내려와 붙음** | 0 으로 안 내려오거나 계속 출렁임 |
| 평면의 초록 ○ | 선체 **안에** 들어와 있음 | 선체 밖에 떨어져 있음 |
| ② 선수각 | 횡이동 구간에서 $\psi$ 가 **0° 그대로** / 45° 계단을 따라감 | 위치를 잡느라 뱃머리가 따라 돎 |
| 평면의 화살표 | 바람이 오는 쪽(북동)을 **거슬러** 비스듬히 틈 | 두 화살표가 같은 방향으로만 붙어 있음 |
| ③ 방위각 | $\pm45^\circ$ 안에서 완만하게 움직임 | 한계선에 붙어 떨림 (요구 힘이 너무 작을 때 나타남) |

- 위 그림은 `W08_anim_capture('offline')` 로 찍음 — 기본 조건, $t = 290$ s
  (계단 셋 60 · 140 · 220 s 가 모두 지나가고 **제자리 선수 45° 가 정착한** 시점)
- 그때 화면 값: $\lvert e\rvert$ 0.08 m, $\Delta\psi$ 2.8°, 좌현 +7.0 N @ −18.0°, 우현 +18.9 N @ −5.9°
- 270\~290 s 구간 실측: 위치 오차 RMS **0.098 m**, 선수각 오차 RMS **4.07°**, 평균 추력 **14.1 N**
  (2026-09-25, 데스크톱, 바람 6 m/s @ 45° · 파랑 $H_s$ 0.3 m)
- 그림이 느리면 `W08_setup.m` 의 `animate = 0` 또는 `animate_every` 를 키움
...
### E-3. 실시간 화면 — 오프라인과 **같은 네 칸**

- 읽는 법은 §C-0 과 완전히 같음. 바뀐 것은 신호를 어디서 뽑는지뿐임
  (운동방정식 → Gazebo ground truth odometry)

![VRX DP 실시간 화면](W08_simulink/img/W08_anim_vrx.png)

```matlab
setappdata(0, 'W08_skip_run', true);
W08_anim_capture('vrx')          % 페이싱은 실측 RTF 를 그대로 씀
```

- 실측 조건 (2026-09-25, 데스크톱, `W03_vrx_lite/run_vrx.sh lite`, `sydney_regatta`,
  `ROS_DOMAIN_ID = 7`, 스폰 NED $(177.0,\ -444.3)$ m, **측정 RTF 1.068**, 290 s, 벽시계 393.6 s)
- 270\~290 s 구간: 위치 오차 RMS **0.003 m**, 선수각 오차 RMS **0.03°**, 평균 추력 **0.2 N**
- 화면에서 바로 읽히는 세 가지

| 어디 | 무엇이 보이나 |
|---|---|
| ② 선수각 | 스폰 선수각 약 **137°** 에서 시작해 제자리에서 0° 로 돌아섬 — 배가 **옮겨 가지 않고** 돎 (①의 $\lvert e\rvert$ 가 그동안 0 근처) |
| ① 위치 오차 | 60 s · 140 s 계단에서만 1.3 m 까지 솟았다가 곧 0 으로 붙음. 220 s 선수 계단에서는 **거의 솟지 않음** — 위치를 지키며 돌았다는 뜻 |
| ③ 방위각 | 요구 힘이 몇 N 밖에 안 되어 $\delta_i$ 가 $\pm45^\circ$ 한계 사이를 계속 오감 |

> [!note] 방위각이 한계에 붙어 떠는 것은 고장이 아님
> 요구 힘이 0 에 가까우면 그 힘의 **방향**은 정의가 거의 무의미해짐
> 작은 잡음에도 각도가 크게 바뀌므로 $\delta$ 가 떨림 — §E-4 의 방위각 변화율 수치와 같은 현상임
> 오프라인 화면에서도 220 s 뒤(요구 힘이 작아지는 구간)에 같은 떨림이 보임

> [!caution] VRX 의 WAM-V 는 **후방 2추진기**임
> §1-6 에서 본 대로 이 배의 제어입력은 **4개**(추진기 2기 × $F_x, F_y$)이지 추진기 4기가 아님
> 그래서 화면의 화살표도 둘임. 4추진기(예: 전방 2기 추가) 배분은 이 URDF 로는 보일 수 없음
> 경량 모델 `W03_vrx_lite/wamv_lite.urdf` 에도 `wamv/{left,right}_chassis_engine_joint` 와
> `JointPositionController` 가 들어 있어 **방위각 명령이 그대로 먹음** (실측 확인)
...
- [ ] **실시간 화면 네 칸**을 §C-0 의 표대로 읽을 수 있음 (오차 · 선수각 · 추력/방위각)
- [ ] 파랑 필터 on/off 를 비교해 추력 변화율을 기록했음
- [ ] VRX 에서 **개루프 횡이동**을 확인했음 (25 s 에 옆으로 약 24 m)
- [ ] VRX DP 를 돌려 오프라인과 대조했음 (실시간 화면도 §E-3 과 같은 모양으로 떴음)

### 관찰 기록

- [ ] 횡이동 구간에서 두 추진기의 각도와 부호를 적어 두었음
- [ ] **화살표가 외란에 맞서 어느 쪽으로 트는지** 눈으로 확인하고 적어 두었음
- [ ] 요구 힘이 작아지는 구간에서 $\delta_i$ 가 $\pm45^\circ$ 사이를 떠는 것을 확인했음
...
| `W08_simulink/W08_animate.m` | **실시간 화면 네 칸** — 위치 평면(추진기 화살표 포함) + 위치 오차 · 선수각 · 추력/방위각 (§C-0) |
| `W08_simulink/W08_anim_capture.m` | 실시간 화면을 돌려 PNG 로 저장 (`'offline'` · `'vrx'`) |
