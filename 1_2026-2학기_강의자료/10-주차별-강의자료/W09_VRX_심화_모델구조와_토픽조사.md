...
- $R_M$, $R_N$ 은 지구 곡률반경 (자오선·묘유선 곡률반경, 3주차 1-5)
...
- 순서: **센서 모델로 먼저**(2-8-0, VRX 불필요) → 같은 `RateMeter` 로 VRX 토픽을 잼(2-8-1) → QoS(2-8-2) → **실시간 화면**(2-8-3)
- 4주차부터는 제어기가 Simulink 안에서 토픽을 받음. 터미널에서 본 숫자와 **Simulink 가 받는 숫자가 같은지** 먼저 확인해 두는 절
- 2-8-0 과 2-8-1 의 모델은 도는 동안 **창 하나**가 열려 GPS 점 · 수신 주기 · 자이로를 그림. 화면 구성은 **2-8-3** 에 있음

- 배포 폴더 = 받은 `1_2026-2학기_강의자료` 폴더 안 `10-주차별-강의자료` 의 Windows 경로
- `W09_simulink/` 안에 받는 것 — 모델 `W09_0_offline` · `W09_1_sensor_rates` · `W09_2_qos_test`, 빌더 `build_w09_models.m`, 스크립트 `W09_setup.m` · `W09_offline_run.m` · `W09_rates_run.m`, **실시간 화면 `W09_animate.m` · `W09_anim_capture.m`**
...
### 2-8-3. 실시간 화면 — 센서가 제대로 오고 있는가

- 2-8-0 과 2-8-1 의 모델은 돌아가는 동안 **창 하나**가 열리고, 그 창이 **네 칸**으로 나뉨
- 4주차 화면(2-0-1)이 답한 질문은 "시킨 대로 따라갔는가" 였음. 9주차는 제어 주차가 아니므로 질문이 다름 — **센서가 무엇을, 얼마나 자주, 얼마나 정확히 주는가**

| 자리 | 그리는 것 | 왜 그 자리에 있는가 |
|---|---|---|
| 왼쪽 큰 칸 | **참값 항적**(파랑)과 **GPS 로 본 위치 점**(주황)을 겹쳐서. 그 위에 선체 모양과 선수 방향 | 센서로 본 배와 실제 배가 같은 그림 위에 있어야 차이가 보임 |
| 오른쪽 ① | **수신 주기** — `hz_gps` · `hz_imu` 와 설계값 20 · 100 Hz (점선) | 2-3-7 의 `ros2 topic hz` 를 그림으로 본 것. 점선에 붙는지만 보면 됨 |
| 오른쪽 ② | **센서 대 참값** — 자이로가 준 $r$ (보라)과 참값 $r$ (굵은 회색)을 겹쳐서 | 잡음과 바이어스가 얹혀 있는 모습이 눈으로 보임 (1-5) |
| 오른쪽 ③ | **GPS 점이 있어야 할 자리에서 벗어난 양** — 북쪽 · 동쪽 성분과 그 거리 | 안테나가 선체 원점에 없다는 1-3 · 1-5 의 결론을 m 단위로 확인 |

- 가로축은 `y (동쪽)`, 세로축은 `x (북쪽)` — **위쪽이 북쪽**이며 해도와 같은 방향임 (3주차 1-5)
- GPS 점은 **안테나 자리**이고 선체 원점이 아님. 그래서 화면은 IMU 선수각으로 레버암을 되돌린 자리에 선체를 그리고, 날것의 GPS 점은 따로 찍음. **두 점이 떨어져 보이는 것이 이 주차가 가르치려는 그림**임
- ③ 의 **기준**만 모델마다 다름 — 오프라인은 **참값**, VRX 는 **지금까지의 평균**. VRX 모델은 센서 토픽 세 개만 구독하고 ground truth 는 구독하지 않기 때문임 (그것이 2-8-1 의 목적임)

#### 켜고 끄기

```matlab
W09_setup            % animate = 1, animate_every = 0.25 가 기본값
W09_offline_run      % 창이 열린 채로 돎
```

- 느린 노트북이면 `animate = 0` 으로 끄거나 `animate_every` 를 키움. 20 초 실행의 벽시계가 **1.9 초(끔) → 4.1 초(켬)** 로 늘어남 (2026-09-25 기준 PC, 다른 작업이 없을 때). 같은 PC 에서 다른 것이 돌고 있으면 더 걸림
- 그림 그리는 일은 센서 모델과 아무 상관이 없음. 끈 실행과 켠 실행의 **수치는 같음**

#### 오프라인 — `W09_0_offline`

![오프라인 실시간 화면](W09_simulink/img/W09_anim_offline.png)

- 조건 — `W09_anim_capture('offline')`, `W09_setup` 기본값(좌 −200 N / 우 +200 N 제자리 좌선회), 20 초, `Ts` 0.005 s, VRX 불필요 (2026-09-25 실측)

| 읽는 법 | 뜻 |
|---|---|
| 왼쪽의 **주황 원** | 배는 제자리에서 도는데 GPS 는 **반지름 0.85 m 의 원**을 그림. 참값(파랑 점)은 가운데에 가만히 있음 — 1-5 절의 안테나 위치가 그대로 보임 |
| ① 두 선이 점선에 붙음 | 20.05 · 100.05 Hz. 센서 모델은 시뮬레이션 시간으로 세므로 설계값과 같음 (2-8-0) |
| ② 보라가 회색을 따라감 | 자이로가 참값 $r$ 을 따라가되 **잡음이 얹혀 있음**. 약 −21 °/s 언저리에서 떠는 폭이 표준편차 0.0091 rad/s ($\approx 0.52$ °/s) 임 (2-8-0) |
| ③ 검은 선이 0.85 에 평평함 | 북쪽 · 동쪽 성분은 선수각을 따라 ±0.85 m 로 출렁이지만 **거리는 일정**함. 안테나까지의 거리는 배가 어디를 보든 변하지 않음 |

#### VRX — `W09_1_sensor_rates`

![VRX 실시간 화면](W09_simulink/img/W09_anim_vrx.png)

- 조건 — `W09_anim_capture('vrx')`, 3주차 **경량 VRX**(`W03_vrx_lite/run_vrx.sh`, `--render-engine-gui ogre`), `ROS_DOMAIN_ID=7`, 20 초 (2026-09-25 실측)
- 이 모델은 배에 추력을 주지 않음. 그래서 항적은 **한 점**이고, ③ 이 그 점이 cm 단위로 얼마나 흔들리는지를 보여 줌

| 읽는 법 | 뜻 |
|---|---|
| 왼쪽 점이 한자리 | 정상. 튀면 Subscribe 가 **첫 메시지 전에 내는 0 버스**를 그린 것임 (CLAUDE.md §11) |
| $\psi_{\text{IMU}}$ 가 45.0° | 스폰 선수각. IMU 쿼터니언이 제대로 오고 있다는 뜻 |
| ② 잡음만 보임 | 배가 서 있으므로 참값 $r$ 은 0 임. 0 둘레로 ±1\~2 °/s 흔들리는 것이 VRX IMU 의 잡음 |
| ③ 20 초에 ±0.05 m | 배가 바람 · 파랑에 천천히 밀리는 양. GPS 가 **cm 단위로 살아 있다**는 뜻 |

> [!important] 이 절의 VRX 확인은 **경량 모델**로 했음
> 2-2 절의 토픽 전수조사는 카메라 · LiDAR 가 필요하므로 `bash run_vrx.sh full` 로 띄워야 함
> 여기서는 **센서 주기와 값만** 보므로 카메라 · LiDAR 를 끈 경량 모델로 충분하고, RTF 가 높아 화면이 매끄러움 (3주차 2-3)

> [!warning] 주기를 **재는** 실행에서는 화면을 끔
> `RateMeter` 는 받은 건수를 **시뮬레이션 시각**으로 나누고, 이 모델은 **벽시계**로 돎 (Pacing)
> 화면을 켜면 그리는 일이 벽시계를 먹어 페이싱이 밀리고, 시뮬레이션 1초당 건수가 **올라감**
> 실제로 위 캡처 실행에서 GPS 가 22.75 Hz 로 나왔음 — 설계값 20 Hz 보다 높음
> 그래서 `W09_rates_run` 은 `animate` 를 **스스로 0 으로 두었다가 끝나고 되돌림.** 2-8-1 의 표는 그렇게 잰 값임

> [!note] 모델 안에서는 포트 없는 회색 상자 `Animate` 하나임
> 신호는 `From` 태그로 받으므로 **신호 사슬에 선이 한 줄도 늘지 않음**
> 상자를 열면 `W09_animate.m` 을 부르는 MATLAB Function 하나가 들어 있음
> **오프라인과 VRX 가 이 함수 하나를 같이 씀** — 갈아 끼우는 것은 값을 어디서 뽑았는지뿐임 (센서 모델이냐, VRX 토픽이냐)
...
| 11 | **실시간 화면으로 센서 보기** | 네 칸 — GPS 점이 참값 둘레 0.85 m 원, 자이로가 참값 + 잡음 (2-8-3) |
...
- [ ] 실시간 화면(2-8-3) 네 칸이 각각 무엇을 그리는지 말할 수 있음
- [ ] 화면에서 **GPS 점이 참값 둘레로 반지름 0.85 m 의 원**을 그리는 것을 보았음
- [ ] 화면에서 **자이로가 참값을 따라가되 잡음이 얹혀 있는 것**을 보았음
- [ ] 주기를 **재는** 실행에서는 화면을 끈다는 것을 앎 (`W09_rates_run` 이 자동으로 함)
