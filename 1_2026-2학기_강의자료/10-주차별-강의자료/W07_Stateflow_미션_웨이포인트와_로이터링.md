...
|---|---|
| 환경 | 1\~6주차 그대로 |
| 배포 파일 | `W07_simulink/` (모델 2개 + 스크립트 7개: setup · plot · animate · anim_capture · vrx_run · build · tidy_layout) |
...
- 4·5·6주차와 **같은 네 칸 배치**임. 오른쪽 세 칸의 내용만 7주차 것으로 바뀜
- 6주차까지 화면이 답한 질문은 "시킨 대로 따라갔는가" 였음 (4주차 2-0-1, 5주차 A절)
- 7주차가 답해야 하는 질문은 하나 더 있음 — **지금 무엇을 할 차례이고, 언제 바뀌었는가**

![실시간 미션 화면 — 오프라인 모델](W07_simulink/img/W07_anim_offline.png)

- 조건 — `W07_0_offline`, 웨이포인트 5개, `loiter_wp = 3`, $r_d = 18$ m, 2바퀴,
  $p_c = 0.313$, `crab_comp = 1`, $\Delta = 10$ m, $R = 5$ m, $u_{\text{ref}} = 1.5$ m/s,
  $K_{p,\psi} = 400$ · $K_{d,\psi} = 200$, $K_{p,u} = 300$ · $K_{i,u} = 100$, 조류 0,
  330 s 주행 (2026-09-25 실측)

| 자리 | 그리는 것 | 왜 그 자리에 있는가 |
|---|---|---|
| 왼쪽 큰 칸 | 항적 + 웨이포인트 · 수락반경 + **로이터 중심과 반경 원** + 선체 모양과 선수 방향 | 궤적선만으로는 뱃머리가 어디를 보는지 안 보임. **선체 색이 곧 지금 상태**이고, 그 뜻은 아래 범례에 있음 |
| 오른쪽 ① | **상태 타임라인** — `mode` 가 시간에 따라 바뀌는 모양을 계단으로 | **7주차의 핵심 지표.** 배 색은 "지금" 만 말하고 "언제" 를 말하지 못함. 미션 모델에서 가장 중요한 수치는 궤적 오차가 아니라 **전이 시각**임 (A절 바로 앞의 콜아웃) |
| 오른쪽 ② | $\psi_{\text{ref}}$ (회색 점선) 와 $\psi$ (주황) 를 **겹쳐서** (deg, `ssa`) | 미션이 고른 지령을 헤딩 루프가 따라가는지 한 칸에서 읽힘 |
| 오른쪽 ③ | 왼쪽 축에 $u_{\text{ref}}$ · $u$, 오른쪽 축에 $F_L$ · $F_R$ | 그 응답을 무엇이 만들었는지 |

- 가로축은 `y (동쪽)`, 세로축은 `x (북쪽)` — **위쪽이 북쪽**이며 해도와 같은 방향임 (3주차 1-5)
- **배 색이 상태를 나타냄.** 왼쪽 칸 아래 범례가 같은 색을 상태 이름과 묶어 줌
...
- ② 의 점선은 모드에 따라 **다른 유도법칙이 낸 값**임 — `mode 1` 이면 `WpManager` 의 LOS,
  `mode 2` 면 `LoiterVF` 의 벡터필드. `ModeSwitch` 가 고른 뒤의 값을 받으므로 한 선에 둘이 섞여 있음.
  **전이 순간에 지령선이 툭 튀는 것이 정상임** — 법칙이 갈린 지점임
- ③ 이 `yyaxis` 로 축을 둘로 나눈 이유는 자릿수임. $u$ 는 1.x m/s, $F$ 는 수백 N 이라 한 축에 겹치면 $u$ 가 직선으로 보임
- ③ 의 지령선은 설정값 $u_{\text{ref}}$ 가 아니라 **$u_{\text{ref}} \times \text{gate}$** 임.
  `Finish` 로 들어가면 `gate` 가 0 이 되어 지령이 0 으로 떨어짐 (그림에서 314 s 부근)
- 오른쪽 ②·③ 의 **점선 세로선이 전이 시각**임. 세 칸이 같은 시간축을 쓰므로 위아래로 훑어 읽으면 됨
- 바퀴 수는 ① 의 제목에 있음. `TurnCount` 는 로이터에서 나오면 0 으로 되돌아가므로,
  화면은 **그 실행에서 채운 최댓값**을 보여 줌 (0 으로 돌아간 값을 그대로 쓰면 캡처에 `0.00 바퀴` 가 남음)

**어디를 보면 미션이 돌고 있는가**

| 볼 곳 | 정상이면 | 아니면 |
|---|---|---|
| ① 의 계단 | `1 WpFollow` → `2 Loiter` → `1 WpFollow` → `3 Finish` 로 **세 번 바뀜.** 제목에 전이 시각 세 개가 찍힘 (74.7 → 248.0 → 314.4 s) | 2 로 안 올라가면 `at_loiter` 판정을, 1 로 안 내려오면 회전수 세기를 의심 (1-4절) |
| ① 제목의 바퀴 수 | **로이터 구간에서 늘어 2.00 에서 빠져나옴** | 2.00 을 못 채우고 나오면 전이 조건 `[turns >= req_turns]` 를 볼 것 |
| 왼쪽 칸의 배 색 | 파랑 → 주황 → 파랑 → 회색 순으로 **세 번 바뀜** | 주황이 안 나오면 WP3 수락반경에 못 닿은 것임 |
| 왼쪽 칸의 초록 원 | 수락반경에 닿을 때마다 다음 웨이포인트로 옮겨 가고, **복귀 후 WP4 를 다시 집음** | 로이터 뒤에도 표식이 안 옮겨 가면 `idx` 가 멈춘 것임 (로이터 중에는 일부러 멈춤 — 1-4절) |
| ② 의 두 선 | 응답선이 지령선에 **붙어서 같이 감.** 전이 시각의 세로선에서만 지령선이 툭 튐 | 붙지 않으면 $K_{p,\psi}$ 가 작거나 추력이 포화임 |
| ② 의 세로 점프 | $\pm 180°$ 이음매를 지날 때만 생김 — `ssa` 로 접은 값이라 그럼. 고장이 아님 | — |
| ③ 의 지령선 | 회색(종료)이 되는 순간 **0 으로 떨어지고 $u$ 가 따라 내려옴** | 안 떨어지면 `gate` 가 `Finish` 에서 0 이 되는지 볼 것 |

| 설정 (`W07_setup.m`) | 뜻 |
|---|---|
| `animate = 1` | 0 으로 두면 그림을 그리지 않아 훨씬 빨리 돎 (330 s 주행 기준 29.4 s → 3.8 s, 2026-09-25 실측) |
| `animate_every = 0.25` | 몇 초마다 다시 그릴지. 키우면 빨라짐 |
| `boat_scale = 2` | 선체를 몇 배로 크게 그릴지. 1 이면 실제 크기(4.9 m) |

- 선체 모양은 5·6주차와 같음 — **선수는 세모, 선미는 네모**
- 빨간 사각형이 웨이포인트, 점선 원이 수락반경 $R$, 굵은 주황 점선 원이 로이터 궤도 $r_d$
...
> 받는 태그는 `x_n` · `y_n` · `psi` · `mode` · `turns` · `psi_ref` · `u` · `gate` · `FL` · `FR` · `idx`
> 열한 개이며, 전부 **이미 있던 태그**임. 속도 지령의 곱 $u_{\text{ref}} \times \text{gate}$ 는 그리는 함수가 함
...
> W07_anim_capture('offline','Stop',120)  % 120초에서 끊어 찍기
> ```
> 조건(게인 · 미션 설정 · 정지 시간 · 페이싱)과 **전이 시각·회전수**를 구조체로 함께 돌려주므로
> 그림과 조건이 어긋나지 않음. 전이가 세 번에 못 미치면 경고를 냄

- 확인할 것
  - [ ] WP1 → WP2 로 올라감
  - [ ] WP2 → WP3 대각선으로 내려옴
  - [ ] **WP3 에서 배가 주황으로 바뀌고 원을 돎** — ① 의 계단이 `2 Loiter` 로 올라감
  - [ ] 2바퀴 후 파랑으로 돌아가 WP4 로 향함 — ① 의 계단이 `1 WpFollow` 로 내려옴
  - [ ] **로이터 원 밖에서 출발해 다시 경로로 붙음**
  - [ ] WP5 에서 회색으로 바뀌고 멈춤 — ① 의 계단이 `3 Finish` 로 올라가고 ③ 의 지령선이 0 이 됨
...
| 3 | `3) W07_1_vrx 실행 (420초)` | 실시간 화면 네 칸 (D-3). 상태가 바뀌면 배 색이 바뀌고 ① 의 계단이 올라감 |
...
> [!tip] 주행 중 실시간 화면이 표시됨
> VRX 모델에도 오프라인과 같은 `Animate` 상자가 들어 있고 **같은 함수**(`W07_animate.m`)가 그림
> 배가 어디쯤 가고 있는지, 지금 어느 상태인지 Gazebo 화면 없이도 보임. 끄려면 `animate = 0`

![실시간 미션 화면 — VRX 연동](W07_simulink/img/W07_anim_vrx.png)

- 조건 — `W07_1_vrx`, `W03_vrx_lite/run_vrx.sh lite`, `ROS_DOMAIN_ID = 7`,
  측정 RTF 1.070 (페이싱도 같은 값), 스폰 $(x_{\text{ENU}}, y_{\text{ENU}}) = (-532.1, 200.0)$,
  420 s 주행 / 벽시계 546.8 s, 게인·미션 설정은 A절과 같음 (2026-09-25 실측)
- 캡처 명령 — `W07_anim_capture('vrx')`

| 항목 | 이 실행 (09-25) | D-1 기준 실행 (09-19) |
|---|---|---|
| 측정 RTF | 1.070 | 0.668 |
| 상태 전이 순서 | 1 → 2 → 1 → 3 | 1 → 2 → 1 → 3 |
| 로이터 진입 [s] | 59.5 | 69.4 |
| 로이터 종료 [s] | 196.1 | 236.4 |
| 회전수 | 2.00 | 2.00 |
| 임무 종료 [s] | 247.1 | 301.1 |

- **순서와 회전수는 같고 시각만 다름** — 위 「전이 시각이 실행마다 흔들리는 이유」 가 그대로 보이는 예임
  - 두 실행의 차이는 페이싱 비율임 (0.668 대 1.070). 이 컴퓨터는 이번에 RTF 가 1 을 넘었음
  - 그래서 임무 종료가 D-1 보다 54 s 이름. D-1 의 지표 수치는 **그 실행의 것으로 그대로 둠**
- 네 칸의 뜻과 읽는 법은 A절 「실시간 창 — 네 칸의 뜻」 과 **완전히 같음.** 나란히 놓고 읽으면 됨
  - ① 의 계단이 같은 순서로 세 번 바뀌는지
  - ① 제목의 바퀴 수가 로이터 구간에서 2.00 까지 오르는지
  - 복귀 후 왼쪽 칸의 초록 원이 WP4 를 다시 집는지
  - ③ 에서 종료 뒤 추력이 0 으로 내려앉고 $u$ 가 따라 내려오는지

> [!note] VRX 화면의 ③ 은 오프라인보다 추력이 잘게 흔들림
> - 로이터 구간에서 $F_L$ · $F_R$ 이 톱니처럼 진동함 — odometry 가 약 10 Hz 로 들어오는데 제어는 20 Hz 라
>   같은 측정값을 두 번 쓰는 스텝이 생기기 때문임
> - 상태 전이와 회전수는 이 진동에 영향받지 않음. 미션 모델은 그 둘로 판정함 (D-2)

### 확인할 것

- [ ] 2단계 출력의 `스폰 위치` 가 launch 파일의 스폰 좌표와 같은가
- [ ] 상태 전이가 오프라인과 같은 순서로 일어나는가 — 실시간 창 ① 의 계단으로 바로 읽힘
...
- [ ] 실시간 창 **네 칸의 뜻**을 알고, 왼쪽 칸에서 **배 색이 세 번 바뀌는 것**을 봤음
- [ ] 실시간 창 ① 의 **상태 계단이 세 번 바뀌고** 제목에 전이 시각 세 개가 찍히는 것을 봤음
- [ ] 실시간 창 ① 제목의 **바퀴 수가 로이터 구간에서 2.00 까지 오르는 것**을 봤음
- [ ] 로이터에서 빠져나온 뒤 왼쪽 칸의 초록 원이 **웨이포인트를 다시 집는 것**을 봤음
...
- [[W05_웨이포인트_유도_atan2와_LOS]] — LOS 유도, 내부루프, **실시간 화면 네 칸의 원형** (A절)
- [[W06_한점을_중심으로_도는_로이터링]] — 벡터필드, 회전수
- [[오프라인-모델이-시뮬레이터와-같아야-게인이-옮겨간다]]
- [[궤적이-겹치면-이격은-오차가-아니라-지연이다]] — D-2 · D-3 의 전이 시각 차이를 읽는 법
...
| `W07_simulink/W07_animate.m` | 실시간 화면 네 칸 그리기 — 항적(상태별 색) · 상태 타임라인 · 지령 대비 응답 · 추력. 오프라인·VRX 공용 (모델 안 `Animate` 상자가 호출) |
| `W07_simulink/W07_anim_capture.m` | 그 화면을 조건과 함께 PNG 로 재현 (`img/W07_anim_offline.png` · `W07_anim_vrx.png`) |
| `W07_simulink/W07_vrx_run.m` | VRX 연동 실행 — 토픽 확인 · RTF 측정 · 페이싱 · 결과 그림 · 오프라인 대조 |
...
- **8주차는 중간 발표 주간임** — 팀별 Term Project 제안서 초안을 준비할 것
...
> [!important] 중간 발표
> 평가의 **15%** 임. 전반부 데모(7주차까지 만든 것)와 Term Project 제안서를 냄
