...
function build_SB9_done()
    ...
    note(m, sprintf(['[I] 연속 시스템\n' ...
        '위: Transfer Fcn 1/(0.3s+1) — 1차 지연. 7주차 모터 모델(MotorLag, 이산형)의 연속 버전이다\n' ...
...
function build_SB10_done()
    ...
    % Stop Simulation — 조건이 참이 되면 시뮬레이션을 멈춘다 (8·9주차는 gate = 0 으로 멈추고 정지 시간은 고정)
    ...
    note(m, sprintf(['[J] 판단 블록\n' ...
        ...
        '  -> 9주차 ModeSwitch(MATLAB Function)가 같은 일을 코드로 한다\n' ...
