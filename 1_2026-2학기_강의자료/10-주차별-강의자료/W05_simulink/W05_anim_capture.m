function info = W05_anim_capture(which, varargin)
% W05_ANIM_CAPTURE  실시간 화면(W05_animate)을 돌려서 그대로 PNG 로 저장한다.
%
%   W05_anim_capture('offline')    img/W05_anim_offline.png
%   W05_anim_capture('vrx')        img/W05_anim_vrx.png     (VRX 기동 필요)
%
%   이름있는 인자
...
%               기본값 240 (오프라인) · 240 (VRX). 사각 경로 한 바퀴가
%               약 207 초이므로 네 웨이포인트를 모두 지난 화면이 남는다
%
%   왜 스크립트로 찍는가
...
%     gcf 로 저장하면 마지막으로 만진 창이 저장된다. W05_animate 가 자기 창
...
switch which
    case 'offline', m = 'W05_0_offline';  vrx = false;
    case 'vrx',     m = 'W05_1_vrx';      vrx = true;
    otherwise, error('W05_anim_capture:which', '모르는 이름 ''%s''. offline 또는 vrx.', which);
end

%  게인·유도 설정을 언제나 기본값에서 출발시킨다. W05_setup 은 스스로
%  오프라인 모델을 돌리므로(auto_run) 그 자동 실행만 건너뛴다
setappdata(0, 'W05_skip_run', true);
evalin('base', 'W05_setup');
...
if vrx
    ...
    %  안전 수역을 벗어난다. W05_vrx_run 이 하는 일과 같다.
...
f = W05_animate();                          % gcf 가 아니라 그리는 함수가 준 핸들
if isempty(f) || ~isvalid(f)
    error('W05_anim_capture:nofig', ...
          '실시간 화면이 열리지 않았다. animate = 1 인지 확인할 것.');
end
figure(f); drawnow;

file = fullfile(img, ['W05_anim_' which '.png']);
...
% ---- 스폰 위치와 RTF 를 한 번에 잰다 (W05_vrx_run 의 measureRTF 와 같다) ----
function [RTF, x0_n, y0_n] = measure_spawn(topic, sec)
n = ros2node(sprintf('/w05_anim_probe_%d', randi(9999)));
c = onCleanup(@() clear('n'));
...
try
    n  = ros2node(sprintf('/w05_stop_%d', randi(9999)));
    ...
    warning('W05_anim_capture:stop', '추력 0 송신 실패: %s', e.message);
end
end
