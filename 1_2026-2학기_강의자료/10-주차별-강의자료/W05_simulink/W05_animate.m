function W05_animate(x_n, y_n, psi, t)
% W05_ANIMATE  시뮬레이션이 도는 동안 배와 웨이포인트를 실시간으로 그린다.
%
%   Simulink 의 Animate 블록이 매 스텝 이 함수를 부른다.
%   직접 부를 일은 없지만, 그림이 마음에 안 들면 이 파일을 고치면 된다.
%
%   입력
%     x_n, y_n : NED 위치 [m]   (출발점 기준 상대좌표)
%     psi    : 선수각 [rad]   (북 기준, 시계방향 +)
%     t      : 시뮬레이션 시각 [s]
%
%   선체 모양은 W05_matlab/shipModel.m 과 같다.
%     선수(앞) = 세모,  선미(뒤) = 네모  ->  화살표처럼 보인다
%
%   그림이 느리면 W05_setup.m 에서
%     animate       = 0     아예 끄기
...
if newRun
    ...
    fig = figure('Name','W05 실시간 항적', 'NumberTitle','off', ...
...
% NED 회전:  N = x_n + bx*cos(psi) - by*sin(psi)
%            E = y_n + bx*sin(psi) + by*cos(psi)
hullX = x_n + bx*cos(psi) - by*sin(psi);
hullY = y_n + bx*sin(psi) + by*cos(psi);
set(hHull, 'XData', hullY, 'YData', hullX);

% 선수 방향선
hd = 2.2 * L;
set(hHead, 'XData', [y_n, y_n + hd*sin(psi)], ...
           'YData', [x_n, x_n + hd*cos(psi)]);

%% ---- 상태 표시 --------------------------------------------------------
psi_disp = atan2(sin(psi), cos(psi)) * 180/pi;    % -180 ~ 180 으로 정리
set(hInfo, 'String', { 'W05 실시간 항적   (선수 = 세모, 선미 = 네모)', ...
    sprintf('t = %6.1f s     x = %7.2f m     y = %7.2f m     psi = %6.1f deg', ...
            t, x_n, y_n, psi_disp) });
