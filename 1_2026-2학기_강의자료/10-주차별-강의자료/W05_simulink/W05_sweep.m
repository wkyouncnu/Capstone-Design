function W05_sweep(part)
% W05_SWEEP  5주차 1-7 · 1-8 절의 표를 다시 잰다 (오프라인, VRX 불필요).
%
%   >> W05_sweep          % 전부 (약 1~2 분)
%   >> W05_sweep('A')     % 1-7 표 — Delta 스윕, 조류 없음 / 0.4 m/s 동쪽
%   >> W05_sweep('B')     % 1-8 표 — 전환 판정 두 가지 x R x 조류
%   >> W05_sweep('C')     % 1-8 본문 — 조류 방향 여덟 개 (R = 2, 0.8 m/s)
%   >> W05_sweep('D')     % 1-8 R 표 — 수락반경과 모서리 컷
%
%   표의 열
%     수렴      경로 이탈 |y_e| 가 처음으로 1 m 안에 들어온 시각 (첫 전환 전)
%     평균|ye|  초기 60 초를 뺀 평균 |y_e| — W05_plot 의 S.mean_ye_ss 와 같은 값
%     평균|r|   같은 구간의 평균 |요각속도| [deg/s] — 요 모멘트를 얼마나 쓰는가
%     가로지름  같은 구간에서 y_e 의 부호가 바뀐 횟수 — 지그재그
%     전환수    웨이포인트를 넘긴 횟수 (x_e 가 한 번에 5 m 넘게 줄어든 곳)
%
%   W05_setup 의 설정을 base 에 올린 뒤 이 함수 안에서만 조건을 바꾼다.
%   끝나면 W05_setup 을 다시 실행해 기본값으로 돌아간다.

if nargin < 1, part = 'ABCD'; end
here = fileparts(mfilename('fullpath'));  cd(here);
setappdata(0, 'W05_skip_run', true);            % W05_setup 이 시뮬레이션을 돌리지 않게
evalc('evalin(''base'', ''W05_setup'')');
assignin('base','animate',0);
m = 'W05_0_offline';  load_system(m);
c = onCleanup(@() restore());
...
function restore()
% 스윕이 바꾼 조건을 W05_setup 기본값으로 되돌린다
