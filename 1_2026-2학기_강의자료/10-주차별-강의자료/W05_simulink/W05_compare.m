% W05_COMPARE  atan2 유도와 LOS 유도를, 조류 유/무로 나누어 한 번에 비교한다.
%
%   과제 5 의 ①번 표를 만드는 스크립트다.
%   >> W05_setup
%   >> W05_compare
%
%   조건을 바꾸려면 아래 CASES 배열만 고치면 된다.

setappdata(0, 'W05_skip_run', true);   % 설정 파일의 자동 실행은 건너뛴다
W05_setup;
...
for i = 1:nC
    ...
    o = sim('W05_0_offline','StopTime','700');
...
%% 궤적 겹쳐 그리기 -----------------------------------------------------
figure('Name','W05  유도법칙 비교','Position',[80 60 1000 520],'Color','w');
sty = {'-','-','--','--'};
...
figure('Name','W05  cross-track error','Position',[140 120 900 400],'Color','w');
