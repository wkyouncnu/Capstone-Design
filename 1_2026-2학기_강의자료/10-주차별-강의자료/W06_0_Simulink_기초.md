...
- **이번 주차 학습 내용**: Simulink 를 **직접 만들어 본다**. 열세 개의 빈칸 모델을 채운다
...
> - **Simulink 는 전혀 몰라도 된다.** 처음 접한다고 가정하고 쓴 문서다
...
> | **2일차** (3시간) | G\~M | **7\~9주차 모델을 읽기 위한 블록들** |
>
> 2일차는 "쓸모 있는 블록 모음"이 아니라 **본 과목의 모델을 읽기 위한 어휘**다.
> `Unit Delay` 를 모르면 9주차 대수 루프를 이해할 수 없고,
> `Transfer Fcn` 을 모르면 7주차 모터 모델(이산판 `Discrete Transfer Fcn`)을 읽을 수 없다.
> 일부 블록(`Unit Delay`·`Integrator` 등)은 7\~9주차 모델에 그대로 들어 있고, 나머지는 그 모델에서 MATLAB Function 이 대신하는 일을 블록으로 보여 준다.
> 내용은 MathWorks 공식 교재 `Simulink Fundamentals.pdf` 의 각 장에서 가져왔다.
...
- 그림의 상태 표시줄에 적힌 `ode4` 는 연속 모델의 예다. 1-4절의 이산 설정을 마친 모델에서는 이산 솔버 이름이 보인다
...
> ROS 2 연동 모델과, 이산 블록만 있는 SB 모델에 이 값을 똑같이 쓴다. 연속 블록이 있는 모델은 아래 note 참고.
...
> - 즉 **ROS 와 주고받는 모델만 이산 0.05 초**로 맞춘다. 이유는 실제 장비의 제어주기와 맞추기 위해서다
...
- **ROS 2 블록도 이번 주차 없다** — 6주차 본편(W06)에서 이 보충 자료의 내용 위에 얹는다
...
2. 왼쪽에서 **Solver** 선택
3. **Solver details ▸** 를 펼친다 — 펼쳐야 **Fixed-step size** 칸이 보인다
4. 아래처럼 맞춘다
...
- `Display` 에는 시각이 흘러가는 것이 보인다. 실행이 끝나면 `20` 이 남아 있다

### A-8. 샘플 타임 색으로 확인하기

- 툴스트립 **Debug** → **Information Overlays** → **Sample Time** → **Colors**
- `Ctrl` + `D`(모델 업데이트)를 눌러야 색이 칠해진다
...
- Simulink 는 실행 **전에** 모든 신호의 크기를 정해야 한다. 값에 따라 크기가 바뀌는 출력은 정할 수 없다
- 해결: 크기는 상수로 (`y = zeros(1,3) + b;`)

> [!note] 함수 **안에서만** 크기가 바뀌는 배열은 R2024b 에서 돈다
> `v = []; for k = 1:3, v = [v, a*k]; end` 는 오류 없이 실행된다 (가변 크기 지원, 2026-09-18 확인).
> 그래도 `v = zeros(1,3);` 로 미리 자리를 잡는 편이 빠르고, 코드 생성에서도 안전하다.

> [!tip] 지원되지 않는 MATLAB 함수가 있다
> `input`, `eval`, `figure`·`plot` 같은 것은 MATLAB Function 블록 안에서 쓸 수 없다.
...
- **위로 나오려면** 캔버스 바로 위 경로 표시줄 왼쪽의 **⇧ 버튼**을 누르거나, 경로에서 모델 이름을 클릭한다
...
- **Sinks → Scope** 를 새로 놓는다 (빈칸본에는 Scope 가 없다)
- `From` 을 하나 더 놓고 (태그도 `RESULT`) 그 `Scope` 에 잇는다
...
| 요소 | 의미 |
|---|---|
| 주황 세로 상자 | Bus Creator — 여러 신호를 한 줄로 묶음. 실제 모델에서는 **검은 세로 막대**이며, 블록 이름이 `Inner`·`Outer` |
| 버스 선 | 버스 (여러 값이 함께 흐름). 실제 모델에서는 일반 선과 같은 굵기로 보인다 |
...
> 오류 메시지가 "신호를 찾을 수 없다"고 나오면 대부분 경로가 짧은 것이다.

### D-4. 한 칸만 바꿔치기 — Bus Assignment

1. **Signal Routing → Bus Assignment** 를 캔버스에 놓고, 블록 이름을 `Asg` 로 바꾼다
2. `Outer` 의 출력(버스 선)에서 선을 하나 더 빼서 `Asg` 의 **1번 포트**에 잇는다
...
> 게인을 왜 그 값으로 정하는지는 6주차 1-7절(극배치)에서 다룬다.
...
> 포화를 붙인 뒤(E-3부터) 출력은 **아무리 해도 1 을 넘지 못한다.** E-1·E-2 에는 아직 포화가 없다.
...
- 여기서 `1.0` 에 멈추는 것은 **포화 때문이 아니다** (아직 포화 없음)
  - 폐루프 정상 이득 $K/(1+K) = 1/(1+1) = 0.5$ 때문 → $2 \times 0.5 = 1.0$, $0.5 \times 0.5 = 0.25$
...
- 이번엔 두 목표(`2`, `0.5`)를 **모두** 맞춘다
  - 아직 포화가 없기 때문 — `2` 에 못 가게 되는 것은 E-3 에서 포화를 넣은 뒤다
...
> Scope 를 끝까지(30초) 보면 30초 가까이 되어서야 `0.5` 에 닿는다.
> 이것이 **적분 와인드업**이다.
...
- 먼저 `Plant` 출력선을 우클릭 → **Log Selected Signals** 로 로깅을 켠다 (빈칸본에는 로깅이 없다. 상세는 F-2)
...
4. 왼쪽에 Run 목록이 보인다. 앞 절(F-1, F-4)에서 돌린 Run 도 함께 있으므로 **마지막 두 Run** 을 고른다
5. 그 두 Run 의 `y` 를 **모두 체크**하면 한 그래프에 겹쳐 그려진다
6. 그래프 위를 클릭하면 **커서**가 생겨 그 시각의 값을 정확히 읽을 수 있다

> [!tip] 이 도구는 6주차 본편에서 바로 사용한다
> 6주차 본편 E\~G절(PID 입문)부터 "상승시간 몇 초, 오버슈트 몇 %" 같은 표를 만들게 된다.
...
> 아래 블록 중 `Unit Delay`·`Integrator`·`Switch` 는 7\~9주차 모델에 그대로 들어 있다.
> 나머지는 7\~9주차 모델에서 MATLAB Function 이나 이산 블록이 대신하는 일을 **블록으로 보여 주는 것**이다.

## 이 블록들이 어디에 쓰이는가

| 절 | 블록 | 본 과목에서 쓰이는 곳 |
|---|---|---|
| G | `Mux` `Demux` `Selector` | 7주 운동모델은 상태 6개를 MATLAB Function(`States`)이 푼다. 같은 일을 블록으로 하는 법 |
...
| I | `Integrator` | **그대로 쓰임** — 운동방정식 적분(7\~9주 `MotionModel`). 배의 위치가 여기서 나온다 |
...
| J | `Stop Simulation` | 임무 종료를 블록으로 거는 법. 8·9주는 `gate = 0` 으로 속도 지령을 끄고 고정 정지 시간까지 돈다 |
...
> 기본 표시에서는 Mux 를 지난 선도 일반 선과 **같은 굵기**로 보인다.
> `Debug → Information Overlays → Signal Dimensions` 를 켜면 선 위에 원소 개수(`3`)가 표시된다.
> 같은 메뉴의 비스칼라 신호 표시(Nonscalar Signals)를 켜면 벡터 선이 굵게 그려진다.

### G-2. 벡터에는 연산이 통째로 걸린다

- `Gain(10)` 하나가 **세 신호 모두**에 걸린다
- `Sum`, `Saturation` 도 마찬가지
- 이것이 벡터를 쓰는 주된 이유다 — 블록 수가 줄어든다
...
- $x$ = 블록 입력, $y$ = 블록 출력, $k$ = 스텝 번호
- 이 블록 하나로 **상태를 가진 계산**을 만들 수 있다
- 누적기(적분기)를 손으로 만들면 이렇게 된다 ($T_s$ = 0.05 s)
...
- 출력은 `Sum`(`SumA`) 에서 뽑는다. `Unit Delay` 출력에서 뽑으면 한 스텝 늦은 값(10초 뒤 10.00)이 된다

> [!note] 완성본의 `[SumA_1]` 같은 태그
> 완성본에 보이는 `[SumA_1]`, `Fr_...` 같은 Goto/From 은 배치 정리 도구(`tidy_layout`)가 되먹임 선을 태그로 바꾼 것이다.
> 선 하나와 같다. 빈칸본에서는 선으로 직접 이어도 된다.
...
> 0초부터 10초까지 0.05 s 간격 샘플은 **201개**다 (t = 0 포함). $201 \times 0.05 = 10.05$.
...
- **오류로 멈추지 않는다.** 경고를 띄우고 반복 계산으로 풀려고 시도한다. $y = y + 0.05$ 처럼 해가 없는 식이면 결과는 의미가 없다
  - 9주차처럼 Stateflow 차트가 고리에 들어가면 그때는 **오류로 멈춘다** (9주차 1-5절의 메시지)
...
> [!tip] 대수 루프의 시각적 확인
> - 명령창에서 `Simulink.BlockDiagram.getAlgebraicLoops('SB8_discrete_todo')` 를 실행하면 고리를 찾아 블록을 강조해 준다
> - 메시지 안의 블록 이름(`SumA`)을 클릭해도 그 블록으로 간다
> - 편집기 메뉴(`디버그` 탭의 진단 항목)에서도 찾을 수 있으나 위치가 판마다 다르다

### H-4. 멀티레이트와 Rate Transition

- 0.5 s 신호를 0.05 s 블록에 그대로 이으면
  - 기본 설정(**단일 태스크**)에서는 **아무 말 없이** 돈다. 느린 값을 붙잡아 쓴다
  - **멀티태스크**(Model Settings → Solver → *각 이산 속도를 별도 태스크로 처리*)를 켜면 오류로 멈춘다: `... 사이에 데이터 무결성 문제가 있습니다`
  - 2026-09-18 확인. 코드를 만들어 실제 제어기에 올리면 멀티태스크가 되므로 그때 문제가 드러난다
- `Rate Transition` 블록이 **주기를 안전하게 넘겨 준다** (값을 붙잡아 둔다). 두 설정 모두에서 조용히 돈다
...
4. `Unit Delay` 를 지우고, `SumA` 출력에서 `SumA` 2번 입력으로 **선을 직접** 잇는다 → 실행해서 어떤 메시지가 나오는지 확인하고 그대로 적어 둘 것 (H-3 의 "대수 루프가 발견됨")
   - 지우기만 하면 고리가 사라져 "입력 미연결" 경고만 나온다
5. `Rate Transition` 없이 0.5 s 신호를 이어 본다 → 단일 태스크에서는 조용함. 멀티태스크를 켜고 다시 실행해 오류를 확인 (H-4)
6. 샘플타임 색 켜기
...
- 여기 $u$ 는 **전후 속도**, $v$ 는 좌우 속도, $\psi$ 는 선수각, $N$ 은 북쪽 위치, $X$ 는 전후 방향 힘
- Simulink 에서 미분방정식을 푸는 방법은 **한 가지** — $\dot{u}$ 를 만들어 `Integrator` 에 넣는다
- 7주차 `MotionModel` 안이 정확히 이 모양이다

### I-2. 1차 지연 — 모터가 즉시 돌지 않는 이유

$$
\dot{y} = \frac{x - y}{T} \quad \Longleftrightarrow \quad \frac{Y(s)}{X(s)} = \frac{1}{T s + 1}
$$

- $x$ = 입력, $y$ = 출력, $T$ = 시상수 [s]. 블록 대화상자와 코드에서는 이 $T$ 를 변수 `tau` 로 적는다

> [!warning] 같은 글자가 다른 뜻 — 이 절에서만
> | 글자 | 본 과목의 뜻 | 이 절에서 피한 이유 |
> |---|---|---|
> | $u$ | 전후 속도 (I-1) | 교과서의 "입력 $u$" 와 겹침 → 입력은 $x$ 로 씀 |
> | $\tau$ | 제어입력 (6주차 본편 1-6) | 교과서의 "시상수 $\tau$" 와 겹침 → 시상수는 $T$ 로 씀 (7주차 모터는 $\tau_n$) |
...
- 63.2 % 도달 시간은 계단이 들어간 **t = 1 s 부터** 잰다 (Scope 커서로는 1.30 s 에서 읽힘)
- Scope 창 → **Tools → Cursor Measurements** 로 커서를 켠다

> [!important] 시상수 $T$ 의 의미
> **63.2 % 에 도달하는 시간**이다. $5T$ 면 거의 다 왔다고 본다.
...
- `SB9` 는 `ode45`(최대 스텝 `MaxStep = 0.01`)로 저장돼 있다. 고정 스텝 `ode4`, 0.05 로 바꿔 보면 곡선이 살짝 달라진다
...
6. tau 를 **0.01** 로 줄이고 고정 스텝 `ode4`, `0.05` 로 돌려 보기 — **발산한다. 왜인가?**
   - 힌트: $h\lambda = 0.05 \times (-1/0.01) = -5$. RK4 의 한 스텝 증폭률은 $1 + z + z^2/2 + z^3/6 + z^4/24$ 에서 $z = -5$ 이면 **13.7** (1 보다 크면 발산). 참값은 $e^{-5} = 0.007$
   - tau = 0.05 면 $z = -1$, 증폭률 0.375 (참값 0.368) — 거의 맞게 돈다 (2026-09-18 계산)
...
- 9주차 `ModeSwitch` 는 같은 일을 **MATLAB Function** 으로 한다 (블록 구성은 다르고 원리는 같음)
...
- 8·9주차 모델은 이 블록을 쓰지 않는다
  - 임무가 끝나면(8주차: 정해진 바퀴 수 / 9주차: 마지막 웨이포인트) `gate = 0` 으로 속도 지령을 끄고, 고정 정지 시간(300 s / 700 s)까지 돈다
  - 끝나는 순간 시뮬레이션을 멈추고 싶을 때 이 블록을 쓴다
...
> 6\~9주차 모델은 아직 이 구조를 쓰지 않는다. 필요해졌을 때 쓰는 방법이다.
...
> 트리거 서브시스템 안의 블록은 샘플타임을 물려받아야 한다.
> H절처럼 `0.05` 로 두면 오류가 난다. 완성본의 누적기도 `-1` 이다.
...
- 도달 시간은 계단이 들어간 **t = 1 s 부터** 잰다 (Scope 커서로는 1.10 s, 2.00 s 에서 읽힘)

> [!note] 익숙한 `PID Controller` 가 마스크 블록이다
> `PID Controller` 는 우클릭 → **Mask → Look Under Mask** 로 안이 보인다.
> `Saturation`·`Transfer Fcn` 은 내장 블록이라 다이얼로그만 있고 안은 없다.
> 학습자가 만든 마스크도 `PID Controller` 와 똑같이 동작한다.
...
| 끝 | 30 s | 30.075 m 에서 멈춰 있다 |

| 읽는 법 | 뜻 |
|---|---|
| 23.05 s | 계산으로는 $3 + 30 / 1.5 = 23.00$ s 에 정확히 30 m 가 되어 **그 스텝에** 넘어가야 한다. 그런데 한 스텝 늦었다 |
| 왜 늦었나 | 0.075 m 를 400 번 더한 값이 30 이 아니라 **29.999999999999805** 다 (부동소수 반올림). 그래서 `>= 30` 이 23.00 s 에 거짓, 23.05 s 에 참 |
| 30.075 m | 그 한 스텝(0.05 s × 1.5 m/s) 만큼 더 갔다 |
...
> - 문턱값을 `29.99` 로 두거나, 거리 대신 "남은 거리 $\le$ 한 스텝 이동량" 처럼 **여유를 둔 조건**을 쓴다
> - 9주차의 도착 판정이 "수락반경 5 m 안" 인 것도 같은 이유다
...
> 이 모델과 같은 문법(`en:`, `[x > 0.5]`)을 쓴다.
> 다만 9주차에는 `Loiter` → `WpFollow` 로 돌아가는 전이가 있고, `after()` 타이머는 없다.
...
2. 오른쪽에 상태를 하나 그린다
   ...
   - 상태 안 글자의 **첫 줄**은 이름 `Stop`, **둘째 줄**에 `en: mode = 3; u_cmd = 0;` 을 적는다
3. `Go` 에서 `Stop` 으로 전이를 만든다
   - `Go` 테두리에서 `Stop` 테두리로 마우스를 끈다 → 화살표가 생긴다
   - 화살표 위의 `?` 를 클릭하고 `[arrived > 0.5]` 를 입력한다
4. 실행해서 위 표의 23.05 s · 30.075 m 가 나오는지 확인한다
...
- 두 스크립트는 끝에서 볼트의 `_tools` 정리 도구(`tidy_model` 등)를 부른다. 배포 폴더만 받은 경우 이 도구가 없어 **경고가 뜰 수 있다**
  - 모델 자체는 정상으로 만들어진다. 배치만 정리되지 않은 상태이며, 아래 `tidy_layout` 으로 정리한다
...
- **`Simulink Fundamentals.pdf`** (`50-원본자료/`) — MathWorks 공식 교육 교재 333쪽
  - 이 문서의 각 절은 아래 표의 장에서 내용을 가져왔다 (M절은 교재 밖)
...
- [[W06_Simulink_ROS2_연동과_첫_제어기]] — 6주차 본편. 이 보충 자료의 내용 위에 ROS 2 를 얹는다
...
- 이번 주차에 학습한 블록 위에 **ROS 2 블록 세 개**가 얹힌다
  ...
  - `Publish` — 토픽으로 발행
- 2일차 G\~M 절의 내용은 7\~9주차 모델을 읽을 때 쓰인다
