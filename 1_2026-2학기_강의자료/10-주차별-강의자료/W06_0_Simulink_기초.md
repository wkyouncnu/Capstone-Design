...
> 메뉴를 찾기 어려우면 명령창에서 `Simulink.BlockDiagram.getAlgebraicLoops('SB8_discrete_todo')` 를 실행한다. 고리를 찾아 강조해 준다.
...
4. `Unit Delay` 를 지우고, `SumA` 출력에서 `SumA` 2번 입력으로 **선을 직접** 잇는다 → 실행해서 어떤 오류가 나는지 확인. 오류 메시지를 그대로 적어 둘 것
...
| 읽는 법 | 뜻 |
|---|---|
| 23.05 s | $3 + 30 / 1.5 = 23.00$ s 에 한 스텝(0.05 s)이 더해졌다. 도착을 **본 다음 스텝**에 넘어가기 때문이다 |
