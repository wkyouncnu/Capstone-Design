...
- 그림의 상태 표시줄에 적힌 `ode4` 는 연속 모델의 예임. 1-4절의 이산 설정을 마친 모델에서는 이산 솔버 이름이 보임
...
| 버스 선 | 버스 (여러 값이 함께 흐름). 실제 모델에서는 일반 선과 같은 굵기로 보임 |
...
> 포화를 붙인 뒤(E-3부터) 출력은 **아무리 해도 1 을 넘지 못함.** E-1·E-2 에는 아직 포화가 없음
...
- 이번엔 두 목표(`2`, `0.5`)를 **모두** 맞춤
  - 아직 포화가 없기 때문 — `2` 에 못 가게 되는 것은 E-3 에서 포화를 넣은 뒤
...
- 먼저 `Plant` 출력선을 우클릭 → **Log Selected Signals** 로 로깅을 켬 (빈칸본에는 로깅이 없다. 상세는 F-2)
...
4. 왼쪽에 Run 목록이 보임. 앞 절(F-1, F-4)에서 돌린 Run 도 함께 있으므로 **마지막 두 Run** 을 고름
...
- **오류로 멈추지 않음.** 경고를 띄우고 반복 계산으로 풀려고 시도함. $y = y + 0.05$ 처럼 해가 없는 식이면 결과는 의미가 없음
...
- $x$ = 입력, $y$ = 출력, $T$ = 시상수 [s]. 블록 대화상자와 코드에서는 이 $T$ 를 변수 `tau` 로 적음
...
> 오류가 나지 않음. 값이 계속 커짐
> 그리고 그 상태로 제어기에 들어가면 **와인드업**이 됨 (E절)
