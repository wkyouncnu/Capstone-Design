...
> [!warning] 현 시점에서 어렵게 느껴지는 것이 정상임
...
> [!warning] 11주차에 역할을 1회 의무적으로 교대함
...
- [ ] 자율운항을 **인지 → 판단 → 제어** 3단계로 설명할 수 있음
- [ ] 기말 과제가 무엇인지(4구간) 말할 수 있음
- [ ] VRX가 무엇이고 왜 쓰는지 설명할 수 있음
- [ ] 내 팀과 희망 역할을 정했음
...
- [ ] **`xeyes` 실행 시 눈 모양 창이 뜸** ← 3주차 Gazebo의 전제
...
- [ ] `Tab` 자동완성을 써 봤음
- [ ] `Ctrl + Shift + C` / `V` 로 복사·붙여넣기를 해 봤음
- [ ] `mkdir` → `cd` → `touch` → `ls` → `rm` 연습을 마쳤음
- [ ] 작업은 `~/` 안에서 해야 한다는 것을 앎

### 신고 사항

- [ ] 노트북 사양이 미달인 경우 신고했음 (해당자만)
