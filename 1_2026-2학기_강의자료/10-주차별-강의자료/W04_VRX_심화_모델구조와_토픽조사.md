...
- 위 $N$ 은 몸체축 FRD($y_b$ 우현, $z_b$ 아래) 기준. 표의 FLU 좌표(좌현 $y_b=+1.027$)를 그대로 넣어 $N = x_b F_y - y_b F_x$ 로 계산하면 부호가 반대로 나옴 → 1-3 경고표
...
>
> 그래서 1-2절 Xacro 의 좌현 추진기 $y_b = +1.027$ (FLU) 은 10주차 배분 행렬에서 $y_L = -1.027$ 이 됨
...
q = \Bigl(0,\ 0,\ \sin\frac{\psi_{\text{ENU}}}{2},\ \cos\frac{\psi_{\text{ENU}}}{2}\Bigr) \quad (x,\ y,\ z,\ w \text{ 순서})
$$
