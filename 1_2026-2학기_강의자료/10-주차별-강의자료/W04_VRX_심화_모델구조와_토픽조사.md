...
> [!warning] 이 그림과 표는 **ROS `base_link` 좌표**다 — 3주차 몸체축과 두 축의 부호가 반대다
> URDF·Xacro 는 ROS 규약(REP-103)을 따라 $x$ 선수, $y$ **좌현**, $z$ **위** 를 쓴다(FLU).
> 3주차에서 제어식에 쓰기로 한 몸체축은 $x$ 선수, $y$ **우현**, $z$ **아래** 다(FRD, Fossen 과 같다).
>
...
>
> 그래서 1-2절 Xacro 의 좌현 추진기 $y = +1.027$ 은 10주차 배분 행렬에서 $y_L = -1.027$ 이 된다.
> 3주차의 ENU → NED 변환(북·동을 바꾸고 아래를 뒤집는다)과 **같은 이유의 다른 판**이다 —
> ENU·NED 는 땅에 붙은 좌표계, FLU·FRD 는 배에 붙은 좌표계다.
