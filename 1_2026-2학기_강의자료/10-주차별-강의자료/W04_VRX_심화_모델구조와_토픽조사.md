...
- 위 $N$ 은 몸체축 FRD($y$ 우현, $z$ 아래) 기준. 표의 FLU 좌표(좌현 $y=+1.027$)를 그대로 넣어 $N = x F_y - y F_x$ 로 계산하면 부호가 반대로 나옴 → 1-3 경고표
...
>
> 그래서 1-2절 Xacro 의 좌현 추진기 $y = +1.027$ 은 10주차 배분 행렬에서 $y_L = -1.027$ 이 됨
