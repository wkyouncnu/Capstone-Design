...
% BUILD_W06_MODELS  6주차 Simulink 모델 2개를 생성한다.
%
%     W06_0_offline.slx   오프라인 WAM-V 로이터링 — Gazebo 없이 돈다
%     W06_1_vrx.slx       VRX 연동 — 운동모델만 Gazebo 로 바뀐다
%
%   5주차와 GNC 골격이 똑같다. 바뀐 것은 1단(유도)뿐이다.
%
...
%   실행 전에 반드시 >> W06_setup 을 먼저 실행할 것.
    ...
    for mm = {'W06_0_offline','W06_1_vrx'}
...
% 로깅 — 포트 없는 서브시스템 하나로 묶는다. W06_plot.m 이 이 변수들을 읽는다.
...
function build_offline()
    ...
    note(m,'n1', ['W06 실습 B  —  오프라인 WAM-V 로이터링 (Gazebo 불필요)' newline ...
        'LoiterVF -> TurnCount -> Inner Loop -> Thrusters -> 운동모델' newline ...
        '유도만 5주차와 다르다. 나머지는 그대로다.' newline ...
        '실행 전 >> W06_setup'], 250, -100);

    save_system(m); close_system(m);
    fprintf('  W06_0_offline  생성\n');
end
...
'chi  = atan2(sin(c), cos(c));'];
    ...
    note(m,'n1', ['W06 실습 D  —  VRX 연동 로이터링' newline ...
        'Guidance -> Inner Loop -> Thrusters -> Gazebo -> Navigation' newline ...
        '유도부·내부루프·게인은 W06_0_offline 과 완전히 같다.' newline ...
        '운동모델만 Gazebo 로 바뀐다.' newline ...
        '실행 전: (1) W06_setup  (2) VRX 기동  (3) 페이싱을 RTF 에 맞출 것'], 250, -100);

    save_system(m); close_system(m);
    fprintf('  W06_1_vrx      생성\n');
end
