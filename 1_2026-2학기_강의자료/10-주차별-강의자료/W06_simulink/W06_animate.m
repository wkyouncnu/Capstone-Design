...
%  되돌아보기 한계. VRX 는 StopTime inf 라 무한정 쌓일 수 있으므로 한계가 필요하다.
%  값을 20만으로 잡은 이유 — **이 함수는 한 스텝에 한 번 불리지 않는다.**
%  오프라인 모델은 ode4 (고정 스텝 0.05 s) 라 한 메이저 스텝마다 마이너 스텝이
%  네 번 돌아 초당 80 번 불린다. C-2 시나리오는 600 s 를 도므로 48,000 개가 쌓인다.
%  한계를 4만으로 두면 **앞 100 초가 잘려 나가** 접근 구간과 방향 반전이 화면에서
%  사라진다 (2026-09-25 실제로 밟았다. 왼쪽 항적이 (0,0) 에서 시작하지 않았다).
%  20만이면 오프라인 2,500 s · VRX(이산 0.05 s, 초당 20 회) 10,000 s 까지 남는다.
NMAX = 200000;
