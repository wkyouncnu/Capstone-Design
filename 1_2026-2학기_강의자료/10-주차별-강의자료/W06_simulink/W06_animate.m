function W06_animate(x_n, y_n, psi, turns, t)
% W06_ANIMATE  로이터링을 실시간으로 그린다.
%
%   목표 원, 중심점, 배, 항적, 누적 회전수를 함께 보여 준다.
%   선체 모양은 5주차와 같다 — 선수는 세모, 선미는 네모.
%
%   느리면 W06_setup.m 에서
%     animate       = 0     끄기
%     animate_every = 1.0   덜 자주 그리기

persistent fig ax hTrail hHull hHead hInfo trailX trailY tLast tPrev hCircle
...
if newRun
    ...
    fig = figure('Name','W06 로이터링', 'NumberTitle','off', ...
...
ce = evalin('base','center_east');
rd = evalin('base','r_d');
rr = hypot(x_n - cn, y_n - ce);

psi_disp = atan2(sin(psi), cos(psi)) * 180/pi;
set(hInfo, 'String', { 'W06 로이터링   (선수 = 세모, 선미 = 네모)', ...
