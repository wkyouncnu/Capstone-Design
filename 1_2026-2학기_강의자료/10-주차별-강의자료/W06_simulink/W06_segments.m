function T = W06_segments(crab)
% W06_SEGMENTS  6주차 C-2 — 도는 도중에 방향·속도·반경을 바꾸는 시나리오를 구간별로 잰다.
%
%   >> T = W06_segments        % 크랩 보상 켬 (C-2 표와 같은 조건)
%   >> T = W06_segments(0)     % 크랩 보상 끔
%
%   W06_setup 을 불러 기본값을 올린 뒤 이 함수 안에서만
%     use_schedule = 1, required_turns = 0, crab_comp = crab, 600 s
%   로 바꿔 W06_0_offline 을 한 번 돌린다 (VRX 불필요, 몇 초).
%
%   구간마다 **앞 30 초는 뺀다** — 방향·속도·반경이 막 바뀐 과도 구간이라서.
%     평균 반경  중심까지 거리 r 의 평균 [m]
%     평균 u     전후 속도 평균 [m/s]
%     방향       중심에서 본 방위 theta 가 늘면 시계방향 (NED: 북에서 시계 +)
%   끝나면 W06_setup 을 다시 불러 기본값으로 돌아간다.

if nargin < 1, crab = 1; end
here = fileparts(mfilename('fullpath'));  cd(here);
setappdata(0, 'W06_skip_run', true);
evalc('evalin(''base'', ''W06_setup'')');
...
evalc('out = sim(''W06_0_offline'', ''StopTime'', ''600'');');
