...
> [!note] 이 절은 4\~8주차에서 만든 모델을 에이전트로 다루는 단계
> 제어기 설계는 4\~8주차에서 이미 끝냈고, 이번 주차에는 그 모델을 **읽고 고치는 방법**을 익힘
...
- 수치는 4주차 2-9 의 WAM-V 값 — $T\dot r + r = KN$, $T = 0.816$ s, $K = 1.25\times10^{-3}$ rad/(s·N·m)
...
> [!important] 4\~8주차에서 만든 모델보다 훨씬 큰 모델을 읽는 연습임
...
- 4\~8주차 모델의 게인을 다시 손볼 때, Term Project 모델을 읽을 때 계속 쓰게 됨
