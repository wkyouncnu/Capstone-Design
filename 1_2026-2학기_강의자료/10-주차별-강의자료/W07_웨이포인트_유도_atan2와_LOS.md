...
> [!tip] `atan2` 를 쓰는 이유
> `atan(ΔE/ΔN)` 은 −90°\~+90° 만 나옴. 뒤쪽으로 가야 할 때를 표현하지 못함
...
- $\hat{n}$ 은 $\hat{t}$ 를 **시계방향(북→동)으로** $90^\circ$ 돌린 것임. N-E 좌표계에서 그 방향이 우현임
...
| $N, E$ | 배의 **북쪽 · 동쪽 위치** (NED). 요 모멘트 $N$ 과 같은 글자임 — 3주차 6자유도 절 | 둘 다 |
| $\pi_p$ | 경로 방위각 | 둘 다 |
...
> NED 에서 진행 방향을 바라볼 때 오른쪽이 양수임. 6주차의 요모멘트 부호와 같은 규약임
...
> $\psi_{\text{ref}} = \pi_p - K y_e$ 처럼 비례로만 만들면 이 보장이 없음. $y_e$ 가 크면
> 한 바퀴를 넘어가는 지령이 나옴
...
> - `W07_lookahead_demo` 는 안에서 `W07_setup` 을 다시 부르며 워크스페이스를 지움
> - 명령 창에서 `current_speed = 0.4` 만 치면 무시됨. **`W07_setup.m` 파일에서** `current_speed = 0.4`, `current_direction = 90` 으로 고친 뒤 다시 돌릴 것

### 실제로 재 보면

- 본 과목의 오프라인 모델(`W07_0_offline`)로 $\Delta$ 만 바꿔 가며 미션 전체를 돌린 값임
...
> 8주차 1-8절이 로이터링에서 이것을 해서 반경 오차를 42 % 줄임. $\beta$ 를 모를 때 적분으로 지우는
...
> 넘어가기 때문임. MSS 툴박스의 원본 `GNC/LOSchi.m` · `ILOSpsi.m` 도 이 판정($d - x_e < R_{\text{switch}}$)을 씀
> 배포 폴더 `W07_matlab/LOSchi.m` 은 **연구실 수정본**이라 이 부분이 수락 원 판정으로 바뀌어 있음 (원본 판정은 주석으로 남아 있음)
>
...
- "모서리 컷" 은 배가 웨이포인트에 가장 가까이 간 거리임. $R$ 이 크면 **미리 돌아** 모서리를 잘라먹고, 그만큼 일찍 도착함
...
> - 배포 폴더 `W07_matlab/LOSchi.m` 은 연구실 수정본임 — `pi_p` · `Delta` · `R_switch` 를 쓰고, 전환 판정이 수락 원임 (§1-8)
---
...
> `animate = 1` 이라서 그러함. 느리면 `W07_setup` 안에서 `animate = 0` 으로 끔
...
> 배는 `(0, 0)` 에서 출발하는데 첫 구간은 `East = 20 m` 선임. **20 m 벗어난 채로 시작**함
...
> [!note] 7\~9주차의 모든 모델이 같은 색 규칙을 따름
...
> - 6주차 H절의 `HeadingCtrl` 과 **같은 식**임. 여기서는 지령이 계속 바뀜
...
> 6주차처럼 250 N 으로 제한하면 직진에 다 써 버려 **선회할 여유가 없음**
>
...
2. **조류를 켜면** 두 법칙 모두 나빠지고 **벌어지는 거리가 커짐** — 정착 후 차이가 1.06 m 에서 2.69 m 로. 비율은 2.5 배 그대로임
3. **크랩각이 3° → 9.4°** 로 커짐 — 뱃머리와 진행 방향이 10도 가까이 어긋남

> [!note] 비율만 보면 놓침
> atan2 는 1.76 m → 4.41 m 로 **2.7 m** 나빠졌고, LOS 는 0.70 m → 1.72 m 로 **1.0 m** 나빠졌음
> 조류가 같은 만큼 밀었는데 atan2 가 세 배 가까이 더 밀렸음. 비율이 같게 나오는 것은
...
- 그다음 `W07_setup.m` 에서 `Delta` 만 바꾸며 미션 전체를 실행함. 나머지는 손대지 않음

```matlab
Delta = 2;      % 2 · 3 · 5 · 10 · 20 · 30 으로 여섯 번
```

- 각 조건에서 `W07_plot` 이 찍는 **정착 후 평균 `|y_e|`** 와 **완주 시각**을 적음
...
- 스크립트가 잰 RTF(0.697)는 Gazebo 가 표시하는 RTF(0.99)보다 낮음. MATLAB 쪽에서 메시지 도착 간격으로 재기 때문. **이 값을 그대로 페이싱에 씀** — 8주차 D-2 경고 참고
...
> [!important] 312 m 를 달린 뒤 두 궤적의 차이가 평균 0.28 m 임
> - 경로 길이의 **0.1 %**. 게인을 하나도 바꾸지 않았음
...
- `W07_vrx_run` 기본값(230 s, 임무 완주)으로 실행 — 오프라인 대조까지 자동으로 함
...
| 파일 | 내용 |
|---|---|
| `W07_simulink/W07_setup.m` | **학생이 고치는 설정 파일. 실행하면 바로 시뮬레이션이 돎** |
