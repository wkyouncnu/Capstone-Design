...
1) VRX 토픽 확인
   OK — 토픽 37개
2) RTF 측정 (20초)
   RTF = 0.867  ->  페이싱 비율을 이 값으로 둔다
3) W07_1_vrx 실행 (150초)

===== VRX / LOS 유도 =====
  평균 |y_e|              2.296 m   (전 구간)
  평균 |y_e|              0.736 m   (초기 60초 제외)  <- 과제에 쓸 값
  최대 |y_e|             20.285 m
  완주                     미완주
  평균 |크랩각|            3.94 deg
...
- 이어서 `그림 저장: img/...` 줄과 `5) 같은 초기 선수각으로 오프라인 모델 실행` 이후의 대조 결과(E-6)가 출력됨
- 맨 앞 `0) ROS_DOMAIN_ID = …` 줄은 현재 스크립트가 항상 찍음. 값은 환경마다 다름

| 읽는 법 | 두 환경의 공통점 |
|---|---|
| 완주 = 미완주 | **정상.** 150 s 는 미션 전체(약 206 s)보다 짧아 마지막 웨이포인트에 닿기 전에 끝남 |
...
> 그대로 좌표 변환에 넣으면 위치가 스폰 원점만큼($\sqrt{532^2 + 162^2} \approx 556$ m) 튄다.
...
> [!important] 222 m 를 달린 뒤 두 궤적의 차이가 평균 1.1 m 다 (2026-09-04 기준 환경 표)
