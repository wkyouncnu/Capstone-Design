...
| $\Delta$ [m] | 수렴 [s] | 평균 $\lvert y_e\rvert$ [m] | 평균 $\lvert r \rvert$ [°/s] | 경로 가로지름 [회] | 완주 [s] |
|---|---|---|---|---|---|
| 2 | 18.8 | 1.988 | 11.311 | 22 | 229.2 |
| 3 | 19.3 | 1.460 | 7.233 | 19 | 213.8 |
| **5** | 20.7 | **1.307** | 3.557 | 11 | 207.2 |
| 10 | 26.5 | 1.832 | 1.905 | 7 | 203.7 |
| 20 | 미수렴 | 2.907 | 1.476 | 7 | 204.0 |
| 30 | 미수렴 | 3.724 | 1.369 | 5 | 206.5 |

> [!important] 조류가 있으면 최적 $\Delta$ 가 작아진다
> 조류가 없을 때는 $\Delta = 10$ m 가 최저점이었는데, 조류가 생기자 $\Delta = 5$ m 로 옮겨 갔다.
> 그리고 큰 $\Delta$ 가 훨씬 심하게 벌을 받는다 — $\Delta = 30$ m 는 $3.724$ m 로
> 최저점의 **약 3 배**다. 조류 속에서 큰 $\Delta$ 는 단지 늦게 붙는 정도가 아니라
> **경로에 붙지 못한 채 끝난다.** 이유는 8주차에서 크랩각과 함께 다룬다.
...
- 같은 모델, 같은 미션에서 두 판정을 바꿔 가며 돌린 값이다

| $R$ [m] | 판정 | 조류 [m/s] | 평균 $\lvert y_e\rvert$ [m] | 완주 [s] | 전환 횟수 |
|---|---|---|---|---|---|
| 5 | 경로 방향 | 0.0 | 0.700 | 206.5 | 3 |
| 5 | 수락 원 | 0.0 | 0.700 | 206.5 | 3 |
| 5 | 경로 방향 | 0.4 | 1.832 | 203.7 | 3 |
| 5 | 수락 원 | 0.4 | 1.850 | 204.6 | 3 |
| 2 | 경로 방향 | 0.4 | 2.021 | 211.2 | 3 |
| **2** | **수락 원** | **0.4** | 2.585 | **미완주** | **1** |
| 2 | 경로 방향 | 0.8 | 3.137 | 207.0 | 3 |
| **2** | **수락 원** | **0.8** | 4.810 | **미완주** | **1** |

- 조류가 없으면 두 판정이 **완전히 같은 결과**를 낸다. 예상대로다
- $R$ 이 넉넉하면($5$ m) 조류가 있어도 둘 다 통과한다
- $R$ 을 $2$ m 로 줄이고 조류를 주면 **수락 원 판정은 전환을 한 번밖에 하지 못하고 미션이 끝나지 않는다**
...
- 그다음 `current_speed = 0.4` 로 두고 다시 여섯 번 돌린다. **최적 Δ 가 어디로 옮겨 가는지** 본다
