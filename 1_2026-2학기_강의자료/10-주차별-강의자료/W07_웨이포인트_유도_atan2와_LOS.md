...
| MATLAB | R2024b + Simulink + ROS Toolbox |
| 배포 파일 | `W07_simulink/` (모델 2개 + 스크립트 9개: setup · plot · animate · compare · lookahead_demo · crab_demo · vrx_run · build · tidy_layout), `W07_matlab/` (연구실 참고 코드 — 목록은 문서 끝 "배포 파일") |
...
- $\hat{n}$ 은 $\hat{t}$ 를 **시계방향(북→동)으로** $90^\circ$ 돌린 것이다. N-E 좌표계에서 그 방향이 우현이다
...
| $\Delta \to 0$ | $\psi_{\text{ref}} \to \pi_p \mp 90^\circ$ ($y_e > 0$ 이면 $-$, $y_e < 0$ 이면 $+$) | 아무리 작은 이탈에도 최대로 꺾는다 — 켜짐/꺼짐 제어가 된다 |
...
> 배에서 $\Delta$ 를 재면 보정각이 $\arcsin(y_e/\Delta)$ 가 된다. $y_e \to 0$, $\Delta \to \infty$ 극한은 그대로지만,
> $\lvert y_e\rvert > \Delta$ 이면 조준점이 존재하지 않아 **법칙 자체가 정의되지 않는다.**
...
>
> 조류를 켜면 이 균형점이 **짧은 쪽으로 옮겨 간다.** 아래 두 번째 표(미션 전체 기준)가 그것을 숫자로 보여 준다.
> - `W07_lookahead_demo` 는 안에서 `W07_setup` 을 다시 부르며 워크스페이스를 지운다
> - 명령 창에서 `current_speed = 0.4` 만 치면 무시됨. **`W07_setup.m` 파일에서** `current_speed = 0.4`, `current_direction = 90` 으로 고친 뒤 다시 돌릴 것
...
- 평균 이탈량은 `W07_plot` 이 찍는 **초기 60초 제외** 값 (`S.mean_ye_ss`). 전 구간 평균이 아님
...
- **평균 요각속도**와 **경로 가로지름 횟수**는 $\Delta$ 와 함께 계속 줄어든다. $\Delta = 2$ m 는 $\Delta = 30$ m 보다 평균 요각속도가 **7.4 배** 크고(그만큼 요 모멘트를 더 씀) 경로를 **5.5 배** 더 많이 가로지른다 — 이것이 "지그재그" 의 정체다
...
> 그리고 큰 $\Delta$ 일수록 오차가 훨씬 크게 늘어난다 — $\Delta = 30$ m 는 $3.245$ m 로 최저점의 **약 2.7 배**다.
...
| $\Delta\tan\beta = -2.575$ m | 식이 예측한 값. 측정과 **약 2 %** 차이로 맞는다 |
...
|---|---|---|---|
| 작다 (2\~3 m) | O | X — 지그재그 | △ — 정상 오프셋 $\Delta\tan\beta$ 는 작지만 지그재그로 평균 이탈은 큼 ($\Delta = 2$ m 에서 $1.902$ m) |
| 적당 (5\~10 m) | 보통 | 보통 | 보통 |
...
- $R$ 을 $2$ m 로 줄이고 조류를 주면 **수락 원 판정은 첫 구간 끝의 WP2 (60, 20) 에서 전환하지 못하고 미션이 끝나지 않는다**
...
> **원 안에 들어가는 순간이 영영 오지 않는다.**
>
> 그 뒤 배가 어떻게 되는지는 유도법칙이 정한다.
> - LOS 지령 $\pi_p - \arctan(y_e/\Delta)$ 에는 $x_e$ 가 없음 → 웨이포인트를 지나쳐도 되돌아가지 않음
> - 배는 **구간의 연장선을 따라 계속 북쪽으로** 감 (VRX 라면 N = 60 m 너머는 육지)
...
> 넘어가기 때문이다. MSS 툴박스의 원본 `GNC/LOSchi.m` · `ILOSpsi.m` 도 이 판정($d - x_e < R_{\text{switch}}$)을 쓴다.
> 배포 폴더 `W07_matlab/LOSchi.m` 은 **연구실 수정본**이라 이 부분이 수락 원 판정으로 바뀌어 있다 (원본 판정은 주석으로 남아 있음).
>
...
- 본 과목의 처리: `gate` 출력
  - 마지막 웨이포인트에서 전환 조건이 만족되면 `gate = 0` → 속도 지령 0
- **유도법칙이 아니라 미션의 몫임**
  - 제자리 유지 · 주위 선회 · 추진기 정지 중 무엇을 할지는 임무가 정함
  - 9주차에서 Stateflow 가 이 판단을 맡음
...
> [!note] MSS 툴박스와 변수 이름이 조금 다르다
> MSS 원본 `GNC/LOSchi.m` · `ILOSpsi.m` · `crosstrackWpt.m` 과 맞춰 읽을 때 아래 표로 바꿔 읽는다.
>
> | 뜻 | MSS 원본 | 본 과목 (`W07_setup` · `GuidanceLaw`) |
> |---|---|---|
> | 경로 방위각 | `pi_h` | `pi_p` |
...
>
> - 식의 형태(회전 두 줄, $\pi - \arctan(y_e/\Delta)$)는 MSS 와 같다
> - 배포 폴더 `W07_matlab/LOSchi.m` 은 연구실 수정본이다 — `pi_p` · `Delta` · `R_switch` 를 쓰고, 전환 판정이 수락 원이다 (§1-8)
---
...
- `W07_setup` 이 설정 후 오프라인 모델을 **바로 실행하고 그림까지 그림** (`auto_run = 1`). `sim` 을 따로 부를 필요 없음
- 정상 출력
...
> [!warning] $U = \sqrt{u^2+v^2}$ 가 아니라 **$u$** 를 되먹임한다
> - $u$, $v$ 모두 대지 기준 속도의 선체 성분이다 (1-2절). $U$ 는 그 크기(대지속력)라 옆으로 밀리는 성분 $v$ 까지 포함한다
...
- $b$ = 후방 두 추진기의 좌우 **반폭** (`W07_setup` 의 `half_beam`)
...
> |---|---|---|
> | $k$ | 웨이포인트 번호 | 프로펠러 추력 계수 → 혼동을 피해 `k_prop` 으로 씀 (`W07_setup` 과 같은 이름) |
...
> 1.5 m/s 를 유지하는 데만 $X = (100 + 150 \times 1.5) \times 1.5 = 488$ N, **편당 244 N** 이 든다.
...
- 6주차에 Gazebo 로 실측한 값은 **1.332 m/s** 였음 (6주차 J절)

> [!note] 조류는 오프라인 모델에만 존재한다
> **VRX 2.4.0-2 에는 해류 플러그인이 없다.** 레포 전체를 검색해도 나오지 않고,
...
> - $u$, $v$ 는 대지 기준 속도의 선체 성분이므로, 물에 대한 상대속도 $u_r$, $v_r$ 을 따로 만들어 항력에 넣는다
...
- 실행하면 배가 **WP2 (60, 20) 옆을 스쳐 지나간 뒤 전환하지 못하고, 첫 구간의 연장선을 따라 계속 북쪽으로 간다.** 미션이 끝나지 않는다
...
> - 8주차 로이터링 실습도 같음 — 이어 돌리면 원에 도달하지 못함
...
- 기본 230 s — 임무(약 207 s)를 끝까지 봄. `W07_vrx_run(150)` 이면 완주 전에 끊김. 대조를 건너뛰려면 `W07_vrx_run(230, false)`

> [!caution] 스폰 위치는 설치마다 다르다 — 스크립트가 직접 잰다
> - `competition.launch.py` 의 `Model('wamv', ...)` 스폰 좌표가 같은 VRX 2.4.0-2 인데도 컴퓨터마다 달랐음: ENU $y$ = **162** 와 **200**
> - `W07_setup.m` 의 `origin_north = 162` 는 기본값. `W07_vrx_run` 이 ground truth 첫 샘플로 덮어씀 → 2단계 출력의 `스폰 위치 (N, E) = ...`
> - 고정값이 틀리면 경로 전체가 그 차이만큼 밀린 채 달림 (9주차 D절에서 38 m 밀려 육지에 걸린 실측이 있음)
...
set_param('W07_1_vrx','EnablePacing','on','PacingRate','0.70','StopTime','230');
out = sim('W07_1_vrx');
...
- 기준 환경 실측값 (2026-09-18, 데스크톱, gz RTF 0.99, 스폰 ENU $(-532, 200)$)
...
1) VRX 토픽 확인
   OK — 토픽 39개
2) RTF 측정 (20초)
   스폰 위치 (N, E) = (200.0, -532.0) m — 이 점을 원점으로 쓴다
   RTF = 0.697  ->  페이싱 비율을 이 값으로 둔다
3) W07_1_vrx 실행 (230초)
4) 결과 그림

===== VRX / LOS 유도 =====
  평균 |y_e|              1.913 m   (전 구간)
  평균 |y_e|              0.785 m   (초기 60초 제외)  <- 과제에 쓸 값
  최대 |y_e|             20.045 m
  완주 시각               207.0 s
  평균 |크랩각|            4.01 deg
  정상상태 속도           1.501 m/s  (목표 1.50)
   그림 저장: img/W07_1_vrx_result.png
5) 같은 초기 선수각으로 오프라인 모델 실행
   VRX 초기 선수각 33.06 deg
```

- 이어서 오프라인 지표 6줄과 대조 결과(E-6)가 출력됨
- 스크립트가 잰 RTF(0.697)는 Gazebo 가 표시하는 RTF(0.99)보다 낮다. MATLAB 쪽에서 메시지 도착 간격으로 재기 때문. **이 값을 그대로 페이싱에 쓴다** — 8주차 D-2 경고 참고

| 읽는 법 | |
|---|---|
| 완주 시각 207.0 s | 오프라인 206.7 s 와 0.3 s 차이 |
| 정착 후 $\lvert y_e \rvert$ 0.785 m | 오프라인 0.702 m. 유도 성능은 시뮬레이터가 바뀌어도 같음 |
...
> 같은 2.4.0-2 + ground truth 에서 **37개** 와 **39개** 를 모두 봤다. 개수 자체보다 **필요한 토픽이 있는지**가 중요하다.
...
> 그대로 좌표 변환에 넣으면 위치가 스폰 원점만큼(약 570 m) 튄다.
> `PoseSubscriber` 서브시스템 안의 `Nav` 함수 첫머리에 `if ex == 0 && ey == 0` 가드가 들어 있는 이유다.
...
- 기준 환경 실측값 (2026-09-18, 230 s, 같은 초기 선수각 33.06°)

| 항목 | 오프라인 | VRX | 차이 |
|---|---|---|---|
| 주행거리 [m] | 311.9 | 312.9 | +1.0 |
| 완주 시각 [s] | 206.7 | 207.0 | +0.3 |
| 60초 이후 평균 $\lvert y_e \rvert$ [m] | 0.702 | 0.785 | +0.083 |
| **정상상태 `u`** [m/s] | **1.500** | **1.501** | **+0.001** |
| **궤적 평균 이격** [m] | | | **0.28** |
| **궤적 최대 이격** [m] | | | **0.64** |

> [!important] 312 m 를 달린 뒤 두 궤적의 차이가 평균 0.28 m 다
> - 경로 길이의 **0.1 %**. 게인을 하나도 바꾸지 않았다
> - 오프라인에서 맞춘 것이 실제 시뮬레이터에서도 통한다는 뜻
> - **이것이 이번 주차 실습의 결론이다.** → [[오프라인-모델이-시뮬레이터와-같아야-게인이-옮겨간다]]

> [!warning] 이격이 몇 m 로 커지면 먼저 두 가지를 의심한다
...
> - 판정 기준은 이격이 아니라 **정착 후 $\lvert y_e \rvert$ 와 정상상태 속도**다 → [[궤적이-겹치면-이격은-오차가-아니라-지연이다]]
...
- [ ] **선수각 ψ, 침로각 χ, 크랩각 β** 의 관계 $\chi = \psi + \beta$ 를 설명할 수 있다
- [ ] **두 오차** `x_e` 와 `y_e` 가 각각 무엇을 재고 어디에 쓰이는지 안다
- [ ] `y_e` 가 **경로 좌표계**의 값이지 동쪽 방향 오차가 아님을 안다
- [ ] LOS 법칙 $\psi_{\text{ref}} = \pi_p - \arctan(y_e/\Delta)$ 의 각 항이 무엇인지 안다
...
- [ ] Δ 를 2 / 3 / 5 / 10 / 20 / 30 으로 바꿔 궤적 변화를 관찰했다
- [ ] VRX 에서 **RTF 를 측정**했다
...
- `W07_vrx_run` 기본값(230 s, 임무 완주)으로 실행 — 오프라인 대조까지 자동으로 한다
...
- **MSS 툴박스** — https://github.com/cybergalactic/MSS
  ...
  - `GNC/LOSchi.m` — 침로각 $\chi$ 지령을 내는 LOS. 전환 판정은 경로 방향($d - x_e < R_{\text{switch}}$)
...
| `W07_simulink/W07_plot.m` | 성능 그림 5장 + 추진기 그림 2장 + 성능 지표 |
...
| `W07_simulink/W07_vrx_run.m` | **VRX 연동 실행** — 토픽 확인 · RTF 측정 · 페이싱 · 오프라인 대조 (실습 E-4) |
| `W07_simulink/build_w07_models.m` | 모델 재생성 |
| `W07_simulink/tidy_layout.m` | 블록 배치 정리 |
| `W07_matlab/` | 연구실 참고 코드 (읽기용). `LOSchi.m` 은 MSS 원본의 **연구실 수정본** — 전환 판정이 수락 원. MSS 원본은 https://github.com/cybergalactic/MSS 의 `GNC/` |
