...
| MATLAB | R2024b + Simulink + ROS Toolbox |
| 배포 파일 | `W07_simulink/` (모델 2개 + 스크립트 10개: setup · plot · animate · compare · sweep · lookahead_demo · crab_demo · vrx_run · build · tidy_layout), `W07_matlab/` (연구실 참고 코드 — 목록은 문서 끝 "배포 파일") |
...
- 아래 두 표는 `W07_sweep('A')` 로 그대로 다시 나옴 (2026-09-19 재실행, 모든 칸 같음)
...
- 재현: `W07_sweep('B')` (이 표), `W07_sweep('C')` (아래 여덟 방향)
...
- "모서리 컷" 은 배가 모서리 웨이포인트(WP2 · WP3 · WP4)에 가장 가까이 간 거리이고, 표는 세 모서리 중 **가장 크게 자른 값**임. 재현: `W07_sweep('D')`
...
| `W07_simulink/W07_sweep.m` | 1-7 · 1-8 절 표 네 개를 다시 재는 스크립트 (오프라인, 1\~2 분) |
