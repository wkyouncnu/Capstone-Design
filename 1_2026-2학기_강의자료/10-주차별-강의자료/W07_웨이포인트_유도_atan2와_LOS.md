...
> - `W07_setup.m` 의 `origin_north = 162` 는 기본값. `W07_vrx_run` 이 ground truth 첫 샘플로 덮어씀 → 2단계 출력의 `스폰 위치 (x, y) = ...`
...
- 스크립트가 잰 RTF(0.681)는 Gazebo 가 표시하는 RTF(09-18 데스크톱에서 0.99)보다 낮음. MATLAB 쪽에서 메시지 도착 간격으로 재기 때문. **이 값을 그대로 페이싱에 씀** — 8주차 D-2 경고 참고
