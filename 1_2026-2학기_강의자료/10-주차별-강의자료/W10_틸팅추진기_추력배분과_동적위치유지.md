...
|---|---|
| 환경 | 1\~9주차 그대로 |
| 배포 파일 | `W10_simulink/` (모델 2개 + 스크립트 8개: setup · plot · animate · alloc_demo · sway_demo · vrx_run · build · tidy_layout) |
...
> ($L_{CG}$ = 무게중심에서 추진기까지의 전후 거리, 본 과목의 $|x| = 2.3738$ m 에 해당)
...
- $x = -2.3738$ m 는 두 추진기에 공통인 전후 위치, $y_L$, $y_R$ 는 1-2절 표의 좌우 위치
- 셋째 행은 1-2절의 $N$ 식을 성분별로 풀어 쓴 것 — $N = x F_{yi} - y_i F_{xi}$ 의 합
- WAM-V 수치를 넣으면
...
> 여기서 $b = y_R = -y_L = 1.0271$ m 는 반폭, $F_L = F_{xL}$, $F_R = F_{xR}$ 이다.
> 4주차 1-2절의 차동추진 식과 정확히 같다. `W10_setup` 이 이것을 `assert` 로 확인한다.
...
- $\boldsymbol{\lambda}$ 를 첫 식에 되돌려 넣으면 **유사역행렬(pseudo-inverse)** 해가 나온다
...
- 추진기마다 비용을 다르게 주고 싶으면 가중형을 쓴다
  - 비용을 $\mathbf{f}^{\top}\mathbf{W}\mathbf{f}$ 로 바꾼 것. $\mathbf{W}$ 는 4×4 양의 대각 가중행렬 — 값이 클수록 그 성분을 덜 쓴다
...
| 2 | **경계 클램프** — 가까운 섹터 끝으로 밀고 크기를 $\cos$ 로 줄인다 | 쓸 수 있는 성분만 남긴다 (기본값, `W10_setup.m` 60행) |

- 모드 2 는 요구 방향이 옆쪽 구간의 **앞쪽 절반인지 뒤쪽 절반인지**에 따라 다른 섹터 끝을 쓴다
...
- 예 — 요구 $120^\circ$: 방위각 $-45^\circ$ 로 역추진하면 힘은 $135^\circ$ 방향, 크기는 $T\cos 15^\circ$
  - 두 경우 모두 실현 힘은 **요구 힘을 가까운 섹터 끝 방향으로 투영한 것**이다
...
| 틸팅 ±45° (10주차), $N$ 자유 | $-1000 \sim +1000$ | $-707 \sim +707$ (면적) |
| 틸팅 ±45° (10주차), $X = N = 0$ | 0 | $-397 \sim +397$ |

- 고정 추진기의 제어집합은 **선분**, 틸팅은 **면적**이다
- 그림은 $(X, Y)$ 평면에 **투영한 것**이다. 요모멘트 $N$ 은 따지지 않는다
  - $Y = 707$ N 은 두 기가 모두 옆으로 $45^\circ$ 를 향할 때 — $2F_{\max}\sin 45^\circ = 707$ N
  - 그때 $N$ 은 0 이 아니다. 예: $X = 0$ 이 되도록 좌현 정추진 $+45^\circ$, 우현 역추진 $-45^\circ$ 로 두면 $N \approx -950$ N·m
- **선수각을 지키며 옆으로만** 밀려면($X = N = 0$) 요를 지우는 데 추력을 써야 한다
  - 1-9절의 배분에서 $Y = 750$ N 이 추진기마다 944 N 을 요구하므로, 500 N 한계에서는 $Y_{\max} = 500 \times 750/944 \approx 397$ N
  - 이때 방위각은 좌현 $+23.4^\circ$, 우현 역추진 $-23.4^\circ$ 로 ±45° 안이다. 크기 한계만 걸린다
...
- $\operatorname{ssa}$ (smallest signed angle) — 각도 차를 $-180^\circ \sim +180^\circ$ 로 접는 함수. 코드는 `atan2(sin(d), cos(d))`
  - 접지 않으면 $359^\circ$ 와 $1^\circ$ 의 차이를 $358^\circ$ 로 보고 배가 반대로 한 바퀴 돈다
- $\mathbf{K}_p$, $\mathbf{K}_i$, $\mathbf{K}_d$ 는 3×3 대각행렬 — 대각 성분이 각각 $N$, $E$, $\psi$ 방향 게인
...
- $\mathbf{K}_d$ 는 **선체 감쇠를 뺀 나머지**만 넣으면 된다
  - $X_u = Y_v = 100$ N/(m/s), $N_r = 800$ N·m/(rad/s) 가 이미 감쇠로 작동한다
  - 설계 기준 — 선형 감쇠를 합친 전체 감쇠로 감쇠비 $\zeta$ 를 1 근처에 둔다 (2차 감쇠 $X_{|u|u}$ 등은 무시)
...
| $\mathbf{I}_{\max}$ (적분항 $K_i\!\int$ 의 상한) | 200 N | 200 N | 400 N·m | `i_max` |

- 선수 쪽은 $N_r$ 이 이미 커서 $\zeta_\psi$ 가 1 을 넘는다 — 과감쇠라 느리지만 흔들리지 않는다
...
- $j = 1, 2, 3$ 은 $N$, $E$, $\psi$ 축. $I_{\max,j}$ 는 위 표의 `i_max`
- 6주차 F-5 의 clamping 과 같은 장치다. 여기서는 3축 각각에 건다

---

## 1-9. 기준모델 — 계단 목표를 그대로 주면 안 된다

- 목표를 15 m 옆(동쪽)으로 **계단**으로 옮기면 제어기는 즉시 큰 힘을 요구한다
- 배가 북쪽을 보고 있으면($\psi = 0$, $\mathbf{J}^{\top} = \mathbf{I}$) 동쪽 위치 오차는 $\tilde{E} = E - E_d = -15$ m 이고, 1-8절의 식 그대로 (속도 0, 적분항 0 인 순간)

$$
\tau_Y \;=\; -K_{p}\,\tilde{E} \;=\; -50 \times (-15) \;=\; +750\ \text{N}
$$

- 부호가 맞다 — 목표가 우현(동) 쪽에 있으므로 우현으로 미는 힘($+Y$)이 나온다
...
- 옆으로 750 N 을 내는데 추진기마다 944 N — 요구량의 **1.26 배**다. 옆 성분은 375 N 씩뿐인데 왜 이렇게 커지는가

| 단계 | 무슨 일이 일어나는가 |
|---|---|
| ① 옆 힘 | 두 추진기가 각각 $F_y = 375$ N 을 낸다. 합이 750 N |
| ② 그런데 요가 생긴다 | 추진기가 **선미 2.37 m 뒤**에 있어서, 옆 힘이 $2 \times 375 \times 2.37 = 1780$ N·m 의 요 모멘트를 만든다 |
| ③ 그것을 지운다 | 좌·우 전후 힘을 반대로($\pm 866.7$ N) 내서 모멘트를 상쇄한다. 팔 길이가 1.03 m 로 짧아서 큰 힘이 든다 |
| ④ 합치면 | $\sqrt{866.7^2+375^2} = 944$ N. **힘의 대부분이 요를 지우는 데 쓰인다** |

- 944 N 은 **한계 500 N 을 넘는다.** 이 경우의 문제는 크기뿐이다
  - 우현 추진기의 요구 방위각 $156.6^\circ$ 는 $135^\circ$ 이상이라 1-5절의 역추진으로 접힌다 → $-944$ N @ $-23.4^\circ$, ±45° 안
  - 두 기가 **똑같이** 944 N 을 요구하므로 둘 다 500 N 으로 잘린다 → 같은 비율(500/944)로 줄어든다
  - 실현 $\boldsymbol{\tau} = [\,0,\ 397,\ 0\,]^{\top}$ N — 방향은 그대로이고 크기만 53 % 로 준다 (1-6절의 397 N)
- 실현 $\boldsymbol{\tau}$ 의 **방향이 틀어지는 것**은 두 기의 요구 크기가 **다를 때**다
  - 예: 위치와 선수각을 동시에 계단으로 바꾸면 한 기만 한계를 넘거나 두 기가 서로 다른 비율로 잘린다
  - 그러면 $X$·$Y$·$N$ 의 비율이 깨져 요가 흐트러진다 (과제 ⑤)
...
- 본 과목의 값: $\mathbf{v}_{\max} = [0.5,\ 0.5,\ 8^\circ/\text{s}]$, $\tau_{\text{ref}} = 6$ s
  - $\boldsymbol{\eta}_{\text{raw}}$ 는 시나리오 표의 계단 목표, $\boldsymbol{\eta}_d$ 는 기준모델이 내는 완만한 목표 — 제어기는 $\boldsymbol{\eta}_d$ 를 받는다
  - $\operatorname{sat}(\cdot,\ \pm\mathbf{v}_{\max})$ 는 성분마다 $\pm v_{\max}$ 로 자르는 함수

> [!warning] $\tau_{\text{ref}}$ 는 힘이 아니라 **시상수** [s] 다
> | 기호 | 뜻 | 단위 |
> |---|---|---|
> | $\boldsymbol{\tau}$, $\tau_Y$ | 제어력·모멘트 (1-3절부터) | N, N·m |
> | $\tau_{\text{ref}}$ | 기준모델 시상수 (코드 `tau_ref`) | s |
>
> 코드 변수 이름과 맞추려고 그대로 둔다. `tau_n`, `tau_del` (모터·서보 시상수) 도 같은 뜻의 $\tau$ 다.

> [!important] 실측으로 확인한 효과
> 기준모델을 끄면 최대 위치 오차 **18.1 m**, 켜면 **2.0 m**.
> 제어기도 게인도 그대로다. **목표를 주는 방식만 바꿨다.**
>
> 두 값은 기준이 다르다. `W10_plot` 은 오차를 제어기가 받는 $\boldsymbol{\eta}_d$ 에 대해 잰다.
> - 끄면 $\boldsymbol{\eta}_d = \boldsymbol{\eta}_{\text{raw}}$ — 계단이 바뀌는 순간의 오차가 그대로 잡힌다
> - 켜면 $\boldsymbol{\eta}_d$ 가 0.5 m/s 로 움직인다 — 2.0 m 는 움직이는 목표를 배가 뒤따르며 생기는 **지연 오차**다
> - 감쇠항 $-\mathbf{K}_d\boldsymbol{\nu}$ 가 목표의 속도를 모르고 배의 속도를 줄이려 하므로, 정속 추종 중에도 이 지연이 남는다
> - 어림셈 (적분항 무시): 지연 $\approx (K_d + Y_v)\,v_{\max}/K_p = (110 + 100) \times 0.5 / 50 \approx 2.1$ m
...
- 기호
  ...
  - $\beta_w$ — 참풍향 [rad, NED]. **바람이 불어오는 쪽**의 방위 (`wind_dir`, `W10_setup.m` 117행). $45^\circ$ 면 북동풍
...
- $\operatorname{atan2}$ 를 쓴다 — $\arctan(v_{rw}/u_{rw})$ 는 $u_{rw} < 0$ (뒷바람) 일 때 사분면을 잃는다
- $\rho_a = 1.225$ kg/m³ — 공기 밀도

> [!note] Fossen (2011) 식 (8.28)\~(8.29) 에는 부호가 $-$ 로 적혀 있다
> Fossen 의 $\beta_w$ 는 바람이 **불어가는** 쪽이다. 불어오는 쪽과 $180^\circ$ 차이이므로
> $\cos$, $\sin$ 의 부호가 뒤집혀 $u_{rw} = u - V_w\cos(\beta_w - \psi)$ 가 된다.
> 두 식은 같은 물리를 나타낸다. 본 과목은 기상 관례(풍향 = 불어오는 쪽)를 따른다.
>
> 확인: 선수가 북($\psi = 0$), 북풍($\beta_w = 0$), 정지($u = v = 0$) 이면
> $u_{rw} = +V_w$, $\gamma_{rw} = 0$, $C_X = -c_x < 0$ — 맞바람이 배를 **뒤로** 민다.
...
- $A_{Fw}$, $A_{Lw}$ — 수면 위 정면·측면 투영 면적. $L = 4.9$ m — 전장 (`L_oa`)
- 계수식은 Fossen (2011) 식 (8.21)
- 확인: 우현에서 부는 바람($\beta_w = 90^\circ$, $\psi = 0$, 정지) 이면 $v_{rw} = +V_w$, $\gamma_{rw} = -90^\circ$, $C_Y = -c_y$ — 배를 **좌현 쪽**($-Y$)으로 민다
...
- $\zeta(t)$ 는 수면 변위 [m] — 1-8절의 감쇠비 $\zeta$ 와 다른 뜻이다. 감쇠비는 1-8절에서만, 수면 변위는 1-10절에서만 쓴다
...
- $\theta_m$ — 파향 [rad, NED] (`wave_dir`). $\theta_m - \psi$ 는 선체 기준 파향
  - 파랑력은 평균 0 으로 진동하므로 파향을 "오는 쪽"으로 잡든 "가는 쪽"으로 잡든 위상만 반 주기 바뀐다
- $\ell_w = 1.2$ m — 파랑 요모멘트 팔길이 (`lw_arm`, 가정값)

> [!warning] $\theta$ 가 세 번째 뜻으로 쓰인다
> | 기호 | 뜻 | 처음 나온 곳 |
> |---|---|---|
> | $\theta$ | 피치각 | 3주차 |
> | $\theta$ | 로이터 중심에서 본 방위 | 8주차 1-4 |
> | $\theta_m$ | 파향 (Jin 외 2025 의 기호) | 10주차 1-10 |
>
> 논문과 대조하려고 그대로 둔다. 본 주차에서 $\theta$ 는 **아래첨자 $m$ 이 붙은 파향**뿐이다.
> [!note] 이것은 회절 계산이 아니다
...
- 본 과목의 값: $H_s = 0.30$ m, $T_p = 3.0$ s, $n_w = 5$, $k_w = 900$ N/m
  - 성분 개수를 $N$ 이 아닌 $n_w$ 로 쓴다. $N$ 은 이미 북쪽 위치와 요 모멘트다
...
- $T_s = 0.05$ s — 제어 주기 (`Ts_ctrl`), $\omega_f$ — 차단 주파수 (`w_filt`), $k$ — 스텝 번호
- 같은 필터를 속도 $\boldsymbol{\nu}$ 에도 건다. 감쇠항 $-\mathbf{K}_d\boldsymbol{\nu}$ 도 파랑에 흔들리지 않게 하려는 것
- 선수각은 $\psi[k] - \psi_f[k-1]$ 를 $\operatorname{ssa}$ 로 접은 뒤 거른다 — $\pm 180^\circ$ 경계에서 튀지 않게
...
- `W10_setup` 은 끝에 `auto_run = 1` 이 있어 **모델 실행과 `W10_plot` 까지 한 번에** 한다 (`W10_setup.m` 199행)
  - 따로 `sim` 을 다시 부르면 360초 시뮬레이션을 두 번 돌게 된다
...
| 위치 RMS 0.07\~0.24 m | 바람 6 m/s + 파랑 속에서 약 0.24 m 안쪽을 지킨다 (각 구간 마지막 20 s 기준) |
| 최대 오차 2.027 m | 기준모델이 목표를 옮기는 동안 배가 $\boldsymbol{\eta}_d$ 를 뒤따르며 생기는 지연 오차. 기준모델이 없으면 **18.1 m** 까지 간다 (§1-9, 두 값의 기준 차이도 거기에) |
...
| 읽는 법 | 뜻 |
|---|---|
| 계급 = 3 | 전후·좌우·요를 **모두** 따로 낼 수 있다. 2 가 나오면 방위각이 0 으로 묶여 있는 것 (7\~9주차와 같은 상태) |
| 성분 5개, 합성 진폭 0.189 m | 파랑을 주파수 5개의 합으로 만든다. 0.189 m 는 성분 진폭 $A_i$ 의 **합** — 다섯 파가 한순간 모두 마루에 오면 수면이 이만큼 오른다 (`W10_setup.m` 148행) |
| 추력 한계 500 N/기 | 1-9절에서 계단 목표가 944 N 을 요구해 **넘는** 바로 그 한계 |
| 바람·파랑 @ 45 deg | 바람은 북동쪽(45°)에서 **불어온다** (`W10_setup.m` 117행). 선수가 북이면 배를 비스듬히 밀어 전후·좌우·요가 동시에 흔들린다 — DP 가 세 자유도를 다 써야 하는 조건 |
...
```matlab
setappdata(0, 'W10_skip_run', true);   % W10_setup 의 자동 실행을 이번 한 번 건너뛴다
W10_setup
W10_alloc_demo
```

- 첫 줄이 없으면 `W10_setup` 이 360초 오프라인 시뮬레이션부터 돌린다 (`auto_run = 1`). B·D·E 절에서 같은 줄을 쓴다
- 그림 세 장이 뜬다
...
- 빨간 점선(모드 2)은 섹터 끝에 붙는다 — 클램프 (§1-5 의 두 경우)
  - $45^\circ\sim90^\circ$: 각도 $\pm 45^\circ$, 추력은 양수이고 $\cos$ 로 줄어든다
  - $90^\circ\sim135^\circ$: 각도 $\mp 45^\circ$, 추력은 **음수**(역추진)이고 $135^\circ$ 에 가까울수록 커진다
...
```matlab
setappdata(0, 'W10_skip_run', true);
W10_setup
...
> [!important] 앞 주차 주행이 끝난 상태에서 이어 돌리지 않는다
> DP 는 **스폰 위치를 원점(0, 0)으로 삼는다.** 배가 다른 곳에 있으면 목표가 통째로 옮겨간다.
> VRX 를 다시 띄워 스폰 위치로 되돌린 뒤 시작한다.
> RTF 가 1 % 미만인 노트북은 `"extra_gz_args:=--render-engine-server ogre"` 를 함께 준다.
...
- `W10_vrx_run` 이 끝에 오프라인 모델을 직접 돌려 대조한다. `W10_setup` 의 자동 실행은 필요 없다
...
- `f` 는 `[F_xL, F_yL, F_xR, F_yR]` 순서다. **1·3 성분의 부호가 반대**인 것이 횡이동·선회의 특징이다

- 스크립트 한 줄로 재현한다 (되먹임 없이 25초 발행 후 선체 기준 변위를 잰다)
...
> [!warning] 방위각을 **±90° 안으로 접어서** 보낸다
> 배분 결과를 그대로 극좌표로 바꾸면 우현이 `251.8 N @ +156.6°` 로 나온다.
> 추진기 방위각 한계는 **±45°** 이므로 이 값을 그대로 보내면 안 된다.
> 부호를 접어 **`−251.8 N @ −23.4°`** 로 바꾼 뒤 보낸다 (`W10_sway_demo` 가 자동으로 한다).
> 접지 않고 보냈더니 배가 옆으로 가지 않고 제자리에서 돌았다 (실측).

- 기준 환경 실측값 (25초 개루프)

| 항목 | 기준 환경 (2026-09-04) | 노트북 재측정 (2026-09-15) |
|---|---|---|
| 측정 구간 (시뮬레이션 시각) | 25 s | 19.9 s |
| 선체 기준 **우현** 이동 | **+10.86 m** | **+12.23 m** |
| 선체 기준 전후 이동 | −0.76 m | −0.83 m |
| 선수각 변화 | +4.7° | **−81.8°** |

- 우현 이동은 초당 **0.43 m** (10.86/25) 와 **0.61 m** (12.23/19.9) — 속도는 다르지만 두 환경 모두 **옆으로 간다.** 이것이 이 절의 논점이다
- **선수각 변화는 재현되지 않았다.** 재측정에서는 개루프 20초 만에 80도 가까이 돌았다
  - 되먹임이 없으므로 횡방향 운동이 만드는 요 모멘트를 아무도 잡아 주지 않는다
  - 즉 **틸팅만으로 자세가 유지되지는 않는다.** 자세를 잡는 것은 다음 절의 DP 되먹임이다
  - 두 값의 차이는 아직 원인을 확정하지 못했다. 수업에서는 **직접 돌려 나온 값**을 쓴다

> [!important] 틸팅 추진기가 만든 차이가 이 실험 하나에 드러난다
...
- 다른 노트북 재측정 (2026-09-15, RTF 0.721, 300초)

```
   t=   0 s  목표 (  0.0,   0.0,     0°)   위치 RMS  0.006 m   선수각 RMS  1.42°   정착   0.0 s
   t=  60 s  목표 ( 10.0,   0.0,     0°)   위치 RMS  0.004 m   선수각 RMS  0.03°   정착   0.0 s
   t= 140 s  목표 ( 10.0,  15.0,     0°)   위치 RMS  0.013 m   선수각 RMS  0.19°   정착   0.0 s
   t= 220 s  목표 ( 10.0,  15.0,    45°)   위치 RMS  0.003 m   선수각 RMS  0.18°   정착   0.0 s
   t= 300 s  목표 ( 10.0,  15.0,     0°)   위치 RMS  0.003 m   선수각 RMS  0.18°   정착   0.0 s
  최대 위치 오차           2.001 m
  평균 추력 크기            17.8 N
  포화·불감대 시간 비율      8.3 %
  추력 변화율 RMS            4.3 N/s
  방위각 변화율 RMS         4.81 deg/s
```

- 실제 출력에는 각 줄 끝에 `정착  0.0 s` 열이 붙는다. 목표가 바뀐 뒤 정착까지 걸린 시간이다
  - 이 값은 기준모델 출력 $\boldsymbol{\eta}_d$ 에 대한 오차로 잰다. $\boldsymbol{\eta}_d$ 가 천천히 움직이므로 오차가 처음부터 작아 **항상 0.0 s** 로 나온다 — 계단 목표에 대한 정착시간이 아니다

> [!warning] `t = 300 s` 줄은 300초 실행에서 **따로 측정되지 않는다**
> 각 구간 값은 "그 구간의 마지막 20 s" 로 계산한다. 300초 실행이면 다섯째 구간의 창이
> 넷째 구간과 같은 280\~300 s 가 되어 **두 줄이 같은 값**이 나온다 (위 두 실측 모두 그렇다).
> 목표 선수각이 0° 로 돌아온 뒤를 보려면 `W10_vrx_run(360)` 으로 부른다.

> [!note] 왜 VRX 가 오프라인보다 정확한가
> `sydney_regatta` 는 **바람이 꺼져 있는** world 다 (4주차 2-3-6 에서 확인).
...
- 다른 노트북 재측정 (2026-09-15, 페이싱 0.721): 평균 위치 오차 **0.544 / 0.388 m**, 이격 평균 **0.26 m** · 최대 **1.63 m**
- 7·8주차와 달리 **페이싱이 달라도 이격이 거의 같다.** DP 는 제자리에 머무는 시간이 길어 시간 어긋남이 위치 차이로 드러나지 않기 때문이다
...
- 이 한 쌍이 **배치까지 다시 만든다.** 별도의 정리 명령이 필요 없다
- 생성이 끝나면 배선 검사 결과가 찍힌다. **겹침 0 · 블록관통 0 · 꺾임3회+ 0** 이 정상

> [!note] 최상위의 신호 흐름은 상자 일곱 개로 읽는다
> `DPRef` -> `WaveFilter` -> `DPCtrl` -> `Alloc` -> `Thrusters` -> `Env` -> `MotionModel`
>
> - 게인·추력 한계·바람·파랑 설정 같은 **설정값은 전부 각 상자 안**에 있다
> - 레퍼런스 모델·적분기·서보처럼 **한 스텝 전 값을 기억하는 상태**도 상자 안에서 태그로 돈다
> - `Logging` 과 `Animate`, VRX 모델의 `CmdPublisher` 는 **입출력 포트가 없는 상자**다
...
- [ ] `pos` 부호를 뒤집지 않으면 어떻게 되는지 확인했다 (`S = W10_sway_demo(25, 200, +1);` — 셋째 인자가 `pos` 부호, 기본값 `-1`)
...
- $X = N = 0$ 을 지키는 순수 횡이동의 $Y$ 최대값이 약 397 N 인 것을 $\mathbf{T}_e^{\dagger}$ 로 유도하시오 (§1-6, §1-9)
...
- VRX 에서는 world 를 바꿔 바람을 넣고 같은 실험 (4주차 2-3-6 의 바람 토픽 확인 참조)
...
- 목표를 **위치와 선수각 동시에** 크게 바꾸면 어떻게 되는가
  - 예: `dp_step` 의 마지막 행 `[300 10 15 0]` 을 `[300 0 0 90]` 으로 바꾼다 — 위치 (10, 15) → (0, 0) 과 선수 0° → 90° 를 한꺼번에
...
| 포화 비율이 30 % 를 넘는다 | 게인이 너무 높다 | $\omega_n$ 을 계산해 볼 것 |
...
| `W10_simulink/W10_sway_demo.m` | 개루프 횡이동 실험 (§E-2) |
| `W10_simulink/build_w10_models.m` | 모델 재생성 |
| `W10_simulink/tidy_layout.m` | 옛 배치 복구용 (지금은 `build_w10_models` 가 배치까지 만든다) |
