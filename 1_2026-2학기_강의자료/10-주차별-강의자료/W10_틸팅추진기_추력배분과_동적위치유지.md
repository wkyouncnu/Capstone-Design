...
- $\delta_L$, $\delta_R$ 은 좌·우 추진기 방위각 — 6주차 방향타 각 $\delta_{\text{rud}}$ 와 다름
...
- $\psi_{\text{ref}}$ 는 $\boldsymbol{\eta}_d$ 의 셋째 성분 (목표 선수각, 6주차와 같은 기호)
...
- 부호 주의: 여기 $\tilde{y} = y - y_d$ (현재−목표) 는 6주차 오차 $e = \psi_{\text{ref}} - \psi$ (목표−현재) 와 **부호 방향이 반대**임 — 그래서 식 앞에 $-K_{p,\text{pos}}$ 가 붙음
...
- $T_s = 0.05$ s — 제어 주기 (`Ts_ctrl`), $\omega_f$ — 차단 주파수 (`w_filt`), $k$ — 스텝 번호 (9주차 구간 인덱스 $k$ 와 다름)
...
> [!important] 네 주차 가운데 가장 잘 맞음 — 이격이 실행마다 흔들리지 않는 것은 10주차뿐
...
>
> - 7\~9주차 이격은 실행마다 몇 배씩 달라짐. 페이싱 비율(측정 RTF)과 실제 진행이 어긋난 만큼 같은 길을 다른 시각에 지나기 때문 (위상 오차, 8주차 D-2 · 9주차 D-2)
...
- 오프라인 모델(`W10_0_offline`)에서 먼저 바꾸고, 최대 위치 오차($\boldsymbol{\eta}_d$ 기준)와 추력 변화율을 기본값과 비교한 뒤, 같은 수정을 VRX 모델에 옮겨 확인하시오
- 기본 모델은 바꾸지 않음. 복사본(`save_system('W10_0_offline','W10_my')`)에서 할 것
