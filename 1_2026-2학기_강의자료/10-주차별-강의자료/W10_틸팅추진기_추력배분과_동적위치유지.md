...
> 1\~9주차의 WAM-V 는 추진기 방향이 **고정**이었음. 낼 수 있는 힘은 전후(surge)와
...
> Gazebo 의 `left` 엔진은 ENU 선체계에서 $y=+1.027$ 임. ENU 의 $y$ 는 **좌현**임
> NED 선체계의 $y$ 는 **우현**이므로 좌현 추진기는 $y_L = -1.027$ 이 됨
...
> `rank(T_e) = 3` 임 — `W10_setup` 이 실행할 때마다 출력함
> 7\~9주차의 고정 추진기는 $\delta=0$ 이라 $Y$ 행이 항상 0 이므로 계급이 2 였음

> [!tip] 방위각 0 이면 7\~9주차 식으로 돌아감
> $\delta_i=0$ 이면 $F_{yi}=0$ 이므로
> $N = -y_L F_{xL} - y_R F_{xR} = b\,(F_L - F_R)$.
> 여기서 $b = y_R = -y_L = 1.0271$ m 는 반폭, $F_L = F_{xL}$, $F_R = F_{xR}$ 임
> 4주차 1-2절의 차동추진 식과 정확히 같음. `W10_setup` 이 이것을 `assert` 로 확인함
...
| 2 | **경계 클램프** — 가까운 섹터 끝으로 밀고 크기를 $\cos$ 로 줄임 | 쓸 수 있는 성분만 남김 (기본값, `W10_setup.m` 60행) |
...
> 옆 성분만 남기는 것. 아래 §1-6 의 실제 배분 결과를 볼 것
...
| $\mathbf{K}_d\,\boldsymbol{\nu}$ | 감쇠 | **오차를 미분하지 않음** — 7주차 P–D 헤딩 제어와 같은 이유 |
...
- 6주차 F-5 의 clamping 과 같은 장치. 여기서는 3축 각각에 걺
...
- 944 N 은 **한계 500 N 을 넘음**. 이 경우의 문제는 크기뿐임
  ...
  - 실현 $\boldsymbol{\tau} = [\,0,\ 397,\ 0\,]^{\top}$ N — 방향은 그대로이고 크기만 53 % 로 줆 (1-6절의 397 N)
...
- 가까워지면 시상수 $\tau_{\text{ref}}$ 의 **지수 접근** → 도착할 때 꺾임이 없음
...
> [!warning] $\tau_{\text{ref}}$ 는 힘이 아니라 **시상수** [s] 임
> | 기호 | 뜻 | 단위 |
> |---|---|---|
> | $\boldsymbol{\tau}$, $\tau_Y$ | 제어력·모멘트 (1-3절부터) | N, N·m |
> | $\tau_{\text{ref}}$ | 기준모델 시상수 (코드 `tau_ref`) | s |
>
> 코드 변수 이름과 맞추려고 그대로 둠. `tau_n`, `tau_del` (모터·서보 시상수) 도 같은 뜻의 $\tau$ 임
...
> - `W10_plot` 의 "최대 위치 오차" 는 두 가지를 찍음. 기준모델 출력 $\boldsymbol{\eta}_d$ 기준 (켜면 1.977 m) 과 계단 목표 $\boldsymbol{\eta}_{\text{raw}}$ 기준 (둘 다 약 15 m — 계단 크기 그 자체). **앞의 1.977 m 는 지연 오차**
...
- $\zeta(t)$ 는 수면 변위 [m] — 1-8절의 감쇠비 $\zeta$ 와 다른 뜻임. 감쇠비는 1-8절에서만, 수면 변위는 1-10절에서만 씀
...
- 본 과목의 값: $H_s = 0.30$ m, $T_p = 3.0$ s, $n_w = 5$, $k_w = 900$ N/m
  - 성분 개수를 $N$ 이 아닌 $n_w$ 로 씀. $N$ 은 이미 북쪽 위치와 요 모멘트
...
> [!important] 두 모델의 관계
> `W10_0_offline.slx` 와 `W10_1_vrx.slx` 는 **제어기·배분·게인이 완전히 같음**
> 운동모델만 오프라인 방정식에서 Gazebo 로 바뀜. 7\~9주차와 같은 구성
...
- `W10_setup` 은 끝에 `auto_run = 1` 이 있어 **모델 실행과 `W10_plot` 까지 한 번에** 함 (`W10_setup.m` 199행)
...
| 최대 오차 1.977 m | 기준모델이 목표를 옮기는 동안 배가 $\boldsymbol{\eta}_d$ 를 뒤따르며 생기는 지연 오차 (§1-9) |
...
| 읽는 법 | 뜻 |
|---|---|
| 계급 = 3 | 전후·좌우·요를 **모두** 따로 낼 수 있음. 2 가 나오면 방위각이 0 으로 묶여 있는 것 (7\~9주차와 같은 상태) |
| 성분 5개, 합성 진폭 0.189 m | 파랑을 주파수 5개의 합으로 만듦. 0.189 m 는 성분 진폭 $A_i$ 의 **합** — 다섯 파가 한순간 모두 마루에 오면 수면이 이만큼 오름 (`W10_setup.m` 148행) |
| 추력 한계 500 N/기 | 1-9절에서 계단 목표가 944 N 을 요구해 **넘는** 바로 그 한계 |
| 바람·파랑 @ 45 deg | 바람은 북동쪽(45°)에서 **불어옴** (`W10_setup.m` 117행). 선수가 북이면 배를 비스듬히 밀어 전후·좌우·요가 동시에 흔들림 — DP 가 세 자유도를 다 써야 하는 조건 |
...
- 첫 줄이 없으면 `W10_setup` 이 360초 오프라인 시뮬레이션부터 돌림 (`auto_run = 1`). B·D·E 절에서 같은 줄을 씀
- 그림 세 장이 뜸
...
- `W10_vrx_run` 이 끝에 오프라인 모델을 직접 돌려 대조함. `W10_setup` 의 자동 실행은 필요 없음
...
> Gazebo 의 관절 축은 ENU 선체계(x 전방, **y 좌현**) 기준 $+z$ 임
> 본 모델의 $\delta$ 는 NED 선체계(**y 우현**) 기준임
...
- DP 를 걸기 전에 §1-6 의 횡이동 배분을 **직접 발행**해 봄
...
> 부호를 접어 **`−251.8 N @ −23.4°`** 로 바꾼 뒤 보냄 (`W10_sway_demo` 가 자동으로 함)
...
- `pos_sign` 을 +1 로 주면(`W10_sway_demo(25, 200, +1)`) 두 방위각이 거꾸로 돎. 손으로 계산하면 $F_y$ 가 두 추진기 모두 $-100$ N 이 되어 **좌현으로** 밀리고, 요 모멘트 $2 \times (-2.37) \times (-100) \approx +475$ N·m 가 생겨 **돌면서** 감 — `Gain(-1)` 이 왜 필요한지 확인하는 실험 (결과를 직접 돌려 기록할 것)

> [!important] 틸팅 추진기가 만든 차이가 이 실험 하나에 드러남
> 9주차까지의 배는 이 명령에 대해 **옆으로 0 m** 였음
...
- 기준 환경 실측값 (2026-09-18, 데스크톱, 측정 RTF 0.675, 360 s, `sydney_regatta` — 바람·파랑 없음)
...
===== VRX DP =====
  구간별 정상상태 오차 (각 구간 마지막 20 s)
   t=   0 s  목표 (  0.0,   0.0,     0°)   위치 RMS  0.004 m   선수각 RMS  0.78°   정착   0.0 s
   t=  60 s  목표 ( 10.0,   0.0,     0°)   위치 RMS  0.003 m   선수각 RMS  0.01°   정착  19.8 s
   t= 140 s  목표 ( 10.0,  15.0,     0°)   위치 RMS  0.007 m   선수각 RMS  0.25°   정착  27.9 s
   t= 220 s  목표 ( 10.0,  15.0,    45°)   위치 RMS  0.004 m   선수각 RMS  0.10°   정착   0.0 s
   t= 300 s  목표 ( 10.0,  15.0,     0°)   위치 RMS  0.005 m   선수각 RMS  0.71°   정착   0.0 s
  최대 위치 오차           1.943 m   (기준모델 목표 eta_d 기준)
  최대 위치 오차          15.006 m   (계단 목표 eta_raw 기준)
  평균 추력 크기            16.2 N
  포화·불감대 시간 비율     10.8 %
  추력 변화율 RMS            4.2 N/s
  방위각 변화율 RMS        13.77 deg/s
```

- 정착 19.8 s · 27.9 s — 오프라인 20.2 s · 28.0 s 와 0.4 s 안
...
> `sydney_regatta` 는 **바람이 꺼져 있는** world 임 (4주차 2-3-6 에서 확인)
...
| 항목 | 오프라인 | VRX | 차이 |
|---|---|---|---|
| 평균 위치 오차 [m] | 0.433 | 0.315 | −0.118 |
| **궤적 평균 이격** [m] | | | **0.16** |
| **궤적 최대 이격** [m] | | | **0.86** |

- 2026-09-18, 360 s, 같은 초기 선수각 32.67°. 평균 위치 오차는 전 구간 $\lvert \boldsymbol{\eta} - \boldsymbol{\eta}_d \rvert$ 평균
- 오프라인이 더 큰 것은 바람 6 m/s · 파랑을 넣었기 때문 (VRX 쪽은 외란 없음)
- DP 는 제자리에 머무는 시간이 길어, 시간 어긋남이 위치 차이로 드러나지 않음

> [!important] 네 주차 가운데 가장 잘 맞음 (2026-09-18, 같은 데스크톱)
> 7주차 0.28 m, 8주차 4.96 m, 9주차 0.83 m, **10주차 0.16 m**
> - 8주차만 큰 것은 원을 도는 속도가 조금 달라 **같은 원의 다른 지점**에 있기 때문 (위상 오차, 8주차 D-2)
> - DP 는 속도가 느리고 과도응답이 적어 두 모델의 차이가 드러날 일이 적음
...
| 4 | 도달가능 제어집합 | 선분 → 면적 |
| 5 | 3자유도 DP PID | 위치 RMS 0.07\~0.11 m |
| 6 | 기준모델 | 오버슈트 2.84 → 1.65 m, 추력 변화율 40.2 → 16.4 N/s |
| 7 | 바람·파랑 외란 | 문헌 식 그대로 |
| 8 | 파랑 필터 | 추력 변화율 −52 % |
...
- [ ] `pos` 부호를 뒤집지 않으면 어떻게 되는지 확인했음 (`S = W10_sway_demo(25, 200, +1);` — 셋째 인자가 `pos` 부호, 기본값 `-1`)
...
| 증상 | 원인 | 해결 |
|---|---|---|
| DP 가 선수를 못 잡고 계속 돎 | `pos` 부호 또는 $\mathbf{T}_e$ 요 행 부호 | §E-1 확인. `W10_setup` 의 `assert` 를 지우지 말 것 |
...
| 오프라인은 되는데 VRX 가 다름 | 페이싱이 RTF 와 다름 | `W10_vrx_run` 이 자동으로 맞춤 |
...
| 파일 | 내용 |
|---|---|
| `W10_simulink/W10_setup.m` | 설정. 이 파일만 고치면 됨 |
...
| `W10_simulink/tidy_layout.m` | 옛 배치 복구용 (지금은 `build_w10_models` 가 배치까지 만듦) |
