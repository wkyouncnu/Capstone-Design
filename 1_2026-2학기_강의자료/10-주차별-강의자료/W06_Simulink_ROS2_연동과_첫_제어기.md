...
> - 헤딩 게인을 **계산으로 정하는 방법**은 1-7절(극배치)에서, 추력을 나누는 방법은 10주차에서 다룬다
...
- 정상 출력 (선수각 30° 계단, 6주차 오프라인 모델과 같은 비선형 요 모델 · 포화 ±500 N·m)
...
| OS 선형 > OS 실측 | 비선형 모델에는 $N_{rr}|r|$ 감쇠가 더 있다. 선형 예측은 **보수적**이다 |
...
$$
\begin{aligned}
\text{되돌릴 양} \;&=\; u_{\text{sat}} - u_{\text{raw}} \\[2pt]
\text{적분기 입력} \;&=\; K_i\,e \;+\; K_b\,\bigl(u_{\text{sat}} - u_{\text{raw}}\bigr)
\end{aligned}
$$

| 기호 | 뜻 |
|---|---|
| $u_{\text{raw}}$ | 제어기가 원했던 값 |
| $u_{\text{sat}}$ | 포화를 지나 **실제로 나간** 값 |
| $K_b$ | 되감기 이득 (`Kb_gain`) |
...
> I절 속도 제어의 `PI_u` 는 여기서 쓴 **같은 PID 블록**이다. 바뀌는 것은 무엇을
> 오차로 넣느냐뿐이다 — 위치, 속도.
...
$$
N \;=\; K_p\,\operatorname{ssa}(\psi_{\text{ref}} - \psi) \;-\; K_d\,r
$$

| 기호 | 뜻 | 값 |
|---|---|---|
| $\psi_{\text{ref}},\ \psi$ | 목표·현재 선수각 [rad] | — |
...
오차는 $e = \psi_{\text{ref}} - \psi$ 이므로, 양변을 시간으로 미분하면

$$
\dot{e} \;=\; \dot{\psi}_{\text{ref}} - \dot{\psi} \;=\; \dot{\psi}_{\text{ref}} - r
$$

목표가 가만히 있는 동안에는 $\dot{\psi}_{\text{ref}} = 0$ 이므로
...
즉 $-K_d\,r$ 과 $+K_d\,\dot{e}$ 는 **같은 값**이다. 게인을 바꿀 필요도 없다.
...
닫힌 루프의 요 운동으로 보면 더 분명하다. 선체는 스스로 항력을 가지고 있고,
D 항은 그 옆자리에 들어간다.

$$
I_z\,\dot{r} \;=\; N - (N_r + N_{rr}\lvert r \rvert)\,r
\;=\; \underbrace{K_p\,\operatorname{ssa}(\psi_{\text{ref}}-\psi)}_{\text{끌고 가는 힘}}
