...
- 정상 출력 (선수각 30° 계단, 비선형 요 모델 $I_z\dot r = N - (N_r + N_{rr}|r|)\,r$ · 포화 ±500 N·m)
  ...
  - 열 이름 `OS실측%` 는 VRX 실측이 아니라 이 비선형 모델을 `ode45` 로 푼 **시뮬레이션 값**
...
| OS 선형 > OS 비선형(`OS실측%` 열) | 비선형 모델에는 $N_{rr}|r|$ 감쇠가 더 있음. 선형 예측은 **보수적** |
...
- 아래 그림은 $N_f = 200$ 을 마지막에 그려 나머지 두 곡선을 덮음. $N_f$ = 5 · 20 의 제어입력 크기는 위 표의 표준편차로 비교함
