...
- 양변을 시간으로 미분하면 `d(psi_NED)/dt = −d(psi_ENU)/dt` 다.
...
오차는 `e = psi_ref − psi` 이므로

```
de/dt = d(psi_ref)/dt − r
```

목표가 가만히 있는 동안에는 `d(psi_ref)/dt = 0` 이므로 **`de/dt = −r`** 이다.
즉 `−Kd·r` 과 `+Kd·de/dt` 는 **같은 값**이다. 게인을 바꿀 필요도 없다.

차이는 목표가 계단으로 바뀌는 순간에만 난다.

- `de/dt` — 오차가 한 스텝에 껑충 뛰므로 미분값이 폭발한다 (**미분 킥**)
- `r` — 배가 실제로 도는 속도다. 지령이 아무리 튀어도 배는 천천히 돈다

`W06_heading_sign` 을 실행하면 같은 배 모델에 세 가지를 걸어 본 결과가 나온다.

| D 항 | 오버슈트 | 정착시간 | 포화 전 \|N\| 최대 |
|---|---|---|---|
| **`−Kd·r`** (채택) | 0.2 % | 4.7 s | **628** N·m |
| `+Kd·de/dt` | 0.2 % | 4.7 s | **6912** N·m |
| `+Kd·r` (부호 반대) | 17.0 % | 10.0 s | 678 N·m |

- 추종 성능은 앞의 둘이 **완전히 같다** — 위의 `de/dt = −r` 때문이다
...
- 배가 왼쪽으로 돌고 있으면 `r < 0` → `−Kd·r > 0` → 오른쪽으로 되미는 모멘트
- 부호를 빼면 돌던 방향으로 더 밀어 준다. 회전을 **부추기는** 되먹임이 된다

닫힌 루프의 요 감쇠로 보면 더 분명하다. 선체는 스스로 항력 `Nr` 을 가지고 있고,
D 항은 거기에 `Kd` 를 **더한다.** 부호를 뒤집으면 반대로 **깎는다.**

| `+Kd·r` 의 `Kd` | 남는 감쇠 `Nr − Kd` | 오버슈트 |
|---|---|---|
| 200 | 600 | 10.0 % |
| 400 | 400 | 17.0 % |
| 800 | 0 | 34.7 % |
...
> [!note] 발산까지 가지는 않는다
> 남는 감쇠가 음수가 되어도 2차 항력(`Nrr·|r|`)이 버틴다. 그래도 오버슈트가
> 100 %를 넘으면 쓸 수 있는 응답이 아니다. **`Kd` 에 비례해 나빠진다는 것이
> D 항이 하는 일이 정확히 감쇠라는 증거다.**
...
> `psi` 와 **`r` 을 따로 입력으로 받는다.** D 항은 `Kp·Td·(r_d − r)` 이고,
> 목표 각속도 `r_d = 0` 이면 `−Kp·Td·r` 이다.
