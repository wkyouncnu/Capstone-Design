...
- 1-5절은 게인을 **키우면 무엇이 달라지는지**만 봤음. 이 절은 게인을 **계산으로 정함**
...
| $T$ | 시상수 — 요각속도가 자리를 잡는 데 걸리는 시간 ($= T_r$, 3주차 1-8. 3주차 0.47 s 는 $N_{rr}$ 까지 넣어 선형화한 값) | **0.816 s** |
| $K$ | Nomoto 이득 — 모멘트 1 N·m 당 정상 요각속도 | $1.25 \times 10^{-3}$ rad/(s·N·m) |

> [!note] 방향타가 있는 배라면
> 조종 문헌의 Nomoto 모델은 입력이 모멘트가 아니라 **방향타 각** $\delta_{\text{rud}}$ ($T\dot r + r = K\delta_{\text{rud}}$, rud = rudder)
...
$$
N = K_{p,\psi}\,(\psi_{\text{ref}} - \psi) - K_{d,\psi}\,r
$$

- 이것을 요 운동 식에 넣고 $r = \dot\psi$ 로 바꾸면

$$
I_z\,\ddot\psi + \left(N_r + K_{d,\psi}\right)\dot\psi + K_{p,\psi}\,\psi = K_{p,\psi}\,\psi_{\text{ref}}
$$

$$
\frac{\psi(s)}{\psi_{\text{ref}}(s)}
= \frac{K_{p,\psi} / I_z}{\,s^2 + \dfrac{N_r + K_{d,\psi}}{I_z}\,s + \dfrac{K_{p,\psi}}{I_z}\,}
$$
...
- 식을 보면 역할이 갈림. $K_{p,\psi}$ 는 $\omega_n$ 을, $K_{d,\psi}$ 는 $\zeta$ 를 정함
- $N_r$ 이 $K_{d,\psi}$ 옆에 더해져 있음 — **물이 이미 감쇠를 주고 있다**는 뜻
...
$$
\boxed{\;K_{p,\psi} = \omega_n^2\,I_z,
\qquad
K_{d,\psi} = 2\zeta\sqrt{K_{p,\psi}\,I_z} - N_r\;}
$$
...
- 예 — $\zeta = 0.9$, $\omega_n = 1.0$ rad/s 를 원하면 $K_{p,\psi} = 1.0^2 \times 653 = 653$, $K_{d,\psi} = 2(0.9)\sqrt{653 \times 653} - 800 = 375$

> [!warning] $K_{d,\psi}$ 가 음수로 나오면
> $K_{d,\psi} = 2\zeta\sqrt{K_{p,\psi} I_z} - N_r$ 에서 $N_r$ 이 크면 음수가 됨. 원하는 감쇠를
> **배가 혼자서 이미 넘게 주고 있다**는 뜻임. 예를 들어 $\zeta = 0.9$, $\omega_n = 0.6$ 이면
> $K_{d,\psi} = -95$ 가 나옴. 이때는 $K_{d,\psi} = 0$ 으로 두고 $\zeta$ 가 목표보다 조금 커지는 것을 받아들임
...
1. **페이싱 켜기** — Run 옆 화살표 → Simulation Pacing → Enable 체크, 비율 = §1-3 에서 잰 RTF
2. 정지 시간을 `20` 으로 설정
3. **Run**
...
- 1-5절의 경향표와 같은 내용이며, 위 실측값이 그대로 뒷받침함
...
$$
N \;=\; K_{p,\psi}\,\operatorname{ssa}(\psi_{\text{ref}} - \psi) \;-\; K_{d,\psi}\,r
$$

| 기호 | 뜻 | 값 |
|---|---|---|
| $\psi_{\text{ref}},\ \psi$ | 목표·현재 선수각 [rad]. 모델의 `psi_ref` | — |
| $r$ | 요각속도 [rad/s], NED | 측정값 |
| $\operatorname{ssa}(\cdot)$ | 각도를 $[-\pi,\,\pi]$ 로 접음 | — |
| $K_{p,\psi},\ K_{d,\psi}$ | 헤딩 루프 비례·미분 이득 (블록 `Kp_psi`, `Kd_rate`) | 800, 400 |
...
- 오차 $e = \psi_{\text{ref}} - \psi$ 의 양변을 시간으로 미분

$$
\dot{e} \;=\; \dot{\psi}_{\text{ref}} - \dot{\psi} \;=\; \dot{\psi}_{\text{ref}} - r
$$

- 목표가 가만히 있는 동안에는 $\dot{\psi}_{\text{ref}} = 0$

$$
\dot{e} = -\,r
\qquad\Longrightarrow\qquad
K_{d,\psi}\,\dot{e} \;=\; -\,K_{d,\psi}\,r
$$

- 결과: $-K_{d,\psi}\,r$ 과 $+K_{d,\psi}\,\dot{e}$ 는 **같은 값**. 게인을 바꿀 필요도 없음
...
|---|---|---|---|
| $-K_{d,\psi}\,r$ (채택) | 0.2 % | 4.7 s | **628** N·m |
| $+K_{d,\psi}\,\dot{e}$ | 0.2 % | 4.7 s | **6912** N·m |
| $+K_{d,\psi}\,r$ (부호 반대) | 17.0 % | 10.0 s | 678 N·m |
...
- D 항은 **브레이크**
- 배가 왼쪽으로 돌고 있으면 $r < 0$ → $-K_{d,\psi} r > 0$ → 오른쪽으로 되미는 모멘트
...
\;=\; \underbrace{K_{p,\psi}\,\operatorname{ssa}(\psi_{\text{ref}}-\psi)}_{\text{끌고 가는 힘}}
\;-\;\underbrace{(N_r + K_{d,\psi})\,r}_{\text{감쇠}} \;-\; N_{rr}\lvert r \rvert\,r
$$

- 감쇠 항이 $(N_r + K_{d,\psi})$. D 항은 선체가 가진 $N_r$ 에 $K_{d,\psi}$ 를 **더함**
- 부호를 뒤집으면 $(N_r - K_{d,\psi})$ 가 되어 반대로 **깎음**

- 스크립트가 출력하는 $K_{d,\psi}$ 스윕 중 일부

| $+K_{d,\psi} r$ 의 $K_{d,\psi}$ | 남는 감쇠 $N_r - K_{d,\psi}$ | 오버슈트 |
|---|---|---|
| 200 | 600 | 10.0 % |
| 400 | 400 | 17.0 % |
| 800 | 0 | 34.7 % |
...
| 배가 조금씩 앞서거나 뒤처짐 (궤적은 겹침) | 페이싱 비율 ≠ 실측 RTF | §1-3. RTF 를 다시 재서 비율을 맞춤 |
