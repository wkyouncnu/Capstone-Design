...
> - 헤딩 게인을 **계산으로 정하는 방법**은 1-7절(극배치)에서, 틸팅 추진기의 추력 배분은 10주차에서 다룬다
...
> **재보고, 틀린 만큼 고친다.** 재보지 않으면 제어가 아니라 단순한 명령이다.
...
> 극배치는 1-7절에서 다룬다. 근궤적·주파수 응답은 선수 과목의 몫이다.
...
>   을 학부 캡스톤용으로 간추린 것이다. 원자료는 볼트에 포함되지 않는다
...
- 1-6절은 게인을 **키우면 무엇이 달라지는지**만 봤다. 이 절은 게인을 **계산으로 정한다**
- 필요한 것은 배의 숫자 두 개(요 관성 $I_z$, 요 감쇠 $N_r$)와, 원하는 응답을 나타내는 숫자 두 개($\zeta$, $\omega_n$)뿐이다
- 대학원 강의 W03 §3-1 \~ §3-3 을 WAM-V 수치로 옮긴 것이다

### 요 운동 — Nomoto 1차 모델

- 배를 돌리는 모멘트 $N$ 과 요각속도 $r$ 사이는 1차 시스템이다 (6주차 오프라인 모델과 같은 식에서 $N_{rr}$ 항만 뺀 것)
...
- 양변을 $N_r$ 로 나누면 선박 조종 문헌이 **Nomoto 모델**이라 부르는 모양이 된다
...
> [!note] 방향타가 있는 배라면
> 조종 문헌의 Nomoto 모델은 입력이 모멘트가 아니라 **방향타 각** $\delta$ 다 ($T\dot r + r = K\delta$).
> WAM-V 에는 방향타가 없고 좌·우 추력 차이가 곧 모멘트이므로 $N$ 이 그 자리에 들어간다. 모양은 같다.

### 제어기를 붙이면 2차 시스템이 된다

- 6주차 헤딩 제어기는 요각속도를 되먹이는 P-D 다 (H절)
- 목표 선수각은 수식에서 $\psi_d$ 로 쓴다. 모델·코드·그림의 이름은 `psi_ref` 이며 같은 것이다
...
| $\zeta$ | 감쇠비 | **넘치는가** — 1 에 가까울수록 넘치지 않는다 |

- 식을 보면 역할이 갈린다. $K_p$ 는 $\omega_n$ 을, $K_d$ 는 $\zeta$ 를 정한다
- $N_r$ 이 $K_d$ 옆에 더해져 있다 — **물이 이미 감쇠를 주고 있다**는 뜻이다

### 거꾸로 — 원하는 응답에서 게인을 정한다 (극배치)

- 원하는 $\zeta$, $\omega_n$ 을 먼저 고르고 게인을 푼다
...
- 분모 $s^2 + 2\zeta\omega_n s + \omega_n^2 = 0$ 의 두 근이 **닫힌 루프의 극**이다. $\zeta$, $\omega_n$ 을 고르는 것이 곧 극의 자리를 고르는 것이라 **극배치**라 부른다
...
> $K_d = 2\zeta\sqrt{K_p I_z} - N_r$ 에서 $N_r$ 이 크면 음수가 된다. 원하는 감쇠를
> **배가 혼자서 이미 넘게 주고 있다**는 뜻이다. 예를 들어 $\zeta = 0.9$, $\omega_n = 0.6$ 이면
> $K_d = -95$ 가 나온다. 이때는 $K_d = 0$ 으로 두고 $\zeta$ 가 목표보다 조금 커지는 것을 받아들인다.
...
- 정상 출력 (선수각 30° 계단, 비선형 요 모델 $I_z\dot r = N - (N_r + N_{rr}|r|)\,r$ · 포화 ±500 N·m)
  - 오프라인 모델의 요 자유도와 계수는 같지만 **모터 지연(0.3 s)·샘플링(0.05 s)은 제외**한 연속시간 모델이다
  - 열 이름 `OS실측%` 는 VRX 실측이 아니라 이 비선형 모델을 `ode45` 로 푼 **시뮬레이션 값**이다
...
| 읽는 법 | 뜻 |
|---|---|
| 6주차 게인 $\omega_n = 1.11$, $\zeta = 0.83$ | 셋 중 가장 빠르고(4.3 s) 아주 조금 넘친다 |
| 7주차부터 게인 $\omega_n = 0.78$, $\zeta = 0.98$ | 가장 느리지만(7.9 s) 넘치지 않는다. 극이 실수축에 붙어 있다 |
| 설계값 | 두 세트의 **사이**. 극배치로 원하는 지점에 정확히 놓을 수 있다 |
| OS 선형 > OS 비선형(`OS실측%` 열) | 비선형 모델에는 $N_{rr}|r|$ 감쇠가 더 있다. 선형 예측은 **보수적**이다 |
| 세 세트 모두 극이 왼쪽 반평면 | 모두 안정하다. 다른 것은 속도와 넘침의 **맞바꿈**이다 |

- 6주차의 "적당히 되는 값"과 7주차의 값은 어느 쪽도 틀리지 않았다. **다른 지점을 고른 것**이고, 이 표가 그 지점이 어디인지 보여 준다
...
- 실습 E 의 "Kp 를 키우면 빨라지고 넘친다"가 식으로는 $\omega_n = \sqrt{K_p/I_z}$ 가 커지고 $\zeta \propto 1/\sqrt{K_p}$ 가 작아지는 것이다

> [!tip] 해 볼 것
> `W06_pole_place.m` 의 `zeta_d`, `wn_d` 를 바꿔 극이 어디로 가는지 본다.
> `step_deg` 를 90 으로 올리면 $K_p e$ 가 포화(±500 N·m)에 걸려 **선형 예측이 맞지 않게 된다.**
> 극배치는 포화 전까지만 약속을 지킨다.
...
- 모델을 처음 열면 아래 화면이 나온다
- 먼저 어디를 눌러야 하는지부터 익힌다
...
- 읽은 값으로 MATLAB 의 함수 쪽을 **맞춘다**
  - 프로필을 한 번도 저장하지 않은 새 설치에는 값이 없으므로 `try` 로 감싼다 (없으면 0)
...
> [!note] `W06_vrx_record` 스크립트는 이 작업을 자동으로 한다 (J-4절)
> 실행하면 첫 줄에 `0) ROS_DOMAIN_ID = 8` 처럼 무엇을 쓰는지 찍는다.
> 이 값이 WSL 의 값과 다르면 거기서 멈추고 고칠 것. 7주차부터 쓰는 `W0X_vrx_run` 도 같다.
...
> **초록 = 운동모델**, **회색 = 관찰(Scope·로깅)**, **흰색 = 설정값(Constant)**. A0 의 표와 같은 규칙이다.
...
> 이 관계가 **감쇠(damping)** 다. G절 운동방정식의 $\left(X_u + X_{uu}|u|\right)u$ 항이 그것이다.
...
- **Run** 후 `Logging` 서브시스템 안 `Scope_all` 을 연다
  - 출력 `y` 와 제어입력 `tau` 가 그려짐
  - 목표값은 Scope 에 없다. 목표는 t = 1 s 에 0 → 1 로 바뀌는 계단 (`Ref` 블록)
...
- 1-6절의 경향표와 같은 내용이며, 위 실측값이 그대로 뒷받침한다
...
> 위 그림과 같은 구조가 들어 있다. D 갈래에 `Derivative` 블록이 아니라 필터가 붙은 미분이 들어 있음을 직접 확인할 수 있다.
...
- 출력의 최대 차이가 **10⁻¹⁶**, 즉 배정밀도 실수의 반올림 한계다 (제어입력은 10⁻¹⁴ 수준)
...
> [!warning] $N_f$ 는 요 모멘트 $N$ · 북쪽 위치 $N$ 과 다른 기호다
> 미분 필터 계수는 코드에서 `Nf`, `Nf2` 이므로 수식에서도 $N_f$ 로 쓴다.
> Simulink PID 블록 대화상자의 **Filter coefficient (N)** 가 이것이다.

![Dcompare 서브시스템 — 1번 순수 미분, 2번 pseudo-derivative, 3번 참값](W06_simulink/img/W06_Dcompare.png)

- 결과는 서브시스템 안 `Scope_D` 를 열어 본다
- 순수 미분은 **참값의 220 배**가 나온다. 신호는 알아볼 수 없다
- pseudo-derivative 도 참값보다 크다 (1.73 대 0.50)
  - 잡음(표준편차 약 0.02)의 빠른 성분이 최대 $N_f$ 배까지 커져 남기 때문
  - 그래도 순수 미분보다 약 60 배 작다 — 잡음을 없애는 것이 아니라 **증폭에 상한을 두는 것**
...
> 필터가 있으면 그 값이 $N_f \times$ (계단 높이) 로 막힌다.
...
- 아래 그림은 $N_f = 200$ 을 마지막에 그려 나머지 두 곡선을 덮는다. $N_f$ = 5 · 20 의 제어입력 크기는 위 표의 표준편차로 비교한다
...
- **적당한 값**: 이 과목에서는 $N_f$ = 10 \~ 20 을 기본으로 쓴다
...
- 제어입력은 1-6절 규칙대로 $\tau$ 다 ($u$ 는 전후 속도)
- 상한 포화 중에는 이 값이 음수, 하한 포화 중에는 양수 — 어느 쪽이든 적분기를 한계 반대쪽으로 끌어내어 더 쌓이지 못하게 한다
...
- 즉 2.5 m/s 는 **이 추력 한계에서는** 도달할 수 없는 요구다
- 250 N/편은 본 과목이 정한 한계다. VRX 플러그인의 한계는 편당 2353.6 N 이다 (`wamv_gazebo_thruster_config.xacro` 의 `max_thrust_cmd`)
...
> [!important] 여기서 만든 것이 구조 그대로 다음 절로 간다
> I절 속도 제어의 `PI_u` 는 **같은 PI 구조**의 라이브러리 Discrete PID 블록이다. 블록과 게인은 다르다.
>
...
>
> - 같은 게인을 쓰지 않으므로 G절의 응답 수치를 I절에 그대로 기대하지 않는다
> - 7주차 값은 `W07_setup.m` 에 있다
> H절 헤딩 제어만 예외다. 거기서는 D 항을 오차 미분이 아니라 **요각속도**에서
> 얻는다. 이유는 H절에서 숫자로 확인한다.
...
> [!important] 제어기는 상자 하나다
> `HeadingCtrl` 은 **독립 모듈**이다. 오차 계산·게인·포화가 전부 그 안에 있고,
> 밖에서는 입력 세 개(`psi_ref`, `psi`, `r`)와 출력 하나(`N`)만 보인다.
> 다른 모델로 옮길 때 이 상자만 복사하면 된다.
...
- ENU 는 z 축이 위, NED 는 z 축이 아래이므로 **회전 방향이 반대**다

- 선수각 변환식의 양변을 시간으로 미분하면 회전 속도의 부호도 따라 나온다
...
| $\operatorname{ssa}(\cdot)$ | 각도를 $[-\pi,\,\pi]$ 로 접는다 | — |
...
| `Kd_rate` | **요각속도에 `−400`** 을 곱한다 |
| `SumN` → `SatN` | 두 항을 더하고 ±500 N·m 로 자른다 |
...
- 차이는 목표가 계단으로 바뀌는 순간에만 난다

- $\dot{e}$ — 오차가 한 스텝에 껑충 뛰므로 미분값이 폭발한다 (**미분 킥**)
- $r$ — 배가 실제로 도는 속도다. 지령이 아무리 튀어도 배는 천천히 돈다

- `W06_heading_sign` 을 실행하면 같은 배 모델에 세 가지를 걸어 본 결과가 나온다
...
- 추종 성능은 앞의 둘이 **완전히 같다** — 위의 $\dot{e} = -r$ 때문이다
- 갈리는 것은 계단이 들어오는 순간의 지령이다. 미분 쪽은 포화 한계 500 의
  **14 배**까지 치솟는다. 잘려 나가므로 배는 같게 움직이지만, 실제 추진기에는
  이런 명령이 그대로 부담이 된다
...
- 부호를 빼면 돌던 방향으로 더 밀어 준다. 회전을 **부추기는** 되먹임이 된다

- 닫힌 루프의 요 운동으로 보면 더 분명하다
  - 선체는 스스로 항력을 가지고 있음
  - D 항은 그 옆자리에 들어감
...
- 감쇠 항이 $(N_r + K_d)$ 다. D 항은 선체가 가진 $N_r$ 에 $K_d$ 를 **더한다**
- 부호를 뒤집으면 $(N_r - K_d)$ 가 되어 반대로 **깎는다**
...
> [!note] 발산까지 가지는 않는다
> 남는 감쇠가 음수가 되어도 2차 항력 $N_{rr}\lvert r \rvert\,r$ 이 버틴다. 그래도
> 오버슈트가 100 %를 넘으면 쓸 수 있는 응답이 아니다. **$K_d$ 에 비례해 나빠진다는
> 것이 D 항이 하는 일이 정확히 감쇠라는 증거다.**

![D 항 세 가지](W06_simulink/img/W06_heading_dterm.png)

> [!note] MSS 툴박스도 같은 구조다
> Fossen 의 `demoOtterUSVHeadingControl` 을 열어 보면 `Heading autopilot` 이
> $\psi$ 와 **$r$ 을 따로 입력으로 받는다.** D 항은 $K_p T_d\,(r_d - r)$ 이고,
> 목표 각속도 $r_d = 0$ 이면 $-K_p T_d\,r$ 이다.
> 7주차 이후의 `InnerLoop` 도 이 식을 그대로 쓴다.

### 실행

1. 페이싱 켜기
2. `psi_ref_deg` 를 `45` 로 두고 Run
   - 스폰 직후 $\psi \approx 32.7^\circ$ 다 (3주차 3-1절 좌표 검산 — 스폰 yaw 1 rad = 57.3°, $90^\circ - 57.3^\circ$)
   - 따라서 `psi_ref_deg = 45` 는 0 → 45° 가 아니라 **12.3° 계단**이다
3. `Scope_psi` 를 열면 **실제 헤딩과 목표 헤딩**이 함께 그려짐
   - 단위는 **rad** 이다 (45° = 0.785 rad, 32.7° = 0.571 rad)
4. 정지 시간이 `inf` 이므로 30 s 쯤 지나면 **정지** 버튼으로 멈춘다
...
> `Kp=800, Kd=400` 은 이론 없이 고른 값이지만, 극으로 옮기면 $\omega_n = 1.11$ rad/s, $\zeta = 0.83$ 이다.
> 그 계산과, 원하는 응답에서 게인을 거꾸로 푸는 극배치는 1-7절에 있다.
...
> [!warning] `u_ref = 1.5` 에서는 선회 여유가 거의 없다 — 추력 포화
...
> - 따라서 선회 중 속도 저하의 상당 부분은 **포화** 때문이다. 운동방정식의 결합(J-2)만의 효과가 아니다
> - 7주차 `Thrusters` 서브시스템 절(`F_max` 를 500 N 으로 두는 이유)에서 이 한계를 다시 다룬다
...
| `PI_u` 의 안티와인드업 끄기 | `u_ref` 3.0 으로 20 s 돌린 뒤 1.0 으로 낮춘다 — 내려오는 데 걸린 시간을 켠 경우와 비교 |
...
> 반면 **선회는 요각속도 7.3 %, 반경 11 %** 차이가 난다 (4.61 m 대 4.09 m). 요 방향에는 오프라인 모델이 담지 못한 것이 있다
...
- 검산: P 만 행은 `W06_pid_compare('Kp')` 의 `Kp = 10` 행, PD 행은 `W06_pid_compare('Ki')` 의 `Ki = 0` 행과 일치해야 한다
- PID 행(`Ki = 8`)은 스크립트가 만들지 않으므로 `Kp = 10; Ki = 8; Kd = 4;` 로 두고 직접 실행한다
- 스텝은 t = 1 s 에 들어간다. 상승시간·정착시간은 1 s 부터 잰다
...
- 배는 0° 에서 출발하지 않는다. 스폰 직후 선수각 $\psi_0$ (약 32.7°) 를 `Scope_psi` 에서 읽어 기록한다 (rad → deg 환산)
- `psi_ref_deg` = $\psi_0 + 60$ 으로 두고 실행 — **60° 계단** 기준으로 성능표 작성
- 조건마다 VRX 를 다시 띄워 스폰 위치에서 시작한다
- 아래 세 조건에서 각각
...
- 이 조건은 추진기가 편당 250 N 에 붙는 포화 구간이다 (I절 경고). 분석에서 포화와 운동방정식 결합을 구분할 것
...
- 이번 주차에 작성한 4단계 inner loop 가 **구조 그대로 안쪽에 들어간다.** 앞단에 유도 블록만 붙는다
  - 게인은 7주차 값(헤딩 400/200, 속도 PI 300/100)으로 바뀐다 — 두 헤딩 게인 세트의 차이는 1-7절 표
- 준비물
