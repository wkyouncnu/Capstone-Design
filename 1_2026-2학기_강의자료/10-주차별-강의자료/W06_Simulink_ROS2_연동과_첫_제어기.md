...
> ROS 의 선체축은 $x$ 선수·$y$ **좌현**·$z$ **위**(FLU), 제어식은 $x$ 선수·$y$ **우현**·$z$ **아래**(FRD)
> $y$ 와 $z$ 가 뒤집혀 있으므로 옆 속도와 요각속도는 **부호가 반대** (4주차 센서 배치 절, 3주차 ENU→NED)
> 그래서 이 주차 모델의 `Quat2Yaw` 블록은 `r = -wz` 로 뒤집어서 내보냄 — 헤딩 제어기의 $-K_d\,r$ 이
