...
%
%       위치  x_n = (lat - lat0)*Rm,   y_n = (lon - lon0)*Rn*cos(lat0)   (x 북, y 동)
%       자세  psi_NED = pi/2 - yaw_ENU
%       각속도 r_NED  = -r_ENU
%
%   최상위 구성 (역할별 서브시스템)
%     2단계   SensorSubscriber -> Nav -> [x_n] [y_n] [psi] [r] 태그  (+ Animate · Logging)
...
code = [ ...
'function [x_n, y_n, z_n, yaw_enu, psi] = ' fname '(lat, lon, alt, qx, qy, qz, qw, lat0, lon0, alt0)' newline ...
'%#codegen'                                                                     newline ...
...
''                                                                              newline ...
'x_n =  (lat - lat0)*pi/180 * Rm;           % north [m]'                        newline ...
'y_n =  (lon - lon0)*pi/180 * Rn*cos(lat0r);% east  [m]'                        newline ...
'z_n = -(alt - alt0);                       % down  [m]'                        newline ...
''                                                                              newline ...
...
function build_frame_check()
    ...
    note(m,'n1', ['W03 1단계  —  좌표 변환식 확인 (VRX 불필요)' newline ...
        '9주차 §2-3 에서 실제로 받은 GPS·IMU 값 한 벌을 넣고 NED 로 바꾼다.' newline ...
        'yaw_ENU 와 psi_NED 가 90도 차이인지, x_n·y_n 이 기준점에서 몇 m 인지 눈으로 확인한다.' newline ...
...
%   입력 lat, lon, alt, qx, qy, qz, qw, r_enu   출력 x_n, y_n, psi, r
% =====================================================================
function addNav(m, pos)
    ...
    % z_n(수직)은 수상선 제어에 쓰지 않는다. 열어 두면 미연결 포트로 잡히므로 종단한다
    add_block('simulink/Sinks/Terminator', [ss '/D_unused'], 'Position',[700 640 720 660]);
    L(ss,'Ned/3','D_unused/1');

    % x_n · y_n · psi · r 태그는 최상위에서 만든다 (여기서 또 만들면 태그가 중복된다)
...
'function ok = AnimateFcn(x_n, y_n, psi, t, en)'                         newline ...
'%#codegen'                                                            newline ...
...
function build_vrx_nav()
    ...
    addLogging(m, [650 560 820 650], {'x_n','y_n','psi','r','yaw_enu','valid'});

    note(m,'n1', ['W03 2단계  —  VRX 의 GPS·IMU 를 NED 로 (VRX 필요)' newline ...
        'SensorSubscriber -> Nav -> [x_n] [y_n] [psi] [r] 태그' newline ...
...
function build_vrx_drive()
    ...
    addLogging(m, [1000 700 1170 790], ...
               {'x_n','y_n','psi','r','yaw_enu','valid','thr_left','thr_right'});
...
function build_teleop()
    ...
    addLogging(m, [1000 380 1170 470], {'x_n','y_n','psi','u','v','r','FL','FR'});
...
function build_offline()
    ...
    addTeleopAnimate(m, [650 380 820 470]);
    addLogging(m, [1000 380 1170 470], {'x_n','y_n','psi','u','v','r','FL','FR','valid'});
...
% ---- 운동방정식 + 적분기 + 상태 꺼내기 -------------------------------
%   입력 FL, FR [N]   출력 태그 [x_n] [y_n] [psi] [u] [v] [r]   (NED, 선체축)
...
'function [x_n, y_n, psi, u, v, r] = States(s)'                            newline ...
'%#codegen'                                                              newline ...
'% 상태 벡터 s = [u v r x_n y_n psi] 에서 이름 있는 신호로 꺼낸다.'            newline ...
...
'function [x_n, y_n, psi, u, v, r] = Nav(ex, ey, qx, qy, qz, qw, bx, by, wz)'      newline ...
'%#codegen'                                                                    newline ...
'% odometry 하나로 위치·자세·속도를 모두 얻는다 (ENU -> NED).'                  newline ...
'%   위치   x_n = ey,  y_n = ex      ENU 의 y 가 북, x 가 동'                   newline ...
'%   자세   psi = pi/2 - yaw_ENU'                                              newline ...
'%   속도   u = bx,  v = -by,  r = -wz   y·z 축이 뒤집히므로 부호가 바뀐다'      newline ...
'%'                                                                           newline ...
'% 기준점은 첫 메시지의 위치로 잡는다. 그래야 항적이 (0,0) 에서 시작한다.'      newline ...
'persistent x0 y0 seen'                                                       newline ...
'if isempty(seen), seen = false; x0 = 0; y0 = 0; end'                         newline ...
''                                                                            newline ...
'xr = ey;   yr = ex;'                                                         newline ...
'if ~seen && (abs(ex) + abs(ey)) > 0'                                         newline ...
'    seen = true;  x0 = xr;  y0 = yr;'                                        newline ...
'end'                                                                         newline ...
'x_n = xr - x0;'                                                                newline ...
'y_n = yr - y0;'                                                                newline ...
''                                                                            newline ...
...
% ---- 실시간 그림 — 왼쪽 항적, 오른쪽 추력 F_L·F_R 과 psi·u·v·r ----------
...
'function ok = AnimateFcn(x_n, y_n, psi, u, v, r, FL, FR, t, en)'                      newline ...
'%#codegen'                                                                  newline ...
'% 그림은 코드 생성 대상이 아니므로 extrinsic 으로 부른다.'                    newline ...
'coder.extrinsic(''W03_teleop_plot'');'                                      newline ...
'ok = 1;'                                                                    newline ...
'if en > 0.5'                                                                newline ...
'    W03_teleop_plot(x_n, y_n, psi, u, v, r, FL, FR, t);'                              newline ...
'end']);
