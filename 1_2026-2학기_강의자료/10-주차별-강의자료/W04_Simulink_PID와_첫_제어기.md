...
| 오버슈트 | **0.6 %** | **2.9 %** | VRX 가 조금 더 넘침. 요 방향에 모델에 없는 것이 있음 |
...
| 항목 | 오프라인 쌍둥이 | VRX | 읽는 법 |
|---|---|---|---|
| 헤딩 오버슈트 · 정착 | 0.6 % · 4.0 s | 2.6 % · 6.3 s | R 절과 같음 — 속도 루프가 헤딩에 영향을 주지 않음 |
| $u$ 가 1.5 의 63 % 에 닿는 시각 | 2.5 s | 2.5 s | 전진 방향은 항력 계수가 같아 딱 맞음 |
| 40 초 뒤 $u$ | 1.413 m/s | 1.414 m/s | 아직 1.5 에 못 닿음 — $K_{i,u} = 40$ 이 작아 느림 |
...
- 다른 컴퓨터에서 재측정해도 같음 (2026-09-15, RTF 0.98): $u$ **1.332**, $r$ **+15.72 / $-15.73$** deg/s, 반경 **4.08 m**
- 이 값들은 벽시계가 아니라 **odometry 헤더 스탬프**로 재므로 RTF 가 달라도 재현됨

> [!important] 직진은 소수점 셋째 자리까지 맞음
> 항력 계수를 시뮬레이터에서 그대로 가져왔기 때문임
> 반면 **선회는 요각속도 7.3 %, 반경 11 %** 차이가 남. 요 방향에는 오프라인 모델이 담지 못한 것이 있음
