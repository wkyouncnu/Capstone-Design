...
| 왼쪽 큰 칸 초반의 휨 | 32.7° 에서 출발 | **스폰 자세에서 출발** (이번 캡처는 약 10°) | 휘는 각이 다름 |
| 왼쪽 큰 칸 정착 뒤 | 45° 직선 | 45° 직선 | 같음 |

- 다른 것은 **출발 선수각뿐임.** 오프라인은 32.7° 고정, VRX 는 스폰 자세에 따라 매번 조금 다름 — 초반 곡선의 휨만 달라지고 정착 뒤는 같음
...
- 두 VRX 캡처(2-17 · 이 절) **사이에 VRX 를 완전히 종료하고 다시 띄웠음.** 위 caution 의 이유와 같음
