...
| 배포 파일 | `W04_simulink/` (모델 14개 + 스크립트 9개 + 결과 그림 `img/`) |
...
W04_setup 완료.
  ...
  화면     animate=1  animate_every=0.25 s  (끄려면 animate = 0)
  ROS      ROS_DOMAIN_ID=7  (VRX 쌍둥이를 돌릴 때만)
```

| 읽는 법 | 뜻 |
|---|---|
| 앞의 여섯 줄이 1\~5단계의 모든 값 | 학생이 고칠 파일은 `W04_setup.m` **하나**뿐임 |
| `Kd_psi=-400` 이 음수 | 2-8 의 부호 규칙. 오타가 아님 |
| `boat_open=0`, `head_open=0` | 폐루프가 기본. 개루프 예제(2-1·2-6)에서 1 로 바꿈 |
| `화면` 줄 | 실시간 화면 스위치. 바로 아래 항에서 설명함 |
| `ROS` 줄 | VRX 쌍둥이(2-15\~2-18)가 쓰는 도메인. `setenv` 까지 `W04_setup` 이 대신함 (2-14-5) |
...
- 2부의 배 모델은 돌아가는 동안 **창 하나가 열려** 항적과 지령 대비 응답을 그림. 화면 구성은 2-0-1 에 있음
- 켜고 끄는 것은 `W04_setup.m` 의 두 줄뿐임

| 변수 | 기본값 | 뜻 |
|---|---|---|
| `animate` | 1 | 1 = 켬, 0 = 끔 |
| `animate_every` | 0.25 | 다시 그리는 주기 [시뮬레이션 초] |

```matlab
W04_setup
animate = 0;                       % 화면 없이 숫자만 볼 때
sim('W04_3_heading_offline');
```

- 노트북이 느리면 `animate = 0` 으로 끔 — 40 초 실행의 벽시계가 **1.41 s 에서 0.39 s 로** 줄어듦 (3.6 배)
- 화면을 남기되 부담만 줄이려면 `animate_every` 를 0.5\~1.0 으로 키움
- 실측 — 컴파일 비용을 뺀 두 번째 실행의 벽시계, 오프라인 모델 · 기준 환경 (2026-09-24)

| 모델 | 시뮬 시간 | `animate = 1` | `animate = 0` | 배 |
|---|---|---|---|---|
| `W04_3_heading_offline` | 40 s | 1.41 s | 0.39 s | 3.6× |
| `W04_4_inner_loop_offline` | 40 s | 1.23 s | 0.44 s | 2.8× |
| `W04_5_offline` | 80 s | 1.67 s | 0.44 s | 3.8× |

- 3주차 `W03_setup.m` 과 **같은 이름 · 같은 뜻**임. 주차가 바뀌어도 스위치를 다시 찾을 일이 없음
...
![질량-스프링-댐퍼 장치와 그 자유물체도](../assets/w04-msd-fbd.svg)

**이 그림에서 볼 것** — 왼쪽 ① 이 장치 그 자체이고, 오른쪽 ② 는 **질량 하나만 떼어 내 걸리는 힘을 빠짐없이 그린 것**임. 맨 아래 한 줄이 ② 를 식으로 옮긴 것이며 그것이 곧 아래 유도의 2\~4번임

| 그림 안의 것 | 아래 유도·결과 식의 어느 항 |
|---|---|
| 보라색 화살표 $f$ | 유도 1번의 미는 힘. 결과 식의 오른쪽 변 |
| 왼쪽을 향한 $k\,y$ | 유도 1번·3번의 $-k y$ |
| 왼쪽을 향한 $b\,\dot y$ | 유도 1번·3번의 $-b\dot y$ |
| 회색 상자 $m$ | 유도 2번의 $m\ddot y$ |
| 위쪽 $y$ 화살표 | 변위의 $+$ 방향. $y = 0$ 은 스프링이 늘지도 줄지도 않은 자리 |

- **자유물체도를 그리는 이유는 부호를 외우지 않기 위함**임. $y$·$\dot y$ 가 양일 때 스프링과 댐퍼의 힘은 왼쪽을 향하므로 식에서 $-$ 가 붙음 — **화살표를 그린 방향이 그대로 식의 부호**가 됨
- 3주차의 배 운동방정식도 같은 순서로 나옴. 대상만 수레에서 선체로 바뀔 뿐 "그림 → 힘 → 식" 의 세 걸음은 같음
...
**모델 도면과 여는 법** — 모델은 1-4 와 같은 `W04_P0_second_order` 임 (도면 → 1-4 의 「표준 2차계 모델」)

```matlab
W04_setup
open_system('W04_P0_second_order')   % 도면을 띄워 zeta·wn 이 어느 블록에 들어가는지 확인
W04_pid_compare('zeta')              % 1-4 의 그림을 다시 뽑아 아래 표와 맞춰 봄
```
...
![피드백 루프 하나와 PID 의 세 갈래](../assets/w04-pid-loop.svg)

**이 그림에서 볼 것** — 루프를 한 바퀴 도는 신호의 이름과, 그 가운데 제어기 상자 안이 **세 갈래로 갈라졌다 다시 합쳐지는 것** 하나임

| 그림 안의 것 | 결과 식의 어느 항 |
|---|---|
| 왼쪽 합산점 ($+$ 와 $-$) | $e = y_d - y_m$. **빼는 쪽이 측정값**임 |
| 주황 상자 셋 | 결과 식 $\tau = K_p e + K_i\!\int\! e + K_d \dot e$ 의 세 항 |
| 세 상자 옆의 지금·과거·앞으로 | 결과 식 밑줄의 세 이름과 같음 |
| 오른쪽 합산점 ($+$ 셋) | 세 갈래를 더해 $\tau$ 하나를 만드는 자리 |
| 초록 상자 $G(s)$ | 1-3 에서 적은 플랜트 $1/(s^2+2s+2)$ |
| 보라 상자 센서 | 측정값 $y_m$. 1-11 의 잡음이 들어오는 자리 |

- **세 갈래가 읽는 것은 같은 $e$ 하나**임. 그림에서 세 상자로 들어가는 선이 한 점에서 갈라지는 것이 그 뜻임
- 1-10 에서 이 세 갈래를 Gain·Integrator·Sum 으로 직접 조립함. 그림의 상자 하나가 블록 하나가 됨
...
- 모델은 1-6 과 같은 `W04_P1_pid_step` 임 (도면 → 1-6 의 「PID 스텝응답 모델」). 바뀌는 것은 `PID_lib` 블록의 **P 칸 하나**뿐임

**여는 법과 실행**

```matlab
W04_setup
open_system('W04_P1_pid_step')   % PID_lib 을 더블클릭해 P 칸을 확인한 뒤
...
- 모델은 1-6 과 같은 `W04_P1_pid_step` 임 (도면 → 1-6). 바뀌는 것은 `PID_lib` 블록의 **D 칸 하나**뿐임

**여는 법과 실행**

```matlab
W04_setup
open_system('W04_P1_pid_step')   % PID_lib 의 D 칸과 N 칸을 확인한 뒤
...
- 모델은 1-6 과 같은 `W04_P1_pid_step` 임 (도면 → 1-6). 바뀌는 것은 `PID_lib` 블록의 **I 칸 하나**뿐임

**여는 법과 실행**

```matlab
W04_setup
open_system('W04_P1_pid_step')   % PID_lib 의 I 칸을 확인한 뒤
...
**라이브러리 블록에서는 같은 것을 대화상자로 고른다**

![라이브러리 PID 블록에서 포화와 되감기를 고르는 자리](../assets/w04-pid-dialog.svg)

**이 그림에서 볼 것** — 직접 조립한 블록 하나하나가 라이브러리 블록에서는 **칸 하나**가 된다는 것. 두 판의 차이는 $K_b$ 칸이 나타나는가 하나뿐임

| 대화상자의 칸 | `PID_byhand` 의 어느 블록 | 결과 식의 어느 항 |
|---|---|---|
| Main 탭의 **P / I / D / N** | `Kp_gain` · `Ki_gain` · `Kd_gain` · `PseudoD` | $K_p$·$K_i$·$K_d$·$N_f$ |
| Output saturation → **Limit output** | `Sat` 블록을 넣는 것과 같음 | $\tau_{\text{sat}} = \operatorname{sat}(\tau_{\text{raw}})$ |
| Upper / Lower limit | `Sat` 의 두 한계 | $\pm u_{\max}$ |
| Anti-windup method | 되감기 갈래를 넣는가 | $\dot I$ 의 둘째 항 (1-12) |
| Back-calc. coefficient (Kb) | `Kb_gain` | $K_b$ |

- **(a) back-calculation 이 본 과목이 쓰는 방식**임. `PID_byhand` 의 `SumAW` → `Kb_gain` 갈래가 이 칸에 해당함
- (b) clamping 은 계수 칸이 아예 없음 — 조정할 값이 없는 방식임 (1-12 에서 둘을 나란히 봄)
...
![유사미분 — 미분에 증폭의 천장을 둔 것](../assets/w04-pseudo-derivative.svg)

**이 그림에서 볼 것** — 가로축이 주파수, 세로축이 그 주파수에 걸리는 이득임. 순수 미분만 **끝까지 올라가고** 유사미분은 각자의 높이에서 **평평해짐**

| 그림 안의 것 | 유도의 어느 줄 |
|---|---|
| 검은 직선 (순수 미분 $K_d s$) | 1번 — $\omega$ 가 곱해지므로 천장이 없음 |
| 색 곡선 셋의 **왼쪽 기울어진 부분** | 4번 — $\omega \ll N_f$ 에서는 미분 그대로 |
| 색 곡선 셋의 **오른쪽 평평한 부분** | 5번 — $\omega \gg N_f$ 에서 $K_dN_f$ 에 멈춤 |
| 점선 세 개 ($800$·$80$·$20$) | 그 천장값 $K_dN_f$ 자체 |
| 왼쪽 세로 표시 $\omega_n = 1.41$ | 루프가 실제로 일하는 주파수. 네 곡선이 거의 같음 |
| 오른쪽 세로 표시 $\pi/T_s = 3142$ | 잡음이 들어 있는 주파수. 여기서 $157$ 배로 갈림 |

- **유사미분이 신호를 버리지 않는 이유가 이 그림 한 장**임 — 느린 쪽에서는 순수 미분과 겹치고 빠른 쪽에서만 갈라짐
- 곡선 색은 아래 예제 ② 의 결과 그래프와 같음 (회색 $N_f = 200$ · 주황 $20$ · 파랑 $5$)
...
- 모델은 1-10 과 같은 `W04_P2_pid_byhand` 임 (도면 → 1-10 의 두 그림). 바뀌는 것은 `PID_byhand` 안 **`PseudoD` 의 $N_f$ 하나**뿐임

```matlab
W04_setup
open_system('W04_P2_pid_byhand/PID_byhand')   % PseudoD 블록의 분모를 확인한 뒤
...
![와인드업이 생기는 자리 — 포화 중에 루프가 끊긴다](../assets/w04-windup.svg)

**이 그림에서 볼 것** — (a) 는 **끊기는 자리가 어디인가**, (b) 는 **얼마나 밖으로 나갔는가**임. 둘 다 적분기가 아니라 **포화**를 가리킴

| 그림 안의 것 | 현상·유도의 어느 줄 |
|---|---|
| (a) 빨간 $\times$ 표 | 현상 2번 — 포화 중에는 게인을 올려도 나가는 값이 늘지 않음 |
| (a) `Sat` 앞뒤의 $\tau_{\text{raw}}$·$\tau_{\text{sat}}$ | 결과 식의 두 기호. 포화 중에만 서로 다름 |
| (a) 보라색 갈래 | 되감기. 아래 `SumAW` → `Kb_gain` 이 이것 |
| (b) 가운데 기울기 1 구간 | 포화가 아닌 구간. $\tau_{\text{sat}} = \tau_{\text{raw}}$ |
| (b) 평평한 두 구간 | 포화 구간. 이 구간에 있는 동안 적분기가 쌓임 (현상 3번) |
| (b) 보라 점과 화살표 | 계단 직후 요구 $K_p e = 10$ 가운데 **7.5 가 버려짐** (유도 5번) |

- **(b) 에는 적분기가 아예 없음.** 그런데도 요구와 실제가 다름 — 와인드업의 원인은 적분기가 아니라 그 **간격을 적분기가 모르는 것**임
...
![되감기 두 방식 — 갈라지는 자리는 적분기 입력 한 곳뿐](../assets/w04-antiwindup-two.svg)

**이 그림에서 볼 것** — 두 판의 보라색 블록 **바깥은 완전히 같음**. 넘친 양 $\varepsilon = \tau_{\text{sat}} - \tau_{\text{raw}}$ 를 재는 자리까지 같고, 그것을 적분기 입력에서 어떻게 쓰는가만 다름

| 방식 | 적분기 입력에서 하는 일 | 고를 계수 | 본 과목 |
|---|---|---|---|
| (a) **잠그기** (clamping) | 더 쌓이기만 할 상황이면 적분기를 끔 → $\dot I = 0$ | 없음 | 쓰지 않음 |
| (b) **되감기** (back-calculation) | $K_b\varepsilon$ 을 더해 적분기를 끌어냄 → 결과 식 그 자체 | $K_b$ | **`PID_byhand` 가 이것** |

- (b) 의 보라색 합산점이 `SumAW`, 보라색 이득이 `Kb_gain` 임. 라이브러리 PID 블록에서는 1-10 의 대화상자 그림 (a) 판에 해당함
- **(b) 가 멈추는 자리는 $\varepsilon = 0$ 이 아님** — $\dot I = 0$ 에서 $\varepsilon = -(K_i/K_b)\,e$ 임. 그 값을 적분값으로 바꿔 적은 것이 바로 아래 $I^\star$ 유도임
...
- 모델은 1-10 과 같은 `W04_P2_pid_byhand` 이고 **`Kb` 한 값만** 0 과 2 로 바꿈. 잡음은 꺼서 적분기만 봄

**모델 도면 — `PID_byhand` 안의 되감기 회로**

![되감기 회로 — 보라색 테두리 셋이 `Sat` · `SumAW` · `Kb_gain`](W04_simulink/img/W04_PID_byhand__AW.png)

| 블록 (보라색 테두리) | 결과 식의 어느 항 | 하는 일 |
|---|---|---|
| `Sat` | $\tau_{\text{sat}} = \operatorname{sat}(\tau_{\text{raw}})$ | `SumU` 가 만든 $\tau_{\text{raw}}$ 를 $\pm u_{\max}$ 에서 자름. **출력 포트 `tau` 로 나가는 것은 잘린 쪽**임 |
| `SumAW` | 괄호 안의 $\tau_{\text{sat}} - \tau_{\text{raw}}$ | 자르기 전과 후를 뺌. 부호가 $-$ · $+$ 인 것은 $\tau_{\text{raw}}$ 를 빼기 때문임. 포화가 없으면 이 값이 **0** |
| `Kb_gain` | $K_b(\cdot)$ | 그 차이에 $K_b$ 를 곱함. `Kb = 0` 이면 갈래 전체가 없는 것과 같음 |
| (되돌아가는 곳) `SumI` | $\dot I = K_i e + \cdots$ 의 덧셈 | `Ki_gain` 의 출력과 되감기 값을 더해 `Integ` 로 보냄 |

- **세 블록이 그리는 것은 되먹임 고리 하나**임 — `SumU` → `Sat` → `SumAW` → `Kb_gain` → `SumI` → `Integ` → `SumPI` → `SumU`
- 이 고리는 **포화 중에만 일함**. 포화가 풀리면 `SumAW` 의 출력이 0 이 되어 평소의 PID 와 완전히 같아짐 (결과 식 아래 설명)
- 라이브러리 PID 블록에서 같은 것을 고르는 자리는 1-10 의 대화상자 그림 (a) 임

**여는 법과 실행**

```matlab
W04_setup
open_system('W04_P2_pid_byhand/PID_byhand')   % 위 도면이 그대로 열림
%  Sat 을 더블클릭 → Upper/Lower limit 이 ±u_max
%  Kb_gain 을 더블클릭 → Gain 칸이 Kb
...
![손으로 PID 게인을 정하는 순서](../assets/w04-tuning-order.svg)

**이 그림에서 볼 것** — 상자는 할 일, **보라색 마름모는 판정**, 오른쪽 회색 숫자는 이 플랜트에서 실제로 나온 값임 (아래 실측 표와 같은 숫자)

| 그림 안의 것 | 위 네 단계·아래 유도의 어느 줄 |
|---|---|
| 상자 1 (P 만) | 1번 — $K_p$ 만 올림. 크기는 단위에서 잡음 ($k = 2$ 이므로 $K_p = 5k = 10$) |
| 마름모 「출렁이는가?」 | 1번의 "출렁이기 시작하는 지점 직전" 을 판정하는 자리 |
| 상자 3 (D) | 2번 — 오버슈트가 줄어드는 동안만 올림 |
| 마름모 「남는 오차가 있는가?」 | 3번 — **없으면 I 를 넣지 않음.** 왼쪽 갈래가 그 길 |
| 상자 4 (I) | 3번 — 남는 오차 $0.167$ 을 지움. 대신 오버슈트가 돌아옴 |
| 마름모 「제어입력이 한계 안인가?」 | 4번 — **응답이 아니라 제어입력을 보는 자리** |
| 상자 5 (되감기·목표값 평활) | 4번 — $K_b$ 를 켬. 미분 킥은 1-11 |
| 위로 되돌아가는 보라색 화살표 | 이 절차가 **한 번에 끝나지 않는다**는 뜻 |

- 그림의 상자 2·5 가 본 절의 네 단계에는 없는 것임 — 2 는 응답에서 $\zeta$ 를 거꾸로 읽는 자리(1-5), 5 는 제어입력을 보는 자리(1-11·1-12)
- **판정이 마름모로 그려진 이유**는 그 자리에서 다음 상자가 갈리기 때문임. 게인 값은 판정 결과에서 나오지 순서표에서 나오지 않음
...
- 모델은 1-10·1-12 와 같은 `W04_P2_pid_byhand` 임 (도면 → 1-10 의 두 그림, 되감기 회로 → 1-12)

**여는 법과 실행**

```matlab
W04_setup
open_system('W04_P2_pid_byhand')     % 네 단계가 만지는 칸이 전부 PID_byhand 안에 있음
...
| 1-5 | 응답을 판정하는 다섯 숫자 | 〃 (1-4 와 같은 모델) | `('zeta')` 를 다시 읽음 | `W04_P0_zeta` | 불필요 |
...
| 2-0-1 | 실시간 화면 — 네 칸의 뜻 | 2부 배 모델 7개 공용 | `W04_setup` 의 `animate` | 아래 다섯 줄의 `W04_anim_*` | 불필요 |
...
| 2-7 | 헤딩 P | 〃 | `('Kp')` | `W04_heading_Kp_psi` · `W04_anim_heading_offline` | 불필요 |
...
| 2-12 | 속도 + 헤딩 내부 루프 | `W04_4_inner_loop_offline` | `W04_step_compare(4)` | `W04_4_offline_vs_vrx` · `W04_anim_speed_offline` | 불필요 |
| 2-13 | 샘플 주기 검산 | `W04_3_heading_offline` 의 솔버 설정 | `c2d` · `pzmap` | 그 자리에서 뜨는 `pzmap` 창 | 불필요 |
| 2-14 | ROS 2 연결 · 페이싱 · 경량 기동 | — | `run_vrx.sh` | — | 필요 |
| 2-15 · 2-16 | VRX 직진 · 선회 | `W04_1_straight` · `W04_2_turn` | 모델 창에서 Run | — | 필요 |
| 2-17 · 2-18 | VRX 헤딩 · 내부 루프, 오프라인과 대조 | `W04_3_heading` · `W04_4_inner_loop` | `W04_step_compare(3)` `(4)` | `W04_3_offline_vs_vrx` · `W04_4_offline_vs_vrx` · `W04_anim_heading_vrx` · `W04_anim_speed_vrx` | 필요 |
| 2-19 | 오프라인 WAM-V 전체 대조 | `W04_5_offline` | `W04_offline_plot` | `W04_5_offline_vs_vrx` · `W04_anim_turn_offline` | 선택 |

## 2-0-1. 실시간 화면 — 시킨 대로 따라갔는가

- 3주차 화면이 답한 질문은 하나 — "추력을 주면 배가 어디로 가는가"
- 4주차부터는 제어를 하므로 답해야 할 질문이 하나 늘어남 — **시킨 대로 따라갔는가**
- 2부의 배 모델은 돌아가는 동안 창 하나가 열리고, 그 창이 **네 칸**으로 나뉨

| 자리 | 그리는 것 | 왜 그 자리에 있는가 |
|---|---|---|
| 왼쪽 큰 칸 | 항적 + **선체 모양과 선수 방향** | 궤적선만으로는 뱃머리가 어디를 보는지 안 보임 |
| 오른쪽 ① | $u_{\text{ref}}$ (회색 점선) 와 $u$ (파랑) 를 **겹쳐서** | 지령선과 응답선이 한 칸에 있어야 정착시간이 눈으로 읽힘 |
| 오른쪽 ② | $\psi_{\text{ref}}$ 와 $\psi$ 를 **겹쳐서** (deg, `ssa` 로 접은 값) | 위와 같음 |
| 오른쪽 ③ | 왼쪽 축에 $v$ · $r$, 오른쪽 축에 $F_L$ · $F_R$ | 무엇이 그 응답을 만들었는지 |

- 가로축은 `y (동쪽)`, 세로축은 `x (북쪽)` — **위쪽이 북쪽**이며 해도와 같은 방향임 (3주차 1-5)
- ③ 이 `yyaxis` 로 축을 둘로 나눈 이유는 자릿수임. $v$ 는 0.x m/s, $F$ 는 수백 N 이라 한 축에 겹치면 $v$ 가 직선으로 보임

**어디를 보면 제어가 되고 있는가**

| 볼 곳 | 정상이면 | 아니면 |
|---|---|---|
| ② 의 두 선 | 응답선이 지령선에 **붙어서 같이 감** | 붙지 않으면 게인이 작거나 포화임 (2-7 · 2-10) |
| ③ 의 $F_L$ · $F_R$ | 정착 뒤 **같은 값으로 모임** — 요 모멘트가 0 | 계속 벌어져 있으면 아직 돌고 있는 중임 |
| ③ 의 $r$ | 오차가 줄면 함께 0 으로 내려옴 | 0 을 넘나들며 안 죽으면 $K_{d,\psi}$ 부족 (2-8) |
| 왼쪽 칸의 선체 표식 | 항적의 접선 방향과 거의 나란함 | 크게 어긋나면 옆으로 밀리고 있는 것임 ($v$ 확인) |

> [!note] 지령이 없는 모델은 「지령 없음 (개루프)」 으로 표시됨
> `W04_1_straight` · `W04_2_turn` · `W04_5_offline` 과, 속도 지령이 없는 `W04_3_heading` 계열이 해당함
> 그 칸의 제목이 「지령 없음 (개루프). 응답만 그린다」 로 바뀌고 **범례에서도 지령선이 빠짐**
> 그리지도 않는 선을 범례에 남기면 학습자가 없는 선을 찾게 되므로 아예 빼는 것임

- **오프라인 모델과 VRX 쌍둥이가 화면 함수 하나(`W04_animate.m`)를 같이 씀.** 갈아 끼우는 것은 신호를 어디서 뽑았는지뿐임 — 운동방정식이냐, 참값 오도메트리냐
- 따라서 2-7 과 2-17, 2-12 와 2-18 의 화면은 **같은 함수가 그린 같은 배치**이며 나란히 놓고 읽을 수 있음

> [!important] 모델 안에서는 포트 없는 회색 상자 `Animate` 하나임
> 신호는 `From` 태그로 받으므로 **제어 신호 사슬에 선이 한 줄도 늘지 않음**
> 상자를 열면 `W04_animate.m` 을 부르는 MATLAB Function 하나가 들어 있음
> `W04_6_wrap` 에는 붙이지 않았음 — 제자리에서 도는 각도만 보는 모델이라 항적 칸이 점 하나가 됨
...
- 모델은 2-1 과 같은 `W04_P3_boat_speed` 임 (도면 → 2-1 의 「속도 제어 모델」). `OpenLoop` 스위치만 폐루프 쪽으로 돌려 둠
- 제어기 상자 `PID_u` 는 **1-10 에서 직접 조립한 `PID_byhand` 와 같은 회로**이고 이득 이름만 `Kp_u`·`Ki_u`·`Kd_u`·`Kb_u` 로 바뀐 것임

![속도 제어기 `PID_u` 안쪽 — 1-10 의 `PID_byhand` 와 같은 회로](W04_simulink/img/W04_P3_boat_speed__PID_u.png)

**여는 법과 실행**

```matlab
W04_setup
open_system('W04_P3_boat_speed/PID_u')   % Kp_gain 의 값이 Kp_u
...
- 모델은 2-1 과 같은 `W04_P3_boat_speed` 임 (도면 → 2-1, 제어기 상자 안쪽 → 2-2 의 `PID_u` 그림). 바뀌는 것은 **`Kd_gain` 의 값 하나**뿐임

**여는 법과 실행**

```matlab
W04_setup
open_system('W04_P3_boat_speed/PID_u')   % Kd_gain 의 값이 Kd_u, PseudoD 의 분모가 Nf
...
- 모델은 2-1 과 같은 `W04_P3_boat_speed` 임 (도면 → 2-1, 제어기 상자 안쪽 → 2-2 의 `PID_u` 그림)
- **되감기 회로는 1-12 와 같은 세 블록**임 — `Sat` (한계 $X_{\max}$) · `SumAW` ($\tau_{\text{sat}} - \tau_{\text{raw}}$) · `Kb_gain` (값 `Kb_u`). 바뀌는 것은 `Kb_gain` 의 값 하나뿐임

**여는 법과 실행**

```matlab
W04_setup
open_system('W04_P3_boat_speed/PID_u')   % Sat 의 한계가 X_max, Kb_gain 의 값이 Kb_u
...
**돌아가는 동안의 화면** (2-0-1 의 네 칸)

![헤딩 제어가 도는 실시간 화면 — 오프라인](W04_simulink/img/W04_anim_heading_offline.png)

| 보는 곳 | 읽는 것 |
|---|---|
| 왼쪽 큰 칸 | 32.7° 로 출발해 북동쪽으로 휘고, 그 뒤 45° 방향의 직선으로 펴짐 |
| 오른쪽 ① | 속도 지령이 없어 **「지령 없음 (개루프)」** 임. 전진 추력 $X_{\text{head}} = 300$ N 이 만든 $u = 1.12$ m/s 가 그대로 보임 |
| 오른쪽 ② | $\psi_{\text{ref}} = 45°$ 점선에 $\psi$ 가 **5 초 안에** 올라붙고 넘어가지 않음 |
| 오른쪽 ③ | $r$ 이 먼저 서고 그것이 쌓여 $\psi$ 가 돌아감. $F_L$ · $F_R$ 은 5 초 뒤 **같은 값으로 모임** |

- $F_L$ 과 $F_R$ 이 같은 값으로 모이는 순간이 요 모멘트가 0 이 되는 순간임. 그때 선수각이 멈춤 — 위 표 「아래 칸 끝값이 0」 과 같은 말임
- 조건 : `W04_3_heading_offline`, $K_{p,\psi} = 800$ · $K_{d,\psi} = -400$ · $K_{i,\psi} = 0$, 40 s, ode4 / 0.05 s, `animate = 1` · `animate_every = 0.25`
- 캡처 시점 (40.0 s) : 출발점 기준 $x = 31.61$ m · $y = 30.70$ m · $\psi = 45.0°$ · $u = 1.12$ m/s
- **같은 화면의 VRX 판은 2-17 에 있음.** 나란히 놓고 읽을 것
...
- **이 예제만 Simulink 모델이 아님** — 요축 한 자유도를 MATLAB 으로 직접 적분하는 스크립트임. 부호만 바꾼 세 조건을 한 화면에 올리려고 모델 대신 코드를 씀
- 같은 제어법칙을 블록으로 적은 것이 2-7 의 `HeadingCtrl` 그림임. **스크립트의 세 줄이 그 상자 안의 D 갈래 하나에 해당**함

**여는 법과 실행**

```matlab
W04_setup
open_system('W04_3_heading_offline/HeadingCtrl')   % 블록으로 적은 같은 법칙 (2-7 의 도면)
edit W04_heading_sign                              % (A)(B)(C) 세 줄이 D 항의 세 가지 선택
...
| 위 칸 초록 (C) | 부호를 뒤집은 $+400r$ — 약 53° 까지 넘침 (오버슈트 17.0 %) | 5번 |
...
- 모델은 2-6 과 같은 `W04_3_heading_offline` 임 (도면 → 2-6, 제어기 상자 안쪽 → 2-7 의 `HeadingCtrl` 그림). 바뀌는 것은 **$K_{d,\psi}$ 이득 하나**뿐임

**여는 법과 실행**

```matlab
W04_setup
open_system('W04_3_heading_offline/HeadingCtrl')   % r 되먹임 갈래의 이득이 Kd_psi
...
- 2-8 예제 ① 과 같이 **Simulink 모델이 아니라 MATLAB 스크립트**임 — 복소평면의 극을 함께 그려야 해서 코드로 씀
- 여기서 정한 $K_{p,\psi}$·$K_{d,\psi}$ 가 들어가는 자리는 2-7 의 `HeadingCtrl` 상자임

**여는 법과 실행**

```matlab
W04_setup
open_system('W04_3_heading_offline/HeadingCtrl')   % 계산한 게인이 들어가는 상자 (2-7 의 도면)
...
- 모델은 2-6 과 같은 `W04_3_heading_offline` 임 (도면 → 2-6, 제어기 상자 안쪽 → 2-7 의 `HeadingCtrl` 그림)
- 약해진 추진기를 흉내 내는 자리는 제어기가 아니라 최상위 도면의 **`PortEff` 블록**임 (2-6 의 도면에서 `Alloc` 뒤). 제어기는 그 일이 벌어진 줄 모른 채 오차만 봄

**여는 법과 실행**

```matlab
W04_setup
open_system('W04_3_heading_offline')               % PortEff 가 좌현 추력을 0.7 배로 (최상위)
open_system('W04_3_heading_offline/HeadingCtrl')   % I 갈래의 이득이 Ki_psi
...
- 그 $-340°$ 가 그대로 이득으로 들어가면 배는 **정말로** 반대쪽으로 거의 한 바퀴를 돎 — 실측 $340.0°$

### 한 각도 예시를 끝까지 — $170°$ 에서 $-170°$ 로

![같은 명령, 짧은 길 20° 와 먼 길 340°](../assets/w04-ssa-circle.svg)

**이 그림에서 볼 것** — 원 **하나** 위에 두 길이 같이 있음. 원 바깥의 보라 호가 `ssa` 가 고른 $20°$, 원 안쪽의 빨간 호가 그대로 뺀 $340°$ 임

| 그림 안의 것 | 뜻 |
|---|---|
| 회색 원과 $30°$ 눈금 | 배의 나침반이 읽는 모든 방위. 북에서 시계방향으로 잼 |
| 검은 실선 화살표 | 지금 향한 곳 $\psi = 170°$ |
| 보라 파선 화살표 | 가라는 곳 $\psi_{\text{ref}} = -170°$ — 같은 방향을 $190°$ 라고도 부름 |
| 남쪽의 빨간 점선 | 이음매. $+180°$ 와 $-180°$ 가 **같은 방향인데 수만 뛰는** 자리 |
| 색칠한 얇은 쐐기 | 두 화살표 사이의 진짜 거리 $20°$ |
| 원 **바깥**의 보라 호 | `ssa` 가 내놓는 $+20°$, 우현 |
| 원 **안쪽**의 빨간 호 | 그대로 뺀 $-340°$, 좌현. 한 바퀴에서 $20°$ 모자란 길 |
| 오른쪽 두 상자 | 그 수가 이득에 무엇을 요구하는가 |

**한 단계씩 따라가기**

| 단계 | 수 | 제어기가 그 수로 하는 일 |
|---|---|---|
| 배가 유지 중 | $\psi = 170°$ | 아직 아무 일도 없음 |
| 지령이 들어옴 | $\psi_{\text{ref}} = -170°$ | $190°$ 와 **같은 방향**임. 나침반은 둘을 구별하지 못함 |
| 그대로 빼면 | $e = -170° - 170° = -340°$ | 큰 **음수** — 좌현으로 $340°$ 를 계속 돌라는 뜻 |
| `ssa` 를 거치면 | $\operatorname{ssa}(-340°) = +20°$ | 작은 **양수** — 우현으로 $20°$ 만 |
| 다 돌고 나면 | $190°$ 또는 $-170°$ | **도착 방위는 같음**. 다른 것은 가는 길뿐임 |

- $-340°$ 는 **산수로는 맞음**. 틀린 것은 그 수를 오차라고 부른 것임

### 그 $340°$ 가 각 항에 요구하는 것

- 기본값 — $K_{p,\psi} = 800$ N·m/rad, $K_{d,\psi} = -400$, $N_f = 20$, 포화 $N_{\max} = 500$ N·m
- 모델 안은 rad 이므로 $20° = 0.349$ rad, $340° = 5.934$ rad 로 바꿔 넣음

| 항 | `ssa` 를 거친 $0.349$ rad | 그대로 뺀 $5.934$ rad | 배 |
|---|---|---|---|
| P — $K_{p,\psi}e$ | $800 \times 0.349 = 279$ N·m. **한계 안** | $800 \times 5.934 = 4747$ N·m. 한계 $500$ 의 **9.5 배** | 17 |
| I — $K_{i,\psi}\int e\,dt$ 가 **1 초에** 쌓는 양 (2-10 의 $K_{i,\psi} = 50$) | $17.5$ N·m | $297$ N·m. **2 초면 적분만으로 포화에 닿음** | 17 |
| D — 오차를 미분했다면 $K_d N_f\,\Delta e$ (1-11 의 계단 미분) | $2\,793$ N·m | $47\,473$ N·m. 한계의 **95 배** | 17 |

- 세 항이 **전부 정확히 17 배**를 받음. 오차 하나를 안 접었을 뿐인데 루프 전체가 17 배를 요구함
- D 행에 「했다면」이 붙은 이유 — 본 과목의 헤딩 루프는 D 를 오차가 아니라 **$r$ 에** 검 (2-8). 그래서 지령이 계단으로 바뀌어도 미분 킥이 없음. 교과서대로 오차를 미분하는 구현이었다면 이음매에서 위의 $47\,473$ N·m 가 나왔을 것임

**배가 실제로 하는 일**

1. $4747$ N·m 요구는 포화에서 $500$ 으로 잘림. 잘려도 **부호는 그대로 좌현**임
2. 잘린 최대 모멘트로 계속 돌므로 배는 **전속 선회**가 됨 — 실측 최대 선회율 $24.95$ deg/s 는 2-6 실측표의 $N = 500$ N·m 행과 같은 값임
3. 30 초 중 **12.2 초**(41 %) 를 포화에 붙어 있음. `ssa` 를 켜면 최대 $279.3$ N·m 로 **한 번도 포화하지 않음**
4. 결과 — 우현 $20°$ 면 끝날 일을 좌현으로 **거의 한 바퀴** 돌아 같은 자리에 도착함
...
| 오차의 크기 $\lvert e\rvert$ | `ssa` 가 하는 일 | 왜 그것이 중요한가 |
|---|---|---|
| $180°$ 이하 | **아무것도 하지 않음** — $\operatorname{ssa}(e) = e$ | 평소 운항에서는 통과일 뿐임. 2-7\~2-10 에서 맞춘 게인이 그대로 유효함 |
| $180°$ 초과 | 도착 방위는 그대로 두고 **반대 방향**의 작은 각으로 바꿈 | 먼 길을 짧은 길로 바꾸는 것이 전부임 |
| 정확히 $180°$ | $+180°$ 로 둠 (범위가 오른쪽만 닫힘) | 어느 쪽으로 돌아도 거리가 같음. 한쪽을 정해 두어야 좌우로 떨리지 않음 |

- **왜 `if` 문이 아니라 $\operatorname{atan2}(\sin e,\ \cos e)$ 인가** — $360°$ 를 더하거나 빼도 $\sin e$ 와 $\cos e$ 는 **변하지 않음**. 그 한 쌍이 이미 접힌 각 자체이고 `atan2` 는 그것을 읽어 돌려줄 뿐임
- 그래서 $e = \pm540°$ 든 몇 바퀴든 **식 하나**로 끝나고, 잘못 쓸 조건 분기가 없음

![$\pm180°$ 이음매 — 그대로 빼면 먼 길이 나온다](../assets/w04-ssa.svg)

**이 그림에서 볼 것** — 왼쪽은 **같은 두 방위**를 두 가지로 재는 그림, 오른쪽은 `ssa` 라는 함수 자체의 모양임

| 그림 안의 것 | 유도의 어느 줄 |
|---|---|
| 실선 화살표 $\psi = +170°$ · 파선 화살표 $\psi_{\text{ref}} = -170°$ | 현상의 두 각. 둘 다 북에서 잰 것 |
| 회색 긴 호 | 그대로 뺀 $-340°$. **한 바퀴를 거의 다 도는 길** |
| 보라 짧은 호와 색칠한 쐐기 | $\operatorname{ssa}$ 가 돌려주는 $+20°$. 실제 거리 |
| 오른쪽 톱니의 **가운데 토막** | 3번 — $(-180°, 180°]$ 안에서는 아무 일도 하지 않음 |
| 오른쪽 톱니의 **뛰는 자리** | $\pm180°$ 의 이음매. 본 절의 제목이 가리키는 곳 |
| 보라 점 $-340 \mapsto +20$ | 5번 그 자체 |

- **접는 것은 선수각이 아니라 오차**임. $\psi$ 를 접으면 배가 어디를 향하는지 알 수 없게 됨
- 왼쪽 그림의 두 화살표는 $20°$ 밖에 안 떨어져 있음. 그런데 뺄셈은 $-340$ 을 내놓음 — 그것이 이 절의 전부임

### 어디에 놓는가 — **모든 이득 앞**

- `ssa` 는 뺄셈 **바로 뒤**, P·I·D 어느 이득보다 **앞**에 옴. 블록 순서는 한 줄로 정해져 있음

```
psi_ref ─┐
         ├─► 빼기 ─► ssa ─┬─► Kp_psi ────────┐
psi ─────┘                └─► Ki_psi ─► 적분 ─┤─► SumN ─► SatN ─► N
                                              │
r ───────────────────────────► Kd_rate ───────┘
```

| 어디에 두는가 | 그렇게 하면 | 뒤로 밀면 |
|---|---|---|
| 빼기 바로 뒤, `Kp_psi` 앞 | P 가 $0.349$ rad 를 봄 | $5.934$ rad 를 봐서 포화에 붙음 |
| **적분기 앞** | 적분이 초당 $0.349$ rad·s 를 쌓음 | 초당 $5.934$ rad·s — **하지도 않을 회전**을 저장함 |
| `Kd_rate` 와는 무관 | D 는 $r$ 에서 오므로 오차 경로에 없음 (2-8) | — |

- **적분기 앞이 특히 중요**함. P 는 오차가 줄면 저절로 줄지만, 적분은 한 번 쌓이면 **반대로 빼내야** 사라짐 (1-12 의 되감기)
- $340°$ 오차가 2 초만 적분기에 들어가도 $50 \times 5.934 \times 2 = 594$ N·m — 포화 $500$ 을 혼자서 넘김. 배가 목표에 닿은 뒤에도 그만큼을 되돌리는 동안 **지나침**
- 모델 안에서는 `HeadingErr` 상자가 「빼기 + `ssa`」를 함께 담당함. 그 출력 하나가 `Kp_psi` 와 `Ki_psi` 로 갈라짐

```matlab
function e = HeadingErr(psi_ref, psi, use_ssa)
%#codegen
d = psi_ref - psi;
if use_ssa >= 0.5
    e = atan2(sin(d), cos(d));   % ssa — 359도와 1도의 차이는 2도
else
    e = d;                       % 안 접음 — 170 deg -> -170 deg 가 -340 deg 가 됨
end
```

- 헤딩 제어기를 쓰는 모든 모델(`W04_3_heading`·`_offline`·`W04_6_wrap`)이 **같은 함수**를 씀
- `use_ssa` 는 이 절의 실험을 위한 스위치임. 기본값 1 이며 **끄는 것은 이 절에서만** 함
...
![랩 실험의 헤딩 제어기 안쪽 — `use_ssa` 가 `HeadingErr` 로 들어가고, 그 출력 하나가 모든 이득으로 갈라진다](W04_simulink/img/W04_6_wrap__HeadingCtrl.png)
...
| 아래 칸 | 요 모멘트 $N$ [N·m] | `use_ssa=0` 은 $-500$ 에 **12.2 초** 붙어 있고 `use_ssa=1` 은 279.3 에서 그냥 내려옴 |
| 도착점 | 두 경우 모두 **같은 방향**을 가리킴 | 틀린 답이 아니라 먼 길 |

**실측 표와 유도 검산** — 30 s, ode4/0.05

| 조건 | 초기 $\psi$ | 끝 $\psi$ (unwrap) | 총 회전각 | 정착 2 % [s] | 최대 $\lvert r\rvert$ [deg/s] | 최대 $\lvert N\rvert$ [N·m] | 포화에 붙어 있던 시간 |
|---|---|---|---|---|---|---|---|
| `use_ssa = 1` (최단 방향) | 170° | 190° | **$+20.0°$** | **4.1** | 8.67 | 279.3 | **0 s** |
| `use_ssa = 0` (ssa 없이 빼기) | 170° | $-170°$ | **$-340.0°$** | **14.8** | 24.95 | 500.0 | **12.2 s** (30 초의 41 %) |

- 회전각은 정확히 **17 배**. 시간은 3.6 배, 최대 선회율은 2.9 배
- `use_ssa = 1` 은 요구 모멘트 279.3 N·m 가 한계 500 안에 있어 **포화를 한 번도 건드리지 않음**. 앞의 손계산 $800 \times 0.349 = 279$ 와 같은 값임
- `use_ssa = 0` 의 최대 선회율 24.95 deg/s 는 2-6 실측표의 $N = 500$ N·m 행과 같은 값임 — **모멘트가 포화 한계에 붙어 있었다는 뜻**임

> [!important] 정리
> **각도 오차는 뺀 직후, 어느 이득보다 먼저 한 번 접음.** 적분기 뒤로 밀면 하지도 않을 회전이 쌓임
> 5주차의 유도 법칙에서도 목표 침로와 현재 선수각의 차이에 같은 함수를 씀
...
![속도 + 헤딩 제어가 도는 실시간 화면 — 오프라인](W04_simulink/img/W04_anim_speed_offline.png)

- **2부에서 지령 칸이 둘 다 차 있는 유일한 오프라인 모델임.** 두 루프가 함께 도는 모습이 한 화면에 있음

| 보는 곳 | 읽는 것 |
|---|---|
| 오른쪽 ② | $\psi$ 가 5 초에 45° 점선에 붙음 — **빠른 루프** |
| 오른쪽 ① | $u$ 가 40 초에 1.42 m/s 로 아직 1.5 점선 아래에 있음 — **느린 루프** |
| 오른쪽 ③ | $F_L$ · $F_R$ 이 5 초 뒤 겹쳐서 **함께 올라감.** 선회가 끝나고 속도 루프만 남았다는 뜻임 |
| 왼쪽 큰 칸 | 초반의 휨이 끝난 뒤 45° 직선. 선체 표식 간격이 점점 넓어짐 — 속도가 붙는 중임 |

- 두 루프의 빠르기가 다르다는 것이 이 화면의 요점이며, 위 실측 표의 정착 4.00 s 대 63 % 도달 2.50 s 가 같은 말임
- 조건 : `W04_4_inner_loop_offline`, 속도 루프 $K_{p,u} = 300$ · $K_{i,u} = 40$ · 포화 $\pm500$ N, $u_{\text{ref}} = 1.5$ m/s, $\psi_{\text{ref}} = 45°$, 40 s, ode4 / 0.05 s
- 캡처 시점 (40.0 s) : $x = 35.37$ m · $y = 34.51$ m · $\psi = 45.0°$ · $u = 1.42$ m/s
- **같은 화면의 VRX 판은 2-18 에 있음**
...
- 이 절이 다루는 값은 블록 안이 아니라 **모델 설정(Configuration Parameters)** 에 있음. 도면은 2-6 (`W04_3_heading_offline`) 과 2-12 (`W04_4_inner_loop_offline`) 의 것을 그대로 씀

**여는 법과 실행**

```matlab
W04_setup
open_system('W04_3_heading_offline')                 % 도면은 2-6 의 것
get_param('W04_3_heading_offline','FixedStep')       % 0.05 — 아래 표의 둘째 열
%  또는 Modeling 탭 → Model Settings → Solver 에서 같은 값을 눈으로 확인
```
...
**결과 — 이 절만은 저장된 그래프가 아니라 그 자리에서 뜨는 `pzmap` 창으로 판정함**

```matlab
G  = tf(1, [1 2 2]);
Gd = c2d(G, 0.05);     % Ts 를 바꿔 가며 abs(pole(Gd)) 를 봄
pzmap(Gd); axis equal  % 단위원과 극이 함께 그려짐 — 이 창이 이 절의 결과 그림
```

| 창에서 볼 것 | 정상이면 |
|---|---|
| 단위원 | 안정 경계. 이 원 안이 안정 |
| 극 두 개의 위치 | $T_s = 0.05$ 에서 원 안 깊숙이. $T_s$ 를 키우면 원 중심 쪽으로 모임 |
| 극의 크기 $\lvert z \rvert$ | 아래 표의 값과 같아야 함 |
...
> `W04_step_compare` 의 자동 측정은 잰 값을 `측정 RTF = ...` 줄에 먼저 찍고, 페이싱 비율은 1.0 을 넘지 않게 자름
> 노트북(경량 모델) 실측 — 평균 **0.976**, 표준편차 0.024, 최솟값 0.867 (`measure_vrx.sh 30`, 2026-09-25). 자동 측정은 0.955\~0.983
...
**실시간 화면을 켠 채로 재측정** (2026-09-24, 2-17·2-18 의 VRX 캡처를 뜬 세션)

| 항목 | 값 |
|---|---|
| 실측 RTF | **0.990** (1 초 구간 29개 · 표준편차 0.003 · 최솟값 0.978) |
| Gazebo 화면 | **49.5 fps** |
| 페이싱 비율 | **0.990 — 실측 RTF 를 그대로 넣음** |
| 45 초 실행의 벽시계 | 93.5 s (컴파일 포함) |

**노트북에서 같은 측정** (2026-09-25, 경량 모델, `measure_vrx.sh 30`)

| 항목 | 값 |
|---|---|
| 실측 RTF | **0.976** (1 초 구간 26개 · 표준편차 0.024 · 최솟값 0.867) |
| Gazebo 화면 | **49.5 fps** — 판정 「정상」 |
| 참값 오도메트리 | **9.87 Hz**, `RELIABLE`, 발행자 1 |
| 스폰 위치 | `x = -532.4`, `y = 162.0` — 위 설치(200)와 다름. 아래 caution 참조 |

- 화면(`animate = 1`)을 켜도 **RTF 평균은 0.990 그대로**임. 흔들림만 조금 커짐 (표준편차 0.002 → 0.003)
- 단, RTF 는 **Gazebo 쪽** 속도임. 화면을 그리는 **Simulink 쪽**은 느린 컴퓨터에서 시작 직후 몇 초 뒤처질 수 있음 → 2-17 의 warning
- 페이싱 비율을 실측 RTF 보다 **높이지 않음.** 높이면 모델과 시뮬레이터가 CPU 를 다퉈 오히려 느려짐 (2-14-3)
- 재측정이 필요하면 `bash W03_vrx_lite/measure_vrx.sh 30` 한 줄. 나온 값을 그대로 페이싱에 넣음
- 화면이 부담되면 `animate = 0` 으로 끄고 `W04_step_compare` 가 찍는 숫자로 판정함 (1-0)
...
> 실행하면 첫 줄에 `0) ROS_DOMAIN_ID = 7` 처럼 무엇을 쓰는지 찍음. WSL 의 값과 다르면 거기서 멈추고 고칠 것
> 이 값은 `W04_setup` 이 아니라 **ROS 네트워크 프로필**에서 읽음 — `W04_setup` 의 `7` 과 프로필이 다르면 프로필이 이김
...
| 노트북 재측정 (RTF 0.976 · 페이싱 0.976 · 2026-09-25) | **1.332 m/s** (표준편차 0.0004), $v$ 0.002, $\omega_z$ $-0.001$ | 데스크톱과 같음 |
...
| `W04_3_heading` (이 절) | `OdomSub` → `Sel` → `Quat2Yaw` (Gazebo) | `GateL/R` → Blank → Bus Assignment → Publish |

- **`HeadingCtrl` 과 `Alloc` 은 같은 함수로 만들어짐.** 제어 로직은 한 글자도 다르지 않음
- VRX 쪽에만 있는 것은 **`GateL` · `GateR`** 두 곱셈 블록뿐임 — `Quat2Yaw` 의 셋째 출력 `ok` 를 추력에 곱함

| `ok` | 언제 | 나가는 추력 |
|---|---|---|
| 0 | 첫 오도메트리가 오기 전 + 그 뒤 **1 초** (20 표본) | 0 — 배는 스폰 자세 그대로 |
| 1 | 그 이후 | `Alloc` 이 낸 값 그대로 |

- 막는 이유 ① — 첫 메시지 전의 Subscribe 는 0 으로 채운 버스를 냄. 쿼터니언 0 은 선수각 **정확히 90°** 로 읽혀 가짜 오차 $-45°$ 가 생김
- 막는 이유 ② — Simulink 는 시작 직후 몇 스텝을 벽시계로 수 초씩 멈춤. 그동안 Gazebo 는 첫 추력을 계속 줌
- 막지 않았을 때 같은 설정의 오버슈트가 실행마다 **3.7 · 54 · 347 %** 로 찍힘 (2026-09-25 노트북 실측). 게이트를 넣은 뒤 3.0 %
- `W04_step_compare` 는 **추력이 처음 나간 순간**을 계단의 시작 $t = 0$ 으로 잡음. 그래서 VRX 곡선도 32.7° 에서 출발함

### 유도 — 좌표 변환 두 줄 (2-14-2 의 표를 코드로)

```matlab
function [psi_ned, r, ok] = Quat2Yaw(qx, qy, qz, qw, wz)
...
persistent n                                 % 유효한 메시지를 받은 횟수
if isempty(n), n = 0; end
valid = qx ~= 0 || qy ~= 0 || qz ~= 0 || qw ~= 0;
if valid, n = n + 1; end
ok = double(valid && n > 20);                % 첫 메시지 뒤 1 초(20 표본)부터 1
```
...
5. 쿼터니언 네 성분이 전부 0 이면 아직 메시지가 없는 것임 — 유효한 메시지를 20 번 받기 전까지 `ok = 0`
...
- 정상 출력 (**경량 모델** 노트북 실측, RTF 0.966, 2026-09-25)
...
2) VRX W04_3_heading — RTF 측정 (10초)
   측정 RTF = 0.966  ->  페이싱 0.966
   RTF = 0.966  ->  페이싱 비율
   측정 중 실시간 화면 끔 (animate = 0)
   [VRX] 초기 32.7 deg -> 45 deg (계단 12.3 deg), 오버슈트 3.0 %, 2% 정착 6.2 s, 마지막 5 s 평균 45.03 deg
   그림 저장: img/W04_3_offline_vs_vrx.png
```

- `측정 RTF` 는 컴퓨터마다 다름. 데스크톱은 1.0 을 넘게 재져 `1.000` 으로 잘려 찍힘 (2-14-3)
- 벽시계 약 90 초 — 오프라인 1 초 + RTF 측정 10 초 + 모델 준비 + VRX 40 초

> [!warning] 측정 중에는 실시간 화면을 끈다
> `W04_step_compare` 는 VRX 구간만 `animate = 0` 으로 돌림. `W04_setup` 의 값은 그대로임
> - 이유: 화면을 그리느라 Simulink 가 시작 직후 VRX 를 못 따라가면, 그동안 배는 앞 추력대로 돌고 계단응답이 **두 배 빨리** 오른 것처럼 찍힘
> - 노트북 실측 (2026-09-25, 같은 모델): 화면 켬 — 오버슈트 3.5 %, 정착 3.5 s / 화면 끔 — 2.9 %, 6.2 s
> - 화면은 모델을 직접 열어 Run 할 때 보면 됨 (아래 「돌아가는 동안의 화면」)
...
| 검은 점선 | 목표 45° | — |
| **$t \approx 5$ s 근처의 최고점** | 빨강만 점선을 살짝 넘음 | 오버슈트 3.0 % 대 0.6 % |
...
| 항목 | 오프라인 쌍둥이 | VRX (경량 · 노트북 RTF 0.966) | 읽는 법 |
|---|---|---|---|
| 초기 $\psi$ | 32.700° | 32.697° | 스폰 자세가 같음 |
| 계단 | 12.300° | 12.303° | 같은 조건 |
| 오버슈트 | **0.607 %** | **2.963 %** | VRX 가 조금 더 넘침 |
| 2 % 정착 | **3.95 s** | **6.15 s** | 넘친 만큼 되돌아오는 시간 |
| 마지막 5 초 평균 | 45.000° | 45.031° | 정상상태 오차 $+0.031°$ |

- 2026-09-25 노트북 실측. 데스크톱(RTF 0.990, 2026-09-24)에서는 3.027 % · 6.00 s · 45.029° — 두 컴퓨터가 소수 첫째 자리까지 같음
...
**돌아가는 동안의 화면 (VRX)** — 2-7 의 오프라인 화면과 나란히 읽음

![헤딩 제어가 도는 실시간 화면 — VRX](W04_simulink/img/W04_anim_heading_vrx.png)

- **2-7 과 같은 함수(`W04_animate.m`)가 그린 같은 배치임.** 갈아 끼운 것은 신호를 어디서 뽑았는지뿐임 — 운동방정식 대신 참값 오도메트리
- 칸의 뜻과 볼 곳은 2-0-1 과 같음. 아래는 **두 화면이 같고 다른 지점**만 적은 것임

| 보는 곳 | 오프라인 (2-7) | VRX (이 절) | 판정 |
|---|---|---|---|
| ① 정착 뒤 $u$ | 1.12 m/s | **1.12 m/s** | 소수 둘째 자리까지 같음 |
| ② 정착 뒤 $\psi$ | 45.0° | 45.0° | 같음 |
| ② 올라가는 모양 | 넘지 않음 (오버슈트 0.6 %) | 점선을 **살짝 넘었다가** 내려앉음 (3.0 %) | 위 실측 표와 같은 차이 |
| 왼쪽 큰 칸 초반의 휨 | 32.7° 에서 출발 | **출발 선수각이 다름** (이 캡처 세션은 약 10°) | 휘는 각이 다름 |
| 왼쪽 큰 칸 정착 뒤 | 45° 직선 | 45° 직선 | 같음 |

- 다른 것은 **출발 선수각뿐임.** 오프라인은 32.7° 고정, VRX 는 띄운 세션마다 조금씩 다름 — 초반 곡선의 휨만 달라지고 정착 뒤는 같음
- 이 화면은 위 실측 표와 **다른 세션**의 기록임. 표는 스폰 자세(32.7°)에서 시작한 `W04_step_compare(3)` 의 값이고, 화면은 다시 띄운 VRX 에서 45 초를 돌린 것임
- 조건 : `W04_3_heading`, 경량 VRX (`run_vrx.sh`), 실측 RTF **0.990**, 페이싱 0.990, 45 s (벽시계 93.5 s), `animate = 1`
- 캡처 시점 (45.0 s) : 첫 샘플 기준 $x = 43.26$ m · $y = 40.41$ m · $\psi = 45.0°$ · $u = 1.12$ m/s

> [!note] VRX 항적은 첫 샘플을 출발점으로 잡아 그림
> 참값 오도메트리는 Gazebo 월드 원점 기준이라 값이 수백 m 임 (2-14-4 의 스폰 위치)
> `W04_animate` 는 **0 이 아닌 첫 샘플**을 원점으로 삼아 상대좌표로 그림
> Subscribe 블록이 첫 메시지 전에 0 을 채운 버스를 내므로 0 을 건너뛰는 것임
...
- 속도 루프를 함께 켜도 헤딩 지표는 2-17 과 같음 (2.8 % · 6.4 s)
- 속도 쪽은 **오프라인과 VRX 가 한 선으로 겹침** (63 % 도달 2.50 s 대 2.45 s)
...
- 정상 출력 (**경량 모델** 노트북 실측, RTF 0.963, 2026-09-25, VRX 부분)

```
2) VRX W04_4_inner_loop — RTF 측정 (10초)
   측정 RTF = 0.963  ->  페이싱 0.963
   RTF = 0.963  ->  페이싱 비율
   측정 중 실시간 화면 끔 (animate = 0)
   [VRX] 초기 32.7 deg -> 45 deg (계단 12.3 deg), 오버슈트 2.8 %, 2% 정착 6.4 s, 마지막 5 s 평균 45.05 deg
   [VRX] u 63 % 도달 2.5 s, 마지막 5 s 평균 u 1.409 m/s
```

- 이 모델은 게이트가 하나 더 있음 — **`GateE`** 가 예열 1 초 동안 속도 오차도 0 으로 묶음
- 묶지 않으면 `PI_u` 의 적분기가 추력이 막힌 동안 오차 1.5 m/s 를 쌓아, 게이트가 열리는 순간 포화 500 N 에서 출발함 → 63 % 도달 **1.4 s** (2026-09-25 실측). 묶은 뒤 2.45 s
...
| 위 칸 $t \approx 5$ s | 빨강만 살짝 넘음 | 오버슈트 2.8 % 대 0.6 % |
| **아래 칸이 이 절에서 새로 보는 것** | 선속도 $u$ [m/s] | — |
| 아래 칸 두 곡선 | **한 선으로 겹침** | 전진 방향은 항력 계수가 같음 |
| 아래 칸 검은 점선 | 목표 1.5 m/s | — |
| 아래 칸 곡선이 **40 s 에도 점선 아래** | 1.41 m/s | $K_{i,u} = 40$ 이 작아 느림 (2-12) |
| 아래 칸 초반의 **꺾임** | $t \approx 1.5$ s 에 기울기가 꺾임 | 포화가 풀리는 지점 |

- 위 칸은 갈라지고 아래 칸은 겹침 — **요 방향에만 모델에 없는 것이 있다**는 뜻임

**실측 표와 검산**

| 항목 | 오프라인 쌍둥이 | VRX (경량 · 노트북 RTF 0.963) | 읽는 법 |
|---|---|---|---|
| 헤딩 오버슈트 | **0.606 %** | **2.847 %** | 2-17 과 같음 — 속도 루프가 헤딩에 영향을 주지 않음 |
| 헤딩 2 % 정착 | **4.00 s** | **6.40 s** | 〃 |
| 마지막 5 초 평균 $\psi$ | 45.000° | 45.048° | 정상상태 오차 $+0.048°$ |
| $u$ 가 1.5 의 63 % 에 닿는 시각 | **2.50 s** | **2.45 s** | 전진 방향은 항력 계수가 같아 딱 맞음 |
| 마지막 5 초 평균 $u$ | **1.4127 m/s** | **1.4094 m/s** | 아직 1.5 에 못 닿음. VRX 는 예열 1 초만큼 짧게 돌아 조금 낮음 |

- 2026-09-25 노트북 실측. 데스크톱(RTF 0.990, 2026-09-24, 게이트 넣기 전)에서는 2.887 % · 5.95 s · 45.045° · 2.45 s · 1.4132 m/s

- 모델 창에서 직접 돌릴 때: 페이싱 켜기 → `u_ref` = `1.5`, `psi_ref_deg` = `45` 로 Run → `Scope_u` · `Scope_psi` → 30 s 쯤 뒤 **정지**

**돌아가는 동안의 화면 (VRX)** — 2-12 의 오프라인 화면과 나란히 읽음

![속도 + 헤딩 제어가 도는 실시간 화면 — VRX](W04_simulink/img/W04_anim_speed_vrx.png)

- 이 모델의 화면만 **지령 칸이 둘 다 차 있음.** ① 에 $u_{\text{ref}} = 1.5$ m/s, ② 에 $\psi_{\text{ref}} = 45°$ 점선이 함께 보임

| 보는 곳 | 오프라인 (2-12) | VRX (이 절) | 판정 |
|---|---|---|---|
| ① $u$ | 1.42 m/s @ 40 s | **1.44 m/s @ 45 s** | **같은 곡선 위의 5 초 뒤**임. 둘 다 아직 올라가는 중 |
| ② $\psi$ | 점선을 넘지 않음 | 47° 부근까지 **살짝 넘었다가** 45° 로 내려앉음 | 오프라인에 없던 오버슈트 |
| ③ $F_L$ · $F_R$ | 5 초 뒤 겹쳐서 함께 올라감 | 같음 — 둘 다 천천히 올라감 | **속도 루프가 아직 일하는 중** |

- VRX 쪽 오버슈트는 속도 루프가 만든 전진 추력이 **선회 여유를 줄였기** 때문임 (2-5 「선회 여유는 전진 추력이 결정한다」 · 2-12 의 편당 약 6 N)
- ③ 의 두 추력선이 계속 올라간다는 것이 위 실측 표의 「40 초 뒤 $u$ = 1.413 m/s, 목표 1.5 에 못 닿음」 과 같은 말임
- 조건 : `W04_4_inner_loop`, 경량 VRX, 실측 RTF **0.990**, 페이싱 0.990, 45 s (벽시계 93.5 s), `animate = 1`
- 캡처 시점 (45.0 s) : 첫 샘플 기준 $x = 48.82$ m · $y = 47.86$ m · $\psi = 45.1°$ · $u = 1.44$ m/s
- 두 VRX 캡처(2-17 · 이 절) **사이에 VRX 를 완전히 종료하고 다시 띄웠음.** 위 「VRX 를 새로 띄우지 않으면 숫자가 크게 흔들림」 과 같은 이유임
...
- 노트북 재측정 (2026-09-25) — $u$ 1.3318, 우 $+15.7388$ / 좌 $-15.7463$ deg/s, 반경 4.1279 / 4.1231 m. 위 표와 소수 둘째 자리까지 같음
...
**돌아가는 동안의 화면** — 제어기가 없는 모델의 화면

![직진 → 우선회 → 좌선회가 도는 실시간 화면](W04_simulink/img/W04_anim_turn_offline.png)

- **두 지령 칸이 모두 「지령 없음 (개루프)」 임.** 이 모델은 제어기 없이 추력을 직접 주므로 비교할 지령선이 없음 (2-0-1)
- 개루프 모델의 화면은 「따라갔는가」 가 아니라 「무엇이 나왔는가」 만 보여 줌 — 응답선 하나씩만 그려짐

| 보는 곳 | 읽는 것 |
|---|---|
| 오른쪽 ③ | 20 초 · 40 초의 **계단 두 개**가 좌우 추력을 갈라 놓음 |
| 왼쪽 큰 칸 | 그 계단 두 개가 원 두 개를 만들어 **8 자** 항적이 됨 |
| 오른쪽 ② | $\psi$ 가 $\pm180°$ 에서 **세로로 튐** — 고장이 아니라 `ssa` 로 접은 결과임 (2-11) |
| 오른쪽 ① | 선회 구간에서 $u$ 가 패임 — 위 표의 「전진 속도 칸의 패임」 과 같은 곳 |

- 조건 : `W04_5_offline`, 80 s, ode4 / 0.05 s, `animate = 1` · `animate_every = 0.25`
- 캡처 시점 (80.0 s) : $x = 17.53$ m · $y = 6.48$ m · $\psi = -0.8°$ · $u = 0.00$ m/s — 60 초에 추력을 끊어 **멈춘 뒤**임
...
| VRX 헤딩 곡선이 오프라인보다 **두 배 빨리** 오르거나 첫 순간 수 도 튐 | 화면을 켠 채 재서 Simulink 가 시작 직후 뒤처짐 | 최신 `W04_step_compare` 로 다시 잼 (VRX 구간은 화면을 끔, 2-17) |
| VRX 를 띄웠는데 `W04_step_compare` 가 「오프라인만 돌렸다」 | ROS 네트워크 프로필의 Domain ID 와 WSL 의 `ROS_DOMAIN_ID` 가 다름 | 첫 줄 `0) ROS_DOMAIN_ID = ...` 와 WSL `echo $ROS_DOMAIN_ID` 를 맞춤 (2-14-5) |
...
| `W04_animate.m` · `W04_anim_capture.m` | 실시간 화면 (오프라인·VRX 공용) · 그 화면의 PNG 재현 | 2-0-1 |
...
| `img/` | 본문에 실린 **블록도와 결과 그래프 57장 전부** (실시간 화면 캡처 5장 포함). 스윕 명령이 돌 때마다 덮어씀 | 전 절 |
