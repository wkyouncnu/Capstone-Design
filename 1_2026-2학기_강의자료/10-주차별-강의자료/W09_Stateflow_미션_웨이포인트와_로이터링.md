...
- 오프라인 열은 `W09_vrx_run` 이 **VRX 의 초기 선수각(32.93°)으로 다시 돌린** 값임
...
- 전이 시각이 **1 s 안**, 임무 종료가 2.1 s 안에서 맞음. 미션 모델은 이 시각으로 판정함
...
> - 이 정도 편차는 실제 배에서도 생김. 상태기계가 **시각이 아니라 조건으로** 전이하도록 짜야 하는 이유
