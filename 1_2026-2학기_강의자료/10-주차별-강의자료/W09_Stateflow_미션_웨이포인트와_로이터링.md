...
> 임무 종료가 **약 317초**이므로 300초로는 마지막 웨이포인트에 닿기 전에 끝난다.
> 결과에 `임무 종료  미완료` 가 찍힌다. **`W09_vrx_run(340)`** 처럼 여유를 두고 부른다.
>
> 느린 컴퓨터에서는 340초로도 모자란다. 기준 노트북(RTF 0.69)에서는 로이터 진입이 **84.7 s**,
> 종료가 **327.7 s** 로 밀려 340초 안에 임무가 끝나지 않았다 (2026-09-15 실측).
> **페이싱 비율이 낮을수록 임무 시각이 뒤로 밀린다.** 결과에 `미완료` 가 뜨면 시간을 늘려 다시 부른다.

> [!important] VRX 를 새로 띄운 뒤 시작한다
> 앞 실험이 끝난 자리에서 이어 돌리면 첫 웨이포인트까지의 거리가 달라져 시각 비교가 무의미해진다.
...
- 다른 노트북 재측정 (2026-09-15, RTF 0.839, 420초)

```
  로이터 진입              71.0 s
  로이터 종료             248.4 s   (2.00 바퀴)
  임무 종료                미완료
  WP 구간 평균 |y_e|      2.405 m
  로이터 평균 반경오차    1.187 m
```

| 읽는 법 | |
|---|---|
| 로이터 반경오차 1.19 \~ 1.21 m | 두 환경이 같다. **상태기계와 유도는 컴퓨터 성능과 무관** |
| 회전수 2.00 바퀴 | 종료 조건이 정확히 동작 |
| 임무 종료 `미완료` | 페이싱이 낮아 마지막 구간이 420초 밖으로 밀린 것. 시간을 더 주면 끝난다 |

> [!warning] 같은 자료를 RTF 0.69 에서 돌리면 시각이 크게 밀린다
> 로이터 진입이 **84.7 s**, 종료가 **327.7 s** 로 나왔다 (반경오차는 1.206 m 로 동일).
> **시각 비교는 페이싱이 실측 RTF 와 맞았을 때만 의미가 있다.**
...
- 다른 노트북 재측정 (2026-09-15, 페이싱 0.839, 420초)

| 항목 | 오프라인 | VRX | 차이 |
|---|---|---|---|
| 로이터 진입 [s] | 70.8 | 71.0 | +0.2 |
| 로이터 종료 [s] | 244.9 | 248.4 | +3.5 |
| 회전수 | 2.00 | 2.00 | 0 |
| 궤적 평균 이격 [m] | | | 8.17 |
| 궤적 최대 이격 [m] | | | 21.20 |

- **전이 시각 차이가 0.2 s · 3.5 s** 로, 기준 환경(2.2 s · 4.5 s)과 같은 수준이다
- 이격이 8 m 로 커진 것은 페이싱 지연이 쌓인 결과다. 미션 단위 모델은 **이격보다 전이 시각**으로 판정한다
