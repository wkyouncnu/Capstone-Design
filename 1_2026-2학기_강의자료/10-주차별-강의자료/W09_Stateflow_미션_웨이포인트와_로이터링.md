...
- 두 도착 신호는 `WpManager` 가 계산함. 기호는 아래와 같음
  ...
  - $d_{k+1}$ — 배 $(p_n, p_e)$ 에서 구간 끝 WP $(N_{k+1}, E_{k+1})$ 까지의 거리

$$
d_{k+1} = \sqrt{(N_{k+1}-p_n)^2 + (E_{k+1}-p_e)^2}
$$
...
>
> - **조류가 없다.** 조류가 없으면 두 판정의 결과가 완전히 같다 (7주차 1-8절 표, $R = 5$ m · 조류 0 의 두 줄)
> - **로이터 진입은 "그 점에 왔다" 는 뜻이다.** 경로 방향 판정은 옆으로 멀리 떨어져 있어도 구간 끝에 닿았다고 본다. 원을 그리며 돌 중심에 다가왔는지를 묻는 데는 거리가 더 맞는 질문이다
>
> 조류를 넣는 과제로 확장할 때
...
- 무장 허용폭을 $0.3\,r_d$ 로 넓게 둔 이유: 정착 반경 오차가 $p_c v \tau$ 만큼 남음 (8주차 1-8절). $p_c = 1$ 이면 약 7 m 라 좁은 폭으로는 **영영 무장하지 못함** (8주차 C-1 표)
...
- 로이터 반경이 18 m 이면, 로이터를 끝낸 배는 로이터 중심(= WP3)에서 **약 18 m 떨어진 원 위**에 있다
- 다음 구간 WP3 → WP4 는 그 중심에서 시작한다 → 배는 **경로 밖에서** 출발
...
| 8주차 중심 `center_north`·`center_east` | `loiter_cn`·`loiter_ce` | 지정한 웨이포인트 좌표로 자동 설정 |
...
| 읽는 법 | 뜻 |
|---|---|
| 진입 74.7 s → 종료 248.0 s | 로이터에 **173.3 초** 머물렀다. 앞부분은 중심에서 원으로 나가는 구간이라 회전수를 세지 않음 (1-4절) |
| (2.00 바퀴) | 정확히 2바퀴에서 빠져나왔다. 어긋나면 회전수 세기를 의심한다 |
| 임무 종료 314.4 s | 로이터 뒤 남은 웨이포인트를 마저 돌았다는 뜻 |
| 반경오차 1.282 m | 크랩 보상 켬(`crab_comp = 1`) → 8주차 1-8절 예측 $p_c\,v\,\tau = 0.313 \times 1.5 \times 2.8 = 1.31$ m 와 일치. 8주차 2.136 m 는 보상 끔 |
...
> - 0.313 을 그대로 두면 $\zeta_c = \tfrac{1}{2}\sqrt{p_c\,r_d/(v\,\tau_r)} \approx 0.85$ 로 1 보다 작아져 원에 붙는 과정이 조금 덜 감쇠됨. 대신 반경오차 $p_c\,v\,\tau$ 는 작게 유지됨
...
>
> - `WpManager`·`LoiterVF`·`TurnCount` 는 **`Guidance` 상자 안**에 있다
> - `MissionFSM`·`ModeSwitch`·`ModeDly` 는 **`Mission` 상자 안**에 있다
> - 웨이포인트·로이터 중심·요구 회전수 같은 **설정값도 각 상자 안**에 있다
> - 유도는 두 지령을 **둘 다** 내놓고, 고르는 일은 미션 상자만 한다.
>   그래서 유도(파랑)와 판단(보라)이 색으로도 갈린다
...
> - 모델을 고치기 전에 사본을 만들어 두고 (B-2 절 1단계), 과제 작업은 사본에서 할 것
...
> - 안전 수역: $N \in [-28, 60]$ m, $E \in [-18, 100]$ m
> - 원이 차지하는 범위: 중심 $(N_c, E_c)$, 반경 $r_d$ 이면 $N \in [N_c - r_d,\, N_c + r_d]$, $E \in [E_c - r_d,\, E_c + r_d]$
>
> | `loiter_wp` | 중심 (N, E) | 반경 18 m 일 때 | 판정 |
> |---|---|---|---|
> | 2 | (50, 20) | $N$ 최대 68 | 벗어남 → 반경 8 이하 |
> | **3 (기본)** | (20, 55) | $N \in [2, 38]$, $E \in [37, 73]$ | 안전 |
> | 4 | (50, 90) | $N$ 최대 68, $E$ 최대 108 | 벗어남 → 반경 8 이하 |
...
| 2 | `2) RTF 측정 (20초)` · `스폰 위치 (N, E) = ...` | 실측 RTF 로 페이싱을 정하고, **배가 지금 있는 자리를 원점으로** 삼음 |
| 3 | `3) W09_1_vrx 실행 (420초)` | 실시간 그림. 상태가 바뀌면 배 색이 바뀜 |
| 4 | `===== VRX 미션 ... =====` | VRX 지표 5줄, 그림 `img/W09_1_vrx_result.png` |
| 5 | `5) 같은 초기 선수각으로 오프라인 모델 실행` | 오프라인 지표 5줄, 대조 그림 `img/W09_vrx_vs_offline.png` |

> [!warning] 인자 없이 부르면 **300초** 에서 끊긴다
> - 임무 종료가 약 313 s 라 300 s 로는 마지막 웨이포인트에 닿기 전에 끝남 → `임무 종료  미완료`
> - **`W09_vrx_run(420)`** 처럼 여유를 두고 부름
> - 전이 시각은 **시뮬레이션 시각**이다. 페이싱이 실측 RTF 와 맞으면 느린 컴퓨터에서도 같은 시각이 나옴. 벽시계로는 더 오래 걸릴 뿐
...
> - `competition.launch.py` 의 스폰 좌표가 같은 VRX 2.4.0-2 인데도 컴퓨터마다 달랐음: ENU $y$ = **162** 인 설치와 **200** 인 설치
...
>   - 출발점이 $(N, E) = (38, 0)$ 으로 찍히고, WP4 → WP5 구간이 안전 수역 $N \ge -28$ 밖으로 나가 육지에 걸려 **멈춤** ($N \approx 1$ 에서 정지, 추력 한계)
...
- 기준 환경 실측값 (2026-09-18, 데스크톱, 측정 RTF 0.683, 420 s, 스폰 ENU $(-532, 200)$)

```
===== VRX 미션 — WP -> 로이터 -> WP =====
  로이터 진입              70.2 s
  로이터 종료             244.4 s   (2.00 바퀴)
  임무 종료               312.7 s
  WP 구간 평균 |y_e|      3.046 m
  로이터 평균 반경오차    1.211 m
```
...
| 로이터 반경오차 1.211 m | 오프라인 1.282 m 와 0.07 m 차이. **상태기계와 유도는 시뮬레이터가 바뀌어도 같게 동작** |
| WP 구간 $\lvert y_e \rvert$ 3.046 m | 오프라인 2.955 m 보다 0.09 m 큼. 대부분이 로이터 직후 복귀 구간 |

### D-2. 오프라인과 비교

![오프라인 vs VRX](W09_simulink/img/W09_vrx_vs_offline.png)

- 오프라인 열은 `W09_vrx_run` 이 **VRX 의 초기 선수각(32.93°)으로 다시 돌린** 값이다
  - 그래서 `W09_setup` 기본 실행(초기 선수각 135°, 진입 74.7 s)과 다름
  - `W09_vrx_run` 은 끝나면 `x0` 를 원래대로 되돌린다. 뒤이어 오프라인을 돌려도 기본 조건 그대로

| 항목 | 오프라인 | VRX | 차이 |
|---|---|---|---|
| 로이터 진입 [s] | 70.8 | 70.2 | −0.6 |
| 로이터 종료 [s] | 244.1 | 244.4 | +0.3 |
| 회전수 | 2.00 | 2.00 | 0 |
| 임무 종료 [s] | 310.6 | 312.7 | +2.1 |
| 로이터 평균 반경오차 [m] | 1.282 | 1.211 | −0.071 |
| **궤적 평균 이격** [m] | | | **0.83** |
| **궤적 최대 이격** [m] | | | **3.13** |

- 전이 시각이 **1 s 안**, 임무 종료가 2.1 s 안에서 맞음. 미션 모델은 이 시각으로 판정한다
- 이격은 두 번 솟음 (약 200 s 에 2.5 m, 310 s 에 3.1 m). 둘 다 **로이터를 빠져나오는 순간**과 **마지막 선회** — 같은 길을 조금 다른 시각에 지나는 구간
  - 임무가 끝난 뒤 이격이 0.2\~0.3 m 로 내려감. 두 배가 같은 마지막 웨이포인트에 서기 때문
  - 궤적 그림에서 두 선이 거의 겹친다 → **위상 오차**이지 경로 오차가 아님 → [[궤적이-겹치면-이격은-오차가-아니라-지연이다]]

> [!note] 전이 시각이 1\~2 s 흔들리는 이유
> - 오프라인은 VRX 초기 선수각으로 다시 돌리므로 **출발 조건 차이는 아님**
> - 남는 것은 두 모델의 차이 — VRX 는 odometry 가 약 10 Hz 로 들어오고(제어는 20 Hz), 부력·파랑 없는 수면이라도 선체 롤·피치가 있음
> - 이 정도 편차는 실제 배에서도 생긴다. 상태기계가 **시각이 아니라 조건으로** 전이하도록 짜야 하는 이유다

### 확인할 것

- [ ] 2단계 출력의 `스폰 위치` 가 launch 파일의 스폰 좌표와 같은가
- [ ] 상태 전이가 오프라인과 같은 순서로 일어나는가
- [ ] 전이 시각이 비슷한가 (1\~2 s 차이는 정상)
- [ ] 로이터 반경 오차가 비슷한가
- [ ] 마지막에 두 궤적이 다시 만나는가
...
- [ ] 사본 `W09_my` 에 차트를 직접 그려 74.7 s · 248.0 s · 314.4 s 를 다시 얻었다
