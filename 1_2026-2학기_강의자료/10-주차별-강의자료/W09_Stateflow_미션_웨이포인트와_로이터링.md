...
> [!note] 왜 `> 0.5` 인가
> 도착 신호 `at_loiter`, `at_end` 는 MATLAB Function 이 내는 **0 또는 1 의 double** 이다.
> 실수를 `== 1` 로 비교하면 계산 오차에 걸릴 수 있어서, 0 과 1 의 한가운데인 0.5 로 가른다.
> 6_0주차 M-4 절(SB13)에서 같은 관용구를 작은 모델로 먼저 연습한다.
...
> 7주차 1-8절에서 수락 원 판정은 **조류에 밀리면 걸릴 수 있다**고 했다. 이 주차는 두 가지 이유로 원을 그대로 쓴다.
>
...
>
> 조류를 넣는 과제로 확장한다면 `WpManager` 안의 `reached = d_next <= R;` 한 줄을 7주차 `GuidanceLaw` 처럼 경로 방향 판정 `(d - x_e) < R` 로 바꾸거나, $R$ 을 넉넉히 두어야 한다.
