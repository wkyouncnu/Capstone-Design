...
% BUILD_W04_MODELS  4주차 토픽 조사 Simulink 모델 3개를 생성한다.
%
%     W04_0_offline.slx        0단계 — 운동모델에 GPS · IMU 센서 모델을 붙여 같은 주기를 잰다 (VRX 불필요)
...
%   Command     : 좌·우 추력 상수 (W04_setup 의 thrust_left, thrust_right)
...
'psi = atan2(sin(s(6)), cos(s(6)));']);
    ...
    % 참값은 센서 모델 결과와 비교하려고 태그로 남긴다 (W04_offline_run)
    for k = 1:numel(outs)
...
'%#codegen'                                                                         newline ...
'% GPS 와 IMU 를 VRX 처럼 흉내 낸다 (4주차 1-5 절).'                                 newline ...
...
'lat = lat_h;  lon = lon_h;  qz = qz_h;  qw = qw_h;  wz = wz_h;']);
    ...
    note(m,'n1', ['W04 0단계  —  운동모델에 GPS · IMU 를 붙여 수신 주기를 먼저 잰다 (VRX 불필요)' newline ...
        ...
        '실행: W04_setup -> W04_offline_run'], 250, -60);
