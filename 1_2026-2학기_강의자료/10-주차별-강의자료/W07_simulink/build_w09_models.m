...
%
%   포트   x_n, y_n, beta, mode  ->  psi_los, psi_lo, at_loiter, at_end, turns
%
...
'         x_n, y_n, wp_x, wp_y, Delta, R, loiter_wp, mode, idx_prev)'         newline ...
'%#codegen'                                                                 newline ...
...
'end']);
    ...
    setFcn(ss, 'LoiterVF', [ ...
'function [psi_lo, r, theta] = LoiterVF(x_n, y_n, x_c, y_c, rd, vd, pc, beta, kcrab)' newline ...
'%#codegen'                                                                newline ...
...
'turns = abs(acc_out) / (2*pi);']);
    ...
    % 입력은 상자 안에서 **태그로 나눠 쓴다.** x_n·y_n·mode 를 두 함수가 함께 쓰는데,
    % 포트에서 선을 두 갈래로 끌면 그 선이 설정 상수 열을 가로지른다.
    ipt = {'x_n','y_n','beta','mode'};
    for k = 1:numel(ipt)
    ...
    cfg = {'WPN','wp_north'; 'WPE','wp_east'; 'Dlt','Delta'; 'Rsw','R_LOS'; ...
           'LWP','loiter_wp'; 'LCN','loiter_xc'; 'LCE','loiter_yc'; ...
    ...
    fr = {'x_n_1','i_x_n'; 'y_n_1','i_y_n'; 'mode_1','i_mode'; ...
          'x_n_2','i_x_n'; 'y_n_2','i_y_n'; 'beta_1','i_beta'; 'mode_2','i_mode'};
    for k = 1:size(fr,1)
    ...
    % --- 배선 -----------------------------------------------------------
    wp = {'Fr_x_n_1','Fr_y_n_1','WPN','WPE','Dlt','Rsw','LWP','Fr_mode_1'};
    for k = 1:numel(wp)
        add_line(ss, [wp{k} '/1'], sprintf('WpManager/%d',k), 'autorouting','on');
    end
    lv = {'Fr_x_n_2','Fr_y_n_2','LCN','LCE','LRD','URf','Pc','Fr_beta_1','Kcr'};
    for k = 1:numel(lv)
...
%   "입력 인수가 부족합니다" 로 멈춘다 (2026-09-17 확인).
    add_animate_box(m, {'x_n','y_n','psi','mode','turns'}, 'W09_animate', '', ...
                    [x y], 'Ts_ctrl');
end
...
% 로깅 — 포트 없는 서브시스템 하나로 묶는다. W09_plot.m 이 이 변수들을 읽는다.
    sig = {'x_n','y_n','psi','psi_ref','y_e','idx','mode','turns','gate','u','beta','r_lo','FL','FR'};
