...
>
> **$N$ 도 두 가지로 쓰임.** 이 표의 $N$ 은 요 **모멘트**이고, NED 의 $N$ 은 **북쪽 위치**임
> 본 과목은 둘 다 관례대로 $N$ 으로 씀. 식을 보면 구분됨 — 위치와 함께 나오면
> ($N - N_k$, $\operatorname{atan2}(E, N)$) 북쪽, 힘과 함께 나오면 ($X,\ Y,\ N$, $F_L - F_R$) 모멘트
> 대학원 강의는 이 혼동을 피하려고 위치를 $x^n, y^n$ 으로 씀
...
$$
N = y_{\text{ENU}}, \qquad E = x_{\text{ENU}}, \qquad D = -\,z_{\text{ENU}}
$$
...
$$
\begin{aligned}
\Delta N &= (\varphi - \varphi_0)\,R_M \\
\Delta E &= (\lambda - \lambda_0)\,R_N \cos\varphi_0 \\
\Delta D &= -(h - h_0)
\end{aligned}
$$
...
| ② | 피치 Pitch | $\theta$ | 요로 **이미 돌아간** $y$ 축 | 뱃머리가 **들림** | 뱃머리가 들렸다 잠겼다 |
| ③ | 롤 Roll | $\phi$ | 요·피치로 **이미 돌아간** $x$ 축 | 우현이 **내려감** | 좌우로 기울어짐 |

> [!caution] $z$ 가 아래인데 피치 $+$ 가 "뱃머리 들림" 인 이유
> 오른손 법칙으로 $y$ 축을 감으면 $z$ 가 $x$ 쪽으로 감. $z$ 가 아래이므로 $x$(뱃머리)는 위로 감
...
- **요 → 피치 → 롤** 순서로 돌림. 둘째·셋째 회전은 **이미 돌아간 축**을 기준으로 함 — 피치는 요로 돌아간 $y$ 축 둘레, 롤은 요·피치로 돌아간 $x$ 축 둘레
...
| 읽는 법 | 뜻 |
|---|---|
| 선박 규약 $D = -0.500$ | 뱃머리가 동쪽을 향하고 **위로** 30° 들렸음 — 피치 $+$ 가 뱃머리 들림이라는 것과 맞음 |
| 순서만 바꾸면 $D = +0.500$ | 같은 세 숫자인데 뱃머리가 **아래로** 30° 숙였음. 두 선수 방향이 60° 벌어짐 |
...
| 상태 (위치) | $\boldsymbol{\eta} = [N,\ E,\ \psi]^{\mathsf T}$ | 북 · 동 위치, 선수각 | 지구 (NED) |
| 입력 | $F_L,\ F_R$ | 좌 · 우 추진기 추력 [N] | 선체 $x$ 방향 |
| 코드의 상태 벡터 | `s = [u v r N E psi]'` | 위 두 줄을 이어 붙인 것 | — |

- 1-4 절의 $[x,\ y,\ \psi]$ 를 이 절부터는 $[N,\ E,\ \psi]$ 로 씀. 코드의 변수 이름과 맞추기 위함
- 여기서 $N$ 은 두 뜻으로 나옴 — **북쪽 위치** $N$ 과 **요 모멘트** $N$ (1-4 의 경고). 식에서는 모멘트를 $N$, 위치를 $\dot{N}$ 으로만 쓰므로 섞이지 않음
...
- VRX 파일은 ROS 규칙(FLU: $y$ 좌현, $z$ 위)으로 적혀 있음. 그래서 **$y$ 부호를 뒤집어** 읽음

| 항목 | VRX 파일에 적힌 값 (FLU) | 본 과목 선체 축 (FRD) | 출처 |
|---|---|---|---|
| 좌 추진기 | $(-2.374,\ +1.027,\ 0.318)$ | $x = -2.374,\ y = -1.027$ | `wamv_aft_thrusters.xacro` |
| 우 추진기 | $(-2.374,\ -1.027,\ 0.318)$ | $x = -2.374,\ y = +1.027$ | 〃 |
| GPS 안테나 | $(-0.85,\ 0,\ 1.3)$ | $x = -0.85$ | 4주차 1-3 표 |

- 추력은 두 대 모두 선체 $x$ 방향. 모멘트는 "팔 길이 × 힘" 이고, 팔은 $y$ 방향 거리 $b = 1.027$ m

$$
X = F_L + F_R, \qquad Y = 0, \qquad N = -y_L F_L - y_R F_R = b\,(F_L - F_R)
$$

- 좌 추력이 크면 $N > 0$ → 선수가 **우현으로** 돎. 3부 `Mix` 의 부호와 같음
- 추진기 $x$ 위치($-2.374$ m)는 $N$ 에 들어가지 않음. 추력이 $x$ 방향이라 $x$ 팔이 모멘트를 만들지 않기 때문임
...
> | 센서 위치 · 잡음 | GPS 는 $x = -0.85$ m 에 달림 | 제자리 선회에서도 GPS 는 원을 그림 (3-4) |
...
> NED 의 `D` 는 **아래**가 양수임. 배가 기준 고도보다 위에 있으면 `D < 0` 임
...
- `TeleopPad` · `Animate` · `Logging` 은 빌드 스크립트의 **같은 함수**로 만들어짐. 버튼 · 그림 창 · 로그 이름(`log_N` … `log_r`)이 두 모델에서 같음
...
| `Integ` (1/s) | $\dot{\mathbf{s}}$ 를 적분해 상태 $\mathbf{s}$ 를 만듦. 출력이 위로 돌아 `EOM` 의 `s` 로 들어감 |
| `States` | $\mathbf{s}$ 에서 $N, E, \psi, u, v, r$ 을 꺼내 태그로 내보냄. $\psi$ 는 $[-\pi, \pi]$ 로 접음 (`OdomNav` 와 같게) |
...
- `Nav` 가 1부의 변환식을 그대로 적용해 `N`, `E`, `psi`, `r` 을 냄
...
- 배가 가만히 있으면 `N`, `E` 가 거의 변하지 않음. 실행 중 2-6 절 키보드 노드로 밀어 봄
...
> - VRX 의 위치는 **GPS 안테나**(선체 $x = -0.85$ m)의 위치임. 배가 돌면 안테나가 회전 중심 둘레로 원을 그림
>   - 실측 원 반경 0.69 m (GPS 잡음 포함), 원 중심은 25 초 동안 1.49 m 이동
> - 운동모델의 $N$, $E$ 는 **선체 원점**의 위치라 제자리 선회에서 움직이지 않음
...
> 직진 버튼만 누르면 $v \approx 0$ 임. 선회하면 배가 옆으로 밀려 $v$ 가 생김
