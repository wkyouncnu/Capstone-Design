...
> **선수동요**(yaw)는 6번째 **자유도의 이름**이고, **선수각** $\psi$ 는 그 자유도의 **각**이다.
> 같은 줄의 $N$ 은 요 **모멘트**(N·m), $r$ 은 요 **각속도**(rad/s)다. 한 줄에 네 가지가 있다.
>
> **$N$ 도 두 가지로 쓰인다.** 이 표의 $N$ 은 요 **모멘트**이고, NED 의 $N$ 은 **북쪽 위치**다.
> 본 과목은 둘 다 관례대로 $N$ 으로 쓴다. 식을 보면 구분된다 — 위치와 함께 나오면
> ($N - N_k$, $\operatorname{atan2}(E, N)$) 북쪽, 힘과 함께 나오면 ($X,\ Y,\ N$, $F_L - F_R$) 모멘트다.
> 대학원 강의는 이 혼동을 피하려고 위치를 $x^n, y^n$ 으로 쓴다.
...
> $\boldsymbol{\eta}$ 와 $\boldsymbol{\nu}$ 가 **다른 좌표계**라는 점이 이 표기의 핵심이다.
> 두 벡터를 잇는 것이 아래 1-5 의 변환이다.
...
| (4) | $\psi = 0^{\circ}$, 옆으로 밀림 | 1.00 | 0.50 | 1.00 | 0.50 | 북쪽을 보는데 **북동으로 간다** |

- (1)(2)(3) 은 $u$ 가 모두 `1.00` 이다. 바뀐 것은 **선수각뿐**인데 `V_N`, `V_E` 가 달라진다
- 거꾸로 (4) 는 선수각이 (1) 과 같은데 `V_E` 가 생겼다. 배가 **옆으로 밀렸기** 때문이다

> [!important] 선수각과 침로각은 다르다
> - **선수각** $\psi$ — 배가 **어디를 보는가**. 쿼터니언에서 나온다
> - **침로각** $\chi = \operatorname{atan2}(V_E,\ V_N)$ — 배가 **어디로 가는가**
> - 둘의 차이가 **크랩각** $\beta = \operatorname{atan2}(v,\ u)$ 이며 $\chi = \psi + \beta$ 다
> - 옆바람이나 조류가 있으면 $v \neq 0$ 이 되어 둘이 벌어진다.
>   7주차 LOS 유도에서 이 보정을 넣는다

> [!caution] 속도 되먹임에 `V_N`, `V_E` 를 쓰지 않는다
> 속도 제어기는 **`u` 를 되먹인다.** `V_N`, `V_E` 는 배를 돌릴 때마다 값이 바뀌므로
> 게인이 따라다니지 못한다. `u` 는 배를 어느 쪽으로 돌리든 "앞으로 얼마나 빠른가"
> 하나만 뜻한다.
...
|---|---|---|---|---|---|
| ① | 요 Yaw | $\psi$ | $z_b$ (아래) | 뱃머리가 **우현(동)** 쪽으로 돈다 | 뱃머리 방향이 돌아감 — **선수각** |
| ② | 피치 Pitch | $\theta$ | 요로 **이미 돌아간** $y$ 축 | 뱃머리가 **들린다** | 뱃머리가 들렸다 잠겼다 |
| ③ | 롤 Roll | $\phi$ | 요·피치로 **이미 돌아간** $x$ 축 | 우현이 **내려간다** | 좌우로 기울어짐 |

> [!caution] $z$ 가 아래인데 피치 $+$ 가 "뱃머리 들림" 인 이유
> 오른손 법칙으로 $y$ 축을 감으면 $z$ 가 $x$ 쪽으로 간다. $z$ 가 아래이므로 $x$(뱃머리)는 위로 간다.
> 처음 보면 틀린 것 같지만 맞다. 대학원 강의 W01 §1-2 와 Fossen 이 같은 규약이다.
...
- **요 → 피치 → 롤** 순서로 돌린다. 둘째·셋째 회전은 **이미 돌아간 축**을 기준으로 한다 — 피치는 요로 돌아간 $y$ 축 둘레, 롤은 요·피치로 돌아간 $x$ 축 둘레다
- 대학원 강의 W01 §1-3, Fossen, MSS `Rzyx` 가 모두 이 읽는 법이다

> [!note] 식을 오른쪽부터 읽어도 같은 자세가 나온다
> $\mathbf{R}_z\mathbf{R}_y\mathbf{R}_x$ 를 오른쪽부터 읽으면 **고정된 NED 축** 둘레로 롤 → 피치 → 요 순서가 된다.
> 결과 자세는 똑같다. 다만 "움직이는 축을 따라 요부터" 가 배에서 실제로 느끼는 순서와 맞으므로
> 본 과목은 그쪽으로 설명한다. 둘을 섞어 "롤부터, 그런데 돌아간 축 기준" 으로 읽으면 틀린다.
...
- 같은 세 각($\psi = 90^\circ$, $\theta = 30^\circ$, $\phi = 30^\circ$)을 **순서만 바꿔** 돌린다
...
| 읽는 법 | 뜻 |
|---|---|
| 선박 규약 $D = -0.500$ | 뱃머리가 동쪽을 향하고 **위로** 30° 들렸다 — 피치 $+$ 가 뱃머리 들림이라는 것과 맞다 |
| 순서만 바꾸면 $D = +0.500$ | 같은 세 숫자인데 뱃머리가 **아래로** 30° 숙였다. 두 선수 방향이 60° 벌어진다 |
| MSS `Rzyx` 와 차이 0 | 본문의 식이 Fossen 툴박스와 같다 |
| 두 읽는 법 모두 차이 0 | "움직이는 축으로 요부터" 와 "고정된 축으로 롤부터" 는 **같은 행렬**이다 |
...
- 그림의 화살표는 프로펠러가 **물을 미는** 방향이다. 배가 받는 힘은 반대다 — 전진(`w`)에서 화살표가 뒤를 향하는 이유
