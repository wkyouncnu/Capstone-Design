...
git commit -q -m "$MSG" || { echo "[autopush] 커밋할 것 없음"; exit 0; }
echo "[autopush] 커밋: $(git log --oneline -1)"

# ── 푸시 ──────────────────────────────────────────────────────────────────
git remote get-url origin >/dev/null 2>&1 || { echo "[autopush] origin 미설정 — 로컬 커밋만 함"; exit 0; }

BR="$(git rev-parse --abbrev-ref HEAD)"
if GIT_SSH_COMMAND="ssh -o BatchMode=yes" git push -q origin "$BR" 2>/tmp/autopush_err; then
  echo "[autopush] 푸시 완료 → origin/$BR"
else
  echo "[autopush] 푸시 실패 (커밋은 로컬에 남아 있음)"
  sed 's/^/    /' /tmp/autopush_err | head -4
  case "$(cat /tmp/autopush_err)" in
    *publickey*) echo "    → SSH 공개키가 GitHub 에 등록되지 않았다. 스킬 문서의 '처음 한 번' 절 참고" ;;
    *"not found"*|*"does not exist"*) echo "    → 원격 저장소가 없다. GitHub 에서 먼저 만들 것" ;;
  esac
fi
rm -f /tmp/autopush_err
exit 0
