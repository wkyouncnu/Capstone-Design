...
%       출발 포트와 도착 포트 사이를 훑어, **어떤 블록도 그 이름표도 지나지 않고**
%       다른 선의 세로 토막과 x 를 나눠 쓰지 않는 자리를 찾는다. 찾지 못하면 그 선은
%       그대로 둔다. 어설프게 옮겨 다른 선을 망치는 것보다 낫다.
%
%       이름표를 함께 세는 것은 이름이 블록 **밖**에 쓰이기 때문이다. 사각형만 피한
%       통로는 이름 글자 한가운데를 긋는다 — check_lines 의 (7) 이름표 위 선.
%
%   NAME/VALUE
