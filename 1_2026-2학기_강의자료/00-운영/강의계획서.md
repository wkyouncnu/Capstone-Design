...
| `W03_simulink/W03_1~4*.slx` (4개) | 좌표 변환 검산 → 센서 NED 변환 → 추력 부호 확인 → 버튼 조종 | 2개 | 3주 |
| `W04_simulink/W04_1_sensor_rates.slx` · `W04_2_qos_test.slx` | 센서 수신 주기 · QoS 불일치를 Simulink 에서 재측정 | 무관 | 4주 |
...
| `W06_simulink/W06_1~5*.slx` (5개) | 직진 → 선회 → 헤딩 제어 → 속도+헤딩 내부루프 (VRX 4개) + 오프라인 WAM-V | 2개 | 6주 |
...
- Simulink 로 좌표 변환 확인 — `W03_1_frame_check` · `W03_2_vrx_nav` · `W03_3_vrx_drive` · `W03_4_teleop` (모델 4개)
...
- Simulink 로 재측정 — `W04_1_sensor_rates` (수신 주기) · `W04_2_qos_test` (QoS 불일치)
...
- H — `W06_3_heading` 헤딩 제어
- I — `W06_4_inner_loop` 속도 + 헤딩 (inner loop 완성)
- J — `W06_5_offline` 오프라인 WAM-V, `W06_vrx_record.m` 로 VRX 와 대조

**과제** — 과제 6 첫 제어기 응답 측정: ① PID 게인 역할 (P / PD / PID, 미분 필터 `Nf2`, 안티와인드업 `Kb`) ② 헤딩 60° 스텝응답 3조건 ③ 속도 스텝응답 PI vs P ④ 선회 중 속도 저하 (커플링) ⑤ 분석
**자산** — `W06_simulink/` (모델 8개: VRX 연동 4 + 오프라인 1 + PID 입문 3)
...
- D — Δ · R · 전환 판정 바꿔 보기 (`W07_lookahead_demo.m`, `W07_crab_demo.m`)
...
**과제** — atan2 vs LOS × 조류 유·무 4조건 성능표, Δ 스윕(2/3/5/10/20/30 m × 조류 없음·0.4 m/s), 조류 방향 3종의 크랩각, **오프라인 ↔ VRX 대조**
