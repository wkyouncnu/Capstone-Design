...
- 검증 (2026-09-18): 230 s · 312 m 주행 후 궤적 평균 이격 **0.28 m** (경로의 0.1 %), 최대 0.64 m, 정상상태 속도 1.500 vs 1.501 m/s (§9 참조)
...
> 2026-09-18 재측정: 230 s · 312 m 주행 후 궤적 평균 이격 **0.28 m** (경로 길이의 0.1 %), 최대 0.64 m,
...
> - 2026-09-18 재측정: 정착 후 평균 반경오차 오프라인 **2.138 m** / VRX **2.265 m**, 둘 다 2.00 바퀴.
>   궤적 평균 이격 4.96 m (최대 13.49 m)는 같은 원을 조금 다른 속도로 도는 **위상 오차** — 판정은 반경오차·회전수로 한다
...
> - VRX (같은 초기 선수각의 오프라인과 대조): 전이 시각 1 s 안, 임무 종료 312.7 vs 310.6 s, 궤적 평균 이격 0.83 m (최대 3.13 m)
...
> - VRX DP (360 s): 구간 위치 RMS **0.003\~0.007 m**, 오프라인 대비 궤적 평균 이격 **0.16 m** (최대 0.86 m) — 네 주차 중 최소
...
### VRX 실연동 검증 (2026-09-04 최초, 2026-09-18 재측정)

7\~10주차 네 모델을 **실제로 Gazebo 와 연동해** 돌리고 오프라인과 대조했다.
재측정 환경: 데스크톱, WSL `Ubuntu-22.04`, `sydney_regatta`, `ground_truth_enabled=true`, gz RTF 0.96\~0.99, 페이싱 실측 RTF 0.67\~0.70.
오프라인은 VRX 와 같은 초기 선수각으로 다시 돌린 값과 대조한다.

| 주차 | 모델 | 시간 | 오프라인 ↔ VRX 궤적 평균 이격 | 최대 |
|---|---|---|---|---|
| 7 | `W07_1_vrx` | 230 s | **0.28 m** (경로 312 m 의 0.1 %) | 0.64 m |
| 8 | `W08_1_vrx` | 300 s | **4.96 m** (위상 오차) | 13.49 m |
| 9 | `W09_1_vrx` | 420 s | **0.83 m** | 3.13 m |
| 10 | `W10_1_vrx` | 360 s | **0.16 m** | 0.86 m |

- 2026-09-04 의 값(1.11 / 0.31 / 5.07 / 0.25 m)은 스폰 원점 실측 등 코드 수정 이전 값이라 **폐기**
- 10주차가 가장 잘 겹친다 — DP 는 속도가 느리고 과도응답이 적다
- 8주차의 5 m 는 경로 오차가 아니라 **위상 오차**다. 반경오차는 2.138 vs 2.265 m 로 같고, VRX 가 한 바퀴에 약 4 s 빨라 이격이 시간에 거의 직선으로 커짐
- 9주차는 전이 시각 1 s 안, 임무 종료 310.6 vs 312.7 s
...
| VRX DP | 360 s, 페이싱 실측 RTF 0.675 | 위치 RMS **0.003\~0.007 m**, 궤적 평균 이격 **0.16 m** (최대 0.86 m) |
