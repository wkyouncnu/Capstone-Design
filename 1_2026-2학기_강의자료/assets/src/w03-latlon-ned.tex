...
%
% 투영
%   정사영. 카메라가 경도 0 위, 적도에서 T 만큼 올라간 자리에 있다.
%     screen_x = R cos(phi) sin(lambda)
%     screen_y = R ( cos(T) sin(phi) - sin(T) cos(phi) cos(lambda) )
%   3차원 단위벡터 (vx,vy,vz) 는
...
% 화면 좌표 — #1 = 위도, #2 = 경도
\newcommand{\SX}[2]{\R*cos(#1)*sin(#2)}
\newcommand{\SY}[2]{\R*(cos(\T)*sin(#1) - sin(\T)*cos(#1)*cos(#2))}
...
\begin{scope}[shift={(0,0)}]
  ...
  % ---- 적도 : 앞(실선) · 뒤(점선) ----
  \draw[soft, line width=0.35pt, densely dotted, samples=80, domain=90:270, variable=\t]
       plot ({\SX{0}{\t}}, {\SY{0}{\t}});
  \draw[errc, line width=0.7pt, samples=80, domain=-90:90, variable=\t]
       plot ({\SX{0}{\t}}, {\SY{0}{\t}});
  ...
  \draw[guid, line width=0.7pt, samples=60, domain=-90:90, variable=\t]
       plot ({\SX{\t}{0}}, {\SY{\t}{0}});
  \node[font=\scriptsize, guid, anchor=east] at ({\SX{-62}{0}-1}, {\SY{-62}{0}}) {본초자오선};

  % ---- P 의 자오선과 위도선 ----
  \draw[soft, line width=0.45pt, samples=60, domain=-90:90, variable=\t]
       plot ({\SX{\t}{\lP}}, {\SY{\t}{\lP}});
  \draw[soft, line width=0.35pt, densely dashed, samples=70, domain=-90:90, variable=\t]
       plot ({\SX{\pP}{\t}}, {\SY{\pP}{\t}});
...
\begin{scope}[shift={(96,0)}]
  ...
  % P 의 3차원 단위벡터
  \pgfmathsetmacro{\px}{cos(\pP)*cos(\lP)}
  \pgfmathsetmacro{\py}{cos(\pP)*sin(\lP)}
  ...
  \pgfmathsetmacro{\nz}{cos(\pP)}
  % 동 : d(위치)/d(경도)
  \pgfmathsetmacro{\ex}{-sin(\lP)}
  \pgfmathsetmacro{\ey}{cos(\lP)}
