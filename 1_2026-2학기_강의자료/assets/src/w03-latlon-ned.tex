...
\begin{scope}[shift={(96,0)}]
  ...
  \draw[errc, line width=1.0pt, -{Stealth[length=4.5pt,width=3.5pt]}]
       (0,0) -- ({\Rr*\NX}, {\Rr*\NY});
  \node[font=\small, errc] at ({(\Rr+5)*\NX}, {(\Rr+5)*\NY}) {$x$ (북)};
  \draw[errc, line width=1.0pt, -{Stealth[length=4.5pt,width=3.5pt]}]
       (0,0) -- ({\Rr*\EX}, {\Rr*\EY});
  \node[font=\small, errc] at ({(\Rr+5)*\EX}, {(\Rr+5)*\EY}) {$y$ (동)};
  \draw[errc, line width=1.0pt, -{Stealth[length=4.5pt,width=3.5pt]}]
       (0,0) -- ({0.8*\Rr*\DX}, {0.8*\Rr*\DY});
  \node[font=\small, errc] at ({(0.8*\Rr+5)*\DX}, {(0.8*\Rr+5)*\DY}) {$z$ (아래)};
  ...
  \node[note, anchor=west, text width=62mm] at (-32,\R+6)
       {$x$ 는 자오선을 따라 북쪽, $y$ 는 위도선을 따라 동쪽,
        $z$ 는 \textbf{지구 중심}을 향한다};
...
\node[font=\small, anchor=west] at (-28,-\R-22)
     {$\begin{aligned}
        \Delta x &= (\varphi - \varphi_0)\,R_M, \qquad
        \Delta y  = (\lambda - \lambda_0)\,R_N\cos\varphi_0, \qquad
        \Delta z  = -(h - h_0)
       \end{aligned}$};
