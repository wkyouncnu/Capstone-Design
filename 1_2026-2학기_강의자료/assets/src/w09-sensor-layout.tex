% assets/w09-sensor-layout.svg — WAM-V 센서 배치 (실측 좌표 + 시야). 기본 VRX 에 없는 Livox 는 뺐다 (2026-09-18)
%
%   bash _tools/tikz2svg.sh assets/src/w09-sensor-layout.tex assets/w09-sensor-layout.svg
%
% 요점 두 가지
...
\begin{scope}
  ...
  \node[note, pathc, anchor=west] at (30,-21) {카메라 · LiDAR 시야};
  ...
  \node[note, anchor=north] at (17,-1) {$x_b$ 선수};
  \draw[axis, -{Stealth[length=4pt,width=3pt]}] (0,0) -- (0,12);
  \node[note, anchor=east] at (-0.5,12) {$y_b$};
  ...
  \draw[thr, line width=0.7pt] (-2.374*\S-4, -1.027*\S-3) rectangle (-2.374*\S+4, -1.027*\S+3);
  \node[note, thr, anchor=east] at (-2.374*\S-6, 1.027*\S) {추진기 $x_b=-2.37$};
\end{scope}
...
\begin{scope}[shift={(0,-70)}]
  ...
  \node[note, errc, anchor=west] at (0.700*\S+3,1.800*\S) {3D LiDAR  $z_b=1.80$};
  \fill[pathc] (0.750*\S,1.500*\S) circle (1.2);
  \node[note, pathc, anchor=west] at (0.750*\S+3,1.500*\S) {카메라  $z_b=1.50$};

  \draw[soft, line width=0.5pt] (-0.850*\S,0) -- (-0.850*\S,1.300*\S);
  \fill[flow] (-0.850*\S,1.300*\S) circle (1.5);
  \node[note, flow, anchor=east] at (-0.850*\S-3,1.300*\S) {GPS  $z_b=1.30$};

  \fill[thr] (-2.652*\S,-0.191*\S) circle (1.3);
  \node[note, thr, anchor=east] at (-2.652*\S-3,-0.191*\S) {프로펠러 $z_b=-0.19$};
\end{scope}

% ===================== 주의 =====================
\node[wnote, anchor=west, text width=104mm] at (-46,-100)
     {센서가 전부 앞쪽에 몰려 있어 \textbf{후방이 사각지대}다.
      좌표는 VRX 실행 중 TF 에서 측정한 값이며(2026-09-06), 9주차에 직접 바꿔 본다.
