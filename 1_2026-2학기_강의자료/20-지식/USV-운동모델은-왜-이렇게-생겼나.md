...
- $K$ = 선회 이득, $T$ = 시상수 (3주차 $T_r$), $\delta_{\text{rud}}$ = 방향타 각 (6주차 기호. 10주차 추진기 방위각 $\delta_L$, $\delta_R$ 과 다름)
...
- 추진기가 여러 개면 $\boldsymbol{\tau} = \mathbf{T}_e\,\mathbf{f}$ 를 뒤집어야 함 ($\mathbf{f}$ = 추진기별 힘 성분을 모은 확장 추력 벡터, 10주차 1-3절)
- 미지수가 더 많으면 **가중 의사역행렬** (10주차는 $\mathbf{W} = \mathbf{I}$)
...
- 7\~9주차는 **후방 고정 2추진기**라 $X$, $N$ 두 식뿐이고 2×2 역행렬이 손으로 풀림 ($F_L$, $F_R$ = 좌·우 추력)
...
- 10주차는 같은 2기를 **틸팅**(방위각 $\delta_L$, $\delta_R$ 포함)으로 씀 — $\mathbf{f} = [F_{xL}, F_{yL}, F_{xR}, F_{yR}]^{\top}$, $\mathbf{T}_e$ 는 3×4, 계급 3 이라 의사역행렬로 배분
...
- 10주차 틸팅 방위각을 쓰면 옆 힘이 생겨 3자유도를 모두 제어함 (동적위치유지)
